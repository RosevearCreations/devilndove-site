# Devil n Dove AI Handoff — Build 276
...
4. Canadian cosmetic ingredient labels use INCI. The soap-ribbon renderer uses the two side panels as one ordered **INGREDIENTS / INGRÉDIENTS** declaration with a continuation panel instead of duplicating the same list in English and French. If the complete INCI still cannot fit at the tested minimum size, approval is blocked; use an extended/peel-back or other compliant extended ingredient panel rather than clipping ingredients.
...
6. Build 276 increases the actual printed claim-icon/text separation again; the editor spacing and SVG claim panel are both widened.
...
Build 276 supersedes the earlier bilingual side-panel ingredient arrangement. The current soap ribbon uses the two ingredient-side zones as **one ordered INCI declaration**: `INGREDIENTS / INGRÉDIENTS` begins on the left and `CONTINUED / SUITE` uses the right only when needed. The ingredient list itself is INCI and is not duplicated as English/French translations. Claims begin lower in their panel, the net-weight separator/line stays separated from claims, the centre title remains paired with the botanical rose, and ingredient text remains bounded to printable safe zones. If the full declaration cannot fit legibly within the tested two-panel capacity, print approval must fail and an extended compliant ingredient-label method is required.
...
Packaging Studio Build 256 adds a review-first **Create draft from Amazon link** action for soap bases, waxes, essential/fragrance oils, colourants and micas. Amazon-derived data is a convenience draft, never an approval authority: supplier/manufacturer INCI/SDS/allergen evidence must be reviewed before the source template is marked verified. Soap ribbon rendering uses `soap_reference_v3`. Historical Build 256 introduced the two ingredient-side zones around the front oval; Build 276 now treats those zones as part 1 + continuation of one ordered INCI declaration rather than duplicated English/French ingredient lists. Ingredient/claim text remains bounded to its safe zone. Packaging Studio and Media Studio load `v=256` assets.
