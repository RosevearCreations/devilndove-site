# Devil n Dove AI Handoff — Build 279
...
The current code release is **Build 279**. The retained broad D1 feature-migration boundary remains **Build 264** (`database_upgrade_current_pass.sql`). Focused migrations applied after it are Build 269 for CAIP duplicate/integrity support, Build 274 for auditable Creative Process timeline corrections, and Build 276 (`database_build276_packaging_inventory_inci_capacity.sql`) for reference-only Inventory identity on structured Packaging Studio ingredients. Build 276 does **not** create Inventory movements; actual material consumption remains in Creative Process/production. Builds 270–273, Build 275, Builds 277–279 are code/documentation/UI/runtime hardening with no new production D1 migration. Build 279 synchronizes the fresh aggregate analytics schema so UTM columns are no longer dependent on request-time self-healing. Older production databases that still used `payment_refund_id` also require the one-time Build 266 refund compatibility repair before Build 264 can complete. The TMDB movie helper still requires the encrypted Cloudflare secret `TMDB_READ_ACCESS_TOKEN`; the token must never be returned to browser code.

## Build 279 Worker efficiency and go-live authority

1. Build 279 is a preventive Cloudflare CPU/go-live hardening release. Devil n Dove had no observed CPU-limit failures in the inspected production window, but the Free Workers CPU allowance is tight enough that routine work must stay bounded before traffic grows.
2. Public analytics is now deliberately lightweight: no request-time `sqlite_master`/`PRAGMA`/`ALTER TABLE`, no IP hashing, no public analytics on `/admin`, cheap bot exclusion, visitor/session UPSERTs, a 15-minute same-path page-view throttle, and checkout-only abandonment tracking. Analytics fails open and must never block storefront/admin business actions.
3. Checkout recovery writes are throttled to at most one normal write per 60 seconds while editing, with one unload beacon. The duplicate checkout-start event was removed.
4. Admin Live Activity no longer polls every 30 seconds. Smoke-test and release/preflight dashboard cards are opt-in. Standard dashboard and mobile-health summaries request scoped projections rather than the former broad aggregation payload.
5. Shared authenticated GET retries must not automatically retry Cloudflare resource-limit failures. A CPU/resource failure is diagnostic evidence, not an instruction to multiply the same expensive request.
6. Public SEO override, Product Review and Trust Block endpoints no longer perform table-existence/schema introspection on every read. Featured Products caches optional schema capabilities for a bounded isolate lifetime instead of probing all optional tables on every request.
7. CAIP multipart integrity remains fail-closed, but the part hot path is slimmer: successful schema readiness is cached per isolate, per-part responses no longer reload the full part plan, session-wide aggregation runs every eighth part or at the file boundary, and selected control-plane actions use compact responses before a final canonical refresh. The exact expected-part/ETag/contiguous-range/byte-total checks and R2 HEAD size verification remain mandatory before completion.
8. Workers Logs are enabled in `wrangler.toml` at 100% invocation sampling for the current low-volume Free-plan deployment. Build 279 does not enable tracing beta or change the compatibility date. Use Cloudflare native request-path/CPU/outcome fields before adding custom telemetry overhead.
9. No focused Build 279 production D1 migration is required. Build 276 remains the latest Packaging migration, Build 274 Creative Process, Build 269 CAIP. Build 279 does update `database_full_schema.sql` + `fullSchemaRequirements.js` so fresh installs explicitly contain the analytics UTM columns that older production databases received through the Build 151 compatibility path.
10. Post-deploy acceptance is observational: monitor the Production Functions metrics/logs for a representative 24-hour period, require **Exceeded CPU Time Limits = 0**, compare request count/CPU percentiles to the pre-release baseline, and investigate the top request paths before resuming major feature growth.

## Build 278 Media & Content Studio edit-mode and image-plan authority

1. Public/static pages no longer show an Edit badge on every managed location by default. After `dd:admin-ready` confirms an administrator, one fixed **Admin page preview** control appears. **Edit page** toggles every slot badge/outline on or off for that whole page; OFF is the clean presentation preview. The state is stored only for the current browser tab/session.
2. The edit switch is presentation-only. It does not bypass server-side admin authentication, publish content, change D1, or expose Product/Inventory specialist records. Per-slot links still route to Media & Content Studio only after Edit mode is intentionally enabled.
3. Media & Content Studio owns an explicit live **Editable image spaces & outstanding artwork** checklist. It combines the 139-slot deployment catalog with current D1 assignments through bounded `mode=visual_plan`; a custom D1 assignment marks that slot complete even if the source build still contains an SVG placeholder.
4. Build 278's deployment catalog contains **71 image slots + 68 background slots**. Source baseline: 6 P1 required placeholder replacements, 23 P2 recommended placeholder replacements, 68 P3 optional blank backgrounds, and 42 authored/default visual locations. Live counts may improve as D1 assignments are made.
5. Standard authoring targets are 1600×1000 (8:5) for hero/content imagery, 1920×1200 (8:5) for general backgrounds, 1920×480 for navigation backgrounds, 1920×600 for footer backgrounds, and 1080×1080 transparent artwork for the main logo/mark. Keep important subjects inside the documented mobile-safe centre region because responsive cover-cropping may trim edges.
6. `docs/media-content/IMAGE_SPACE_REQUIREMENTS.md` and `.csv` are the static/deployment checklist; the Admin Studio's live checklist is the operational truth after deployment. Product/catalog images, finished-product photos, Inventory, supplies and tools remain intentionally excluded.
7. Build 278 adds no D1 migration. Build 276 remains the latest Packaging schema boundary; Build 274 and Build 269 remain the focused Creative Process and CAIP schema boundaries.
