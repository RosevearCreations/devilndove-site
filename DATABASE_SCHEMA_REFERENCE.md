- No new core tables were introduced in this pass. The strongest work was compatibility hardening around existing `catalog_items`, `products`, `product_seo`, `product_images`, `product_resource_links`, and `site_item_inventory` so partially migrated databases keep the admin UI usable during source-of-truth cleanup.
