...
Still outstanding after Build 183:
1. Visual media picker rows are prepared, but direct R2 browser upload/signing still needs live binding work.
2. Screenshot pair rows and capture jobs exist, but real screenshot upload/capture still needs deployment-side wiring.
3. Visual diff rows are placeholders until image comparison is generated from uploaded screenshots.
4. Image-slot assignments are review rows; publishing into static page content needs a guarded bake step.
5. Compression budgets are visible but not yet enforced in all image upload/promotion endpoints.
6. Alt-text suggestions are copy-ready but do not yet auto-write to product/page records.
7. Schema validation result rows still expect manual validator import after live-page checks.
8. Low-bandwidth mode is browser-local; member/account sync is still outstanding.
9. CSS token drift rows are seeded; automatic token extraction and comparison still needs a parser pass.
10. Cloudflare/D1/R2/email/provider secrets still cannot be live-tested from the static zip.
...
- Reduced D1 rerun risk by adding a Deployment Preflight API/page that detects the Build 171 “schema exists but ledger marker missing” case.
- Added a clean Build 173 additive migration file instead of requiring admins to rerun a large upgrade file against a partially upgraded live D1 database.
- Added static and live preflight checks for one-H1/title/meta/local wording, CSS brace drift, JSON fallback parse health, and core admin page fetch health.
- Added saved preflight snapshots so release safety can be reviewed over time.

Still outstanding:
1. Deployment Preflight does not yet compare every live D1 column against the full SQL schema file automatically.
2. Release notes changed-file lists are still curated from JSON and not yet generated by file hashing/diffing.
3. Public image alt-text and structured-data preview are not yet part of the preflight result.
4. JSON-to-D1 duplicate ownership checks still need a dedicated report for Tools, Supplies, featured items, and SEO bake actions.
5. R2 derivative/evidence health checks are still split across separate admin tools instead of being summarized in Deployment Preflight.
6. Admin dashboard does not yet show a latest-preflight status badge.
7. D1 Console copy blocks need separate fresh-install vs repair-only instructions to reduce manual mistakes.
8. Post-deploy actions are documented but not yet a completed/not-completed workflow with admin/time tracking.
9. Schema file checks are stronger, but live Cloudflare secrets/bindings still need post-deploy verification.
10. Mobile screenshot review remains checklist-based; it does not yet store device/browser dimensions for each row.

Next risk-reduction direction: connect Deployment Preflight, Smoke Tests, Schema Drift, R2 derivative health, and Safe Deploy Package into one release-control center that can export a complete Markdown support packet.
