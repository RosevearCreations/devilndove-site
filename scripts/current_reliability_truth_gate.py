...
endpoint_compact = compact(endpoint)
...
req("production_promotion_proof_count:4" in helper_compact, 'current reliability helper must preserve four-proof Production promotion')
req("rollback_readiness:'release-neutral-read-only'" in helper_compact, 'current reliability helper must preserve release-neutral read-only rollback readiness')
req("inherited_engine_role:'HISTORICAL_REGRESSION_COMPATIBILITY'" in helper_compact, 'legacy reliability engine must be explicitly historical compatibility only')
...
req("from'../_lib/currentReliability.js'" in endpoint_compact, 'current endpoint must use the current reliability projection')
req("mutation_capability:'none'" in endpoint_compact, 'current endpoint must explicitly expose no mutation capability')
