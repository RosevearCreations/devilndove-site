...
req(pointer.get('build') in (233,234),'current authority must identify Build 233 or its authorized Build 234 successor')
if pointer.get('build')==233:
    req(pointer.get('title')=='Universal Help & Quality-of-Life Coverage','Build 233 pointer title drifted')
    ...
    req(last.get('build')==233 and last.get('dev_sha')=='c9882fef84e23f7416a7042f52ec8b5ea151287f','Build 234 successor must retain verified Build 233 closure')
...
req(any(x in mid for x in ('/public/js/admin-context-help.js?v=467b233-universal-help','/public/js/admin-context-help.js?v=467b234-workflow-recovery')),'public middleware must inject Build 233+ help revision')
req(any(x in auth for x in ('/public/js/admin-context-help.js?v=467b233-universal-help','/public/js/admin-context-help.js?v=467b234-workflow-recovery')),'Creator/Admin bootstrap must load Build 233+ help revision')
...
req(len(manifest.get('migrations') or [])>=22 and (manifest.get('migrations') or [])[21].get('file')=='0022_release467_capability_profile_coverage_closure.sql','Build 233 canonical prefix must be retained; successor migrations may follow')
