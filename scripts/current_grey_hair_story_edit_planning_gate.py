...
API=ROOT/'functions/api/admin/grey-hair-story-edit-planning.js'; PAGE=ROOT/'admin/grey-hair-story-edit-planning/index.html'; CLIENT=ROOT/'public/js/admin-grey-hair-story-edit-planning-v47.js'; SYNC_PAGE=ROOT/'admin/grey-hair-sync-alignment/index.html'; LEGACY_MIGRATION=ROOT/'migrations/dev/20260830_release461_caip_production_pipeline.sql'; CANONICAL=ROOT/'migrations/canonical'
for path in (API,PAGE,CLIENT,SYNC_PAGE,LEGACY_MIGRATION):
    if not path.is_file(): raise SystemExit(f'FAIL — missing Build 47 authority: {path.relative_to(ROOT)}')
api=API.read_text(encoding='utf-8'); page=PAGE.read_text(encoding='utf-8'); client=CLIENT.read_text(encoding='utf-8'); migration=LEGACY_MIGRATION.read_text(encoding='utf-8'); sync_page=SYNC_PAGE.read_text(encoding='utf-8')
forbidden_ddl=re.compile(r'\b(?:CREATE|ALTER|DROP|RENAME)\s+(?:TABLE|INDEX|TRIGGER|VIEW)\b',re.I)
if forbidden_ddl.search(api): raise SystemExit('FAIL — Build 47 API contains request-time schema DDL')
if forbidden_ddl.search(client): raise SystemExit('FAIL — Build 47 client contains schema DDL')
...
if canonical_files[:4]!=baseline: raise SystemExit(f'FAIL — Build 47 canonical baseline changed; found {canonical_files!r}')
checks={
...
'generated story remains draft':"story_status: 'draft'" in api and "VALUES(?,?,?,'draft'" in api,
...
'fresh authoritative readback after writes':'...(await bundle(state.db, projectId, groupId))' in api,
...
if failed: raise SystemExit('FAIL — Build 47 story/edit planning regression: '+'; '.join(failed))
