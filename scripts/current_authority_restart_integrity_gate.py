...
for path in ROOT.glob('release467-build*.json'):
    m=AUTHORITY_RE.match(path.name)
    if m: authority_builds.append((int(m.group(1)),path.name))
if authority_builds:
    newest=max(x[0] for x in authority_builds)
    req(build==newest,f'current authority pointer Build {build} must match newest Release 467 authority Build {newest}')
    named=pointer.get('current_release_authorities') or []
    req(bool(named),'current authority must list current release authorities')
    if named:
        m=AUTHORITY_RE.match(Path(str(named[0])).name)
        req(bool(m) and int(m.group(1))==build,'first current release authority must match current pointer build')
...
bound_build102=(
    build==103 and last_build==102 and last_tree==BUILD102_TREE and last_runs==BUILD102_PROOFS
...
req(bound_build102 or bool(SHA_RE.match(last_sha)),'last fully verified checkpoint must carry an exact commit identity')
req(bool(SHA_RE.match(last_tree)),'last fully verified checkpoint must carry an exact tree SHA')
for key in BUILD102_PROOFS:
...
bound_prod_build102=(bound_build102 and prod_build==102 and prod_tree==BUILD102_TREE and prod_run==BUILD102_PAGES and prod_live==BUILD102_LIVE and BUILD102_SHA in pointer_text)
...
req(bound_prod_build102 or bool(SHA_RE.match(prod_sha)),'Production checkpoint must carry an exact commit identity')
...
if authority_path:
    authority=load(authority_path); final=authority.get('final_closure') or {}; authority_text=read(authority_path)
...
if prod_build==last_build and prod_authority_path:
    pa=load(prod_authority_path); pp=pa.get('production_checkpoint') or {}; authority_text=read(prod_authority_path)
...
for path in doc_paths:
    ...
    for run in last_runs.values(): req(str(run) in text,f'{path} missing last fully verified proof run {run}')
    req(str(prod_run) in text,f'{path} missing current Production proof run')
    if prod_live: req(str(prod_live) in text,f'{path} missing current Production live-resource proof run')
    if not bound_build102: req(last_sha in text,f'{path} missing last fully verified dev SHA')
    if not bound_prod_build102: req(prod_sha in text,f'{path} missing current Production main SHA')
...
for text,label in ((it_api,'I.T. API'),(preflight,'Deployment Preflight'),(reliability,'Reliability')):
    ...
    if bound_build102 and bound_prod_build102:
        for run in BUILD102_PROOFS.values(): req(str(run) in text,f'{label} missing bounded Build 102 Development proof {run}')
...
if bound_build102: print('Build 102 checkpoint: IMMUTABLE MACHINE AUTHORITY + EXACT TREE/SIX-PROOF BINDING')
