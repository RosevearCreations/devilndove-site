#!/usr/bin/env python3
"""Build 441 source acceptance across Product, Inventory, Tool and responsive Admin mutation paths.

Build 440 implementation assets remain accepted subsystem provenance until those files are
touched. Build 441 is the active release authority, so this gate validates the current
release while preserving the already-proven Product/Inventory/Tool contracts.
"""
...
def accepted_asset(html: str, asset_name: str) -> bool:
    """Accept Build 440 subsystem provenance or the current Build 441 cache major."""
    match = re.search(
        rf"{re.escape(asset_name)}\?v=(\d+)(?:\.\d+)?(?![\d.])",
        html,
    )
    if not match:
        return False
    major = int(match.group(1))
    return 440 <= major <= release
...
check('canonical acceptance release is Build 441', release == 441)
...
# Desktop/tablet/mobile must keep the proven authority bundles and responsive mutation controls.
...
print(f'\nBUILD 441 CROSS-MUTATION / RESPONSIVE ACCEPTANCE: {passed}/{len(checks)} passed')
...
print('Build 440 cache majors: ACCEPTED SUBSYSTEM PROVENANCE UNDER BUILD 441')
print('Mobile/Desktop: RESPONSIVE / SAME MUTATION AUTHORITIES')
