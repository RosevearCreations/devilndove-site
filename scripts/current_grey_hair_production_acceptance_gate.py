...
API=ROOT/'functions/api/admin/grey-hair-production-acceptance.js'; PAGE=ROOT/'admin/grey-hair-production-acceptance/index.html'; CLIENT=ROOT/'public/js/admin-grey-hair-production-acceptance-v48.js'; BUILD47_API=ROOT/'functions/api/admin/grey-hair-story-edit-planning.js'; BUILD47_PAGE=ROOT/'admin/grey-hair-story-edit-planning/index.html'; CANONICAL=ROOT/'migrations/canonical'
for path in (API,PAGE,CLIENT,BUILD47_API,BUILD47_PAGE):
    if not path.is_file(): raise SystemExit(f'FAIL — missing Build 48 authority: {path.relative_to(ROOT)}')
api=API.read_text(encoding='utf-8'); page=PAGE.read_text(encoding='utf-8'); client=CLIENT.read_text(encoding='utf-8'); b47_api=BUILD47_API.read_text(encoding='utf-8'); b47_page=BUILD47_PAGE.read_text(encoding='utf-8')
forbidden_ddl=re.compile(r'\b(?:CREATE|ALTER|DROP|RENAME)\s+(?:TABLE|INDEX|TRIGGER|VIEW)\b',re.I)
if forbidden_ddl.search(api): raise SystemExit('FAIL — Build 48 API contains request-time schema DDL')
if forbidden_ddl.search(client): raise SystemExit('FAIL — Build 48 client contains schema DDL')
...
if canonical_files[:4]!=baseline: raise SystemExit(f'FAIL — Build 48 canonical baseline changed; found {canonical_files!r}')
checks={
...
if failed: raise SystemExit('FAIL — Build 48 automated production acceptance regression: '+'; '.join(failed))
