...
q(p.get('build')==231 and p.get('title')=='Workshop Journal & Capability Case-Study Activation','pointer identity')
q(p.get('promotion_state')=='BUILD231_CANDIDATE_NOT_YET_VERIFIED','pointer fail-closed state')
q(b.get('state')=='DEVELOPMENT_CANDIDATE','Build 231 authority candidate state')
...
mf=[x.get('file') for x in m.get('migrations',[]) if isinstance(x,dict)];q(len(mf)==22 and mf[-1]=='0022_release467_capability_profile_coverage_closure.sql','Build 231 must add no migration')
