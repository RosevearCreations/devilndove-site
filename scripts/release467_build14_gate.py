...
Build 14 is exact while current. Newer Release 467 builds must preserve its manifest, runtime
quality/crop/marketplace contracts and safety boundary without inheriting its changed-file allowlist.
"""
...
pointer=load("current-development-authority.json");manifest=load("release467-build14-product-release-quality.json");compat=load("development-release.json");migrations=load("migrations/canonical/manifest.json")
b13=read("scripts/release467_build13_gate.py");b13workflow=read(".github/workflows/release467-build13-proof.yml");quality=read("public/js/admin-product-quality-command-center.js");products_page=read("admin/products/index.html");readiness=read("functions/api/admin/product-readiness.js");market=read("functions/api/_lib/marketplaceReadiness.js");media=read("public/js/admin-product-images.js");export_preview=read("functions/api/admin/marketplace-export-preview.js");backlog=read("docs/operations/RELEASE_467_AUTONOMOUS_20_ITEM_BACKLOG.md")
...
req(pointer.get("release")==467 and pointer_build>=14,"current authority must remain Release 467 Build 14 or newer")
...
if pointer_build==14:
    ...
    req(pointer.get("last_green_build")==13 and pointer.get("last_green_dev_sha")==GREEN13_SHA and pointer.get("last_green_dev_tree_sha")==GREEN13_TREE,"Build 13 green predecessor drifted")
...
runtime=manifest.get("runtime") or {};req(runtime.get("workspace")=="/admin/products/" and runtime.get("command_center_read_only") is True and runtime.get("automatic_repair") is False,"Build 14 command center contract drifted");req(runtime.get("original_r2_media_preserved") is True,"Build 14 must preserve original R2 media");req(runtime.get("provider_execution") is False and runtime.get("provider_publication") is False,"marketplace provider execution/publication must remain false")
for k in ("schema_change_authorized","request_time_schema_mutation","new_d1_mutation_authorized","d1_mutation_authorized","r2_mutation_authorized","provider_execution_authorized","provider_publication_authorized","cloudflare_access_policy_mutation_authorized","main_mutation_authorized","production_mutation_authorized","secret_values_emitted"):req(manifest.get(k) is False,f"Build 14 manifest safety drift: {k}")

b13c=b13.replace(" ","");req("pointer_build>=13" in b13c and "ifpointer_build==13" in b13c,"Build 13 gate is not forward-compatible");req("pointer_build" in b13workflow and 'if [ "${pointer_build}" -eq 13 ]' in b13workflow,"Build 13 workflow is not forward-compatible");req("runtime scope is governed by the newer build gate" in b13workflow,"Build 13 preservation-mode boundary missing")
for token in ("Product Release Quality Command Center","/api/admin/products","/api/admin/product-readiness?limit=300&show_ready=1","Crop / focal","Proof-image recommendations","Marketplace image checks","read-only","nothing is published automatically","linked_resource_cost_cents","gross_margin_cents","inventory","hero","gallery","alt","seo","canonical","shipping","structured","marketplace"):req(token.lower() in quality.lower(),f"Build 14 quality UI marker missing: {token}")
req("productQualityCommandCenterMount" in products_page and "/public/js/admin-product-quality-command-center.js" in products_page,"Build 14 Product Quality mount/script missing")
for token in ("marketplace_image_readiness","image_recommendations","duplicate_image_url_count","packaging_pickup_role_count","material_tool_proof_role_count","request_time_schema_mutation:false","release:467","build:14"):req(token in readiness,f"Build 14 readiness marker missing: {token}")
req("CREATE TABLE" not in readiness.upper() and "ALTER TABLE" not in readiness.upper(),"Build 14 readiness GET contains request-time DDL")
for token in ("validateSelectedImageSet","duplicate marketplace image URL","missing useful alt text","not cleared for public use","800×800","1200×1200","merchandising score is below 70%","provider_execution_allowed","publication_allowed"):req(token in market,f"Build 14 marketplace image validation marker missing: {token}")
for token in ("product-image-focal-thumb","data-row-set-square-crop","data-row-queue-derivative","focal_point_x","focal_point_y","crop_x","crop_y","crop_width","crop_height","Derivative history"):req(token in media,f"non-destructive crop/focal authority missing: {token}")
for token in ("validateListingDraft","format')==='csv'","hardBlocked","provider_execution:false","publication_allowed:false"):req(token in export_preview,f"marketplace export fail-closed authority missing: {token}")
for n in range(1,21):req(f"\n{n}. **" in backlog,f"autonomous backlog item {n} missing")
for token in ("Build 14","Product Release Quality Command Center","Stripe Development","PayPal sandbox","Social/OAuth","Cloudflare Access","Production promotion","HOLD_EXTERNAL"):req(token in backlog,f"autonomous backlog boundary missing: {token}")
req([x.get("file") for x in migrations.get("migrations",[])]==EXPECTED,"canonical migrations drifted")
for body in docs:
    for token in ("Release 467 Build 14","Product Release Quality",GREEN13_SHA,"Release 467 Build 13","Repository Hygiene","374983f68fb16172fb357b1755293a29e5d2953f","HOLD_EXTERNAL"):req(token in body,f"Build 14 documentation token missing: {token}")
if pointer_build==14:
    allowed={"functions/api/admin/product-readiness.js","functions/api/_lib/marketplaceReadiness.js","public/js/admin-product-quality-command-center.js","admin/products/index.html","current-development-authority.json","release467-build14-product-release-quality.json","scripts/release467_build13_gate.py","scripts/release467_build14_gate.py",".github/workflows/release467-build13-proof.yml",".github/workflows/release467-build14-proof.yml","AI_HANDOFF.md","PROJECT_STATUS_AND_ROADMAP.md","SANITY_HEALTH_CHECK.md","MARKDOWN_INDEX.md","docs/operations/RELEASE_467_BUILD_14_PRODUCT_RELEASE_QUALITY.md","docs/operations/RELEASE_467_AUTONOMOUS_20_ITEM_BACKLOG.md"}
