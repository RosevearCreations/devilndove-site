...
API=ROOT/'functions/api/admin/grey-hair-content-studio-handoff.js'; PAGE=ROOT/'admin/grey-hair-content-studio-handoff/index.html'; CLIENT=ROOT/'public/js/admin-grey-hair-content-studio-handoff-v50.js'; BUILD48_API=ROOT/'functions/api/admin/grey-hair-production-acceptance.js'; BUILD48_PAGE=ROOT/'admin/grey-hair-production-acceptance/index.html'; CONTENT_API=ROOT/'functions/api/admin/content-studio.js'; CONTENT_HELPER=ROOT/'functions/api/_lib/contentAutomationStudio.js'; AUTHORITY=ROOT/'release467-build50-reviewed-caip-content-studio-handoff.json'; CANONICAL=ROOT/'migrations/canonical'
for path in (API,PAGE,CLIENT,BUILD48_API,BUILD48_PAGE,CONTENT_API,CONTENT_HELPER,AUTHORITY):
    if not path.is_file(): raise SystemExit(f'FAIL — missing Build 50 authority: {path.relative_to(ROOT)}')
api=API.read_text(encoding='utf-8'); page=PAGE.read_text(encoding='utf-8'); client=CLIENT.read_text(encoding='utf-8'); b48_api=BUILD48_API.read_text(encoding='utf-8'); b48_page=BUILD48_PAGE.read_text(encoding='utf-8'); content_api=CONTENT_API.read_text(encoding='utf-8'); helper=CONTENT_HELPER.read_text(encoding='utf-8'); authority=AUTHORITY.read_text(encoding='utf-8')
forbidden_ddl=re.compile(r'\b(?:CREATE|ALTER|DROP|RENAME)\s+(?:TABLE|INDEX|TRIGGER|VIEW)\b',re.I)
if forbidden_ddl.search(api) or forbidden_ddl.search(client): raise SystemExit('FAIL — Build 50 API/client contains request-time schema DDL')
...
if canonical_files[:4]!=baseline: raise SystemExit(f'FAIL — Build 50 canonical baseline changed; found {canonical_files!r}')
checks={
...
'Build 50 historical authority records its inherited four-migration baseline':'"canonical_d1_migration_count": 4' in authority,
...
if failed: raise SystemExit('FAIL — Build 50 reviewed CAIP Content Studio handoff regression: '+'; '.join(failed))
