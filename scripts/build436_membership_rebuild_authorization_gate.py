#!/usr/bin/env python3
"""Build 436/437 local-only Membership rebuild authorization gate."""
...
def main() -> int:
    print('BUILD 436/437 TWENTY-ITEM MEMBERSHIP BUILD 395 REBUILD AUTHORIZATION GATE')
    ...
    check(bool(b436), 'Build 436/437 rebuild authorization preflight artifact exists')
    check(b436.get('build435_mapping_green') is True and b436.get('membership_row_count') == 3, 'rebuild preflight is anchored to the green three-row Build 435 mapping')
    check(len(str(b436.get('source_rows_sha256') or '')) == 64 and b436.get('source_rows_sha256') == b435.get('source_rows_sha256'), 'source fingerprint matches fresh Build 435 evidence')
    check(len(str(b436.get('canonical_preview_sha256') or '')) == 64, 'canonical-preview SHA-256 boundary is present')
    ...
    check(
        b436.get('legacy_sort_index_compatible') is True
        and b436.get('legacy_sort_index_columns') == EXPECTED_LEGACY_SORT_COLUMNS
        and b436.get('no_unhandled_user_objects') is True,
        'reviewed legacy Membership sort index is handled and no other user index/trigger exists',
    )
    ...
    print()
    if failures:
        print(f'BUILD 436/437 TWENTY-ITEM MEMBERSHIP BUILD 395 REBUILD AUTHORIZATION GATE: FAIL ({len(failures)}/{checks} failed)')
        for failure in failures:
            print(' -', failure)
        return 1

    print(f'BUILD 436/437 TWENTY-ITEM MEMBERSHIP BUILD 395 REBUILD AUTHORIZATION GATE: PASS ({checks}/{checks})')
    ...
    print('Legacy sort index: HANDLED / WILL BE RECREATED ON tier_code')
    ...
    print(f'NEXT: exact token {AUTH_TOKEN} is required before the guarded backup/apply sequence.')
    return 0
