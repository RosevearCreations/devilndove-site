...
assert.equal(items.length,42,'all 37 prior gates plus five standalone process gates are required');
assert.equal(new Set(items.map((row)=>row.key)).size,42,'gate keys must be unique');
assert.match(api,/const BUILD = '228'/);
...
for(const response of [new Response('<!doctype html><title>Fallback page</title>',{status:200}),Response.json({ok:true,build:'228',expected_total:42,items:[]})]){
  const nodes=await renderWithResponse(response);assert.equal((nodes.startupReadinessMount.innerHTML.match(/data-key=/g)||[]).length,42);assert.match(nodes.startupReadinessMount.innerHTML,/Complete Deployment Preflight as a standalone/);assert.match(nodes.startupReadinessMount.innerHTML,/Run Go-Live Execution as a standalone/);assert.doesNotMatch(nodes.startupReadinessMount.innerHTML,/No readiness items match these filters/);
}
