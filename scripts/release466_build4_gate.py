...
if state=='technical_green_external_acceptance_pending':
 ...
 req(int(final_ev.get('canonical_migrations') or 0)==4 and int(final_ev.get('new_migrations_applied') or -1)==0 and int(final_ev.get('development_foreign_key_violations') or -1)==0,'Build 4 final D1 evidence drifted')
 ...
 req(stripe.get('accepted') is False and int(stripe.get('required_checks') or 0)==6 and int(stripe.get('passed_checks') or -1)==0 and int(stripe.get('successful_provider_refunds') or -1)==0,'Stripe must remain truthful 0/6 HOLD with zero synchronized refunds')
 req(paypal.get('accepted') is False and int(paypal.get('required_checks') or 0)==6 and int(paypal.get('passed_checks') or -1)==0 and int(paypal.get('successful_provider_refunds') or -1)==0,'PayPal must remain truthful 0/6 HOLD with zero synchronized refunds')
