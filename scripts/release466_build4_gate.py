...
if state=='technical_green_external_acceptance_pending':
 ...
 req(a.get('final_technical_closure_sha')=='0ca68e19339da198e25dd7d3d603a2e616bc77ec','Build 4 final technical closure SHA drifted')
 req(final_ev.get('source_sha')=='0ca68e19339da198e25dd7d3d603a2e616bc77ec' and int(final_ev.get('system_gate_run') or 0)==33469027060 and int(final_ev.get('build4_proof_run') or 0)==33469026988,'Build 4 final technical closure evidence drifted')
 req(int(final_ev.get('build4_proof_artifact_id') or 0)==9785894659 and final_ev.get('build4_proof_artifact_sha256')=='de5bc1ea32eeb5ec6584ca7b8f73cd082c5e9939c2ce38db63bf8c98d1907ad2','Build 4 final artifact evidence drifted')
 req(external.get('launch_state')=='HOLD_EXTERNAL_ACCEPTANCE','Build 4 launch state must remain truthful HOLD')
 req((external.get('caip_private_media') or {}).get('accepted') is True,'CAIP acceptance drifted')
 req((external.get('stripe') or {}).get('accepted') is False and int((external.get('stripe') or {}).get('current_required_checks_after_hardening') or 0)==6,'Stripe must remain HOLD with six-part hardening contract')
 req((external.get('paypal') or {}).get('accepted') is False and int((external.get('paypal') or {}).get('current_required_checks_after_hardening') or 0)==6,'PayPal must remain HOLD with six-part hardening contract')
 ...
 req(hardening.get('failed_provider_refund_changes_payment_or_order_status') is False and hardening.get('schema_change_required') is False,'refund failure/schema safety drifted')
