...
for token in ("CURRENT_RELEASE=461","SESSION_ENV='DND_DEV_SESSION_COOKIE'","/api/admin/site-item-inventory","/api/admin/product-media-score","/api/admin/caip-production-pipeline","/api/admin/caip-content-handoff?creative_project_id=","quantity_authority')=='base'","primary_image_thresholds","min_width_px') or 0)==1200","min_quality_score') or 0)==70","r2_delete_active') is False","production_mutation':'FORBIDDEN'"):
...
print('Inventory authority contract: quantity_authority=base / row base authority')
print('Product media contract: HTTP 200 fail-closed migration readiness + exact primary thresholds')
