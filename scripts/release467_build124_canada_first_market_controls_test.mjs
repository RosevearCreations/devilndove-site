...
const usBilling=validateCommerceEnvelope({currency:'CAD',billing_country:'United States',shipping_country:'CA'});check(!usBilling.ok && usBilling.code==='billing_country_blocked' && usBilling.requested_country_code==='US','U.S. billing fails with explicit blocked-country code');
const usShipping=validateCommerceEnvelope({currency:'CAD',billing_country:'CA',shipping_country:'USA'});check(!usShipping.ok && usShipping.code==='shipping_country_blocked' && usShipping.requested_country_code==='US','U.S. shipping fails with explicit blocked-country code');
const otherMarket=validateCommerceEnvelope({currency:'CAD',billing_country:'France',shipping_country:'France'});check(!otherMarket.ok && otherMarket.code==='billing_country_not_supported','other markets remain unsupported until deliberately enabled');
const usAddress=validateCanadianAddress({country:'United States',province:'NY',postal_code:'10001'},{label:'Shipping'});check(!usAddress.ok && usAddress.code==='commerce_country_blocked','U.S. shipping address fails before province/postal validation');
