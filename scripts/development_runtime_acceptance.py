#!/usr/bin/env python3
"""Release-independent, Development-only runtime acceptance for Devil n Dove.
...
def validate_base_url(value: str) -> str:
    ...
    if parsed.scheme != "https" or host not in ALLOWED_HOSTS or parsed.path not in ("", "/"):
        raise AcceptanceError(
            "Refusing runtime target. Only the exact Development Pages host is allowed; "
...
def get_json(base_url: str, path: str, cookie: str | None, timeout: float = 20.0) -> tuple[int, dict]:
    headers = {
        ...
        "User-Agent": "devilndove-development-runtime-acceptance",
    }
    if cookie:
    ...
    try:
        ...
        raise AcceptanceError(f"GET {path} returned non-JSON content (HTTP {status}).") from error
    if not isinstance(payload, dict):
        raise AcceptanceError(f"GET {path} returned a non-object JSON payload.")
...
def provider_readiness(payload: dict) -> dict:
    ...
    stripe_ready = bool(
        stripe.get("ready")
        and stripe.get("webhook_ready")
        and str(stripe.get("environment") or "").lower() == "test"
    )
    paypal_ready = bool(
        paypal.get("ready")
        and paypal.get("webhook_ready")
        and str(paypal.get("mode") or "").lower() == "sandbox"
    )
    return {
        "stripe": {
            "configuration_readiness": "READY" if stripe_ready else "HOLD",
            "provider_acceptance": "HOLD",
            "required_mode": "test",
        },
        "paypal": {
            "configuration_readiness": "READY" if paypal_ready else "HOLD",
            "provider_acceptance": "HOLD",
...
def run_anonymous_check(base_url: str, timeout: float) -> dict:
    checks: list[dict] = []
    for name, path in AUTH_ENDPOINTS.items():
        status, _ = get_json(base_url, path, cookie=None, timeout=timeout)
        passed = status in (401, 403)
        record(checks, f"anonymous_{name}_refused", passed, f"HTTP {status}; expected 401/403")
    return {
...
def run_authenticated(base_url: str, cookie: str, timeout: float) -> dict:
    ...
    payment_status, payment_payload = get_json(
        base_url, PUBLIC_ENDPOINTS["payment_providers"], cookie=None, timeout=timeout
    )
    record(
        checks,
        "payment_provider_readiness_contract",
        payment_status == 200 and payment_payload.get("ok") is True,
        f"HTTP {payment_status}; safe non-secret readiness only",
    )
    ...
    record(
        checks,
        "development_target",
        infra.get("target") == "development" and infra.get("project") == "devilndove-site-dev",
        f"target={infra.get('target')!r} project={infra.get('project')!r}",
    )
    record(
        checks,
        "d1_r2_readiness",
        infra.get("ready") is True
        and infra.get("d1", {}).get("schema_ready") is True
        and len(infra.get("r2", [])) == 2
        and all(row.get("storage_ready") is True for row in infra.get("r2", [])),
        "D1 schema plus both Development R2 bindings",
    )
    record(
        checks,
        "no_runtime_migration",
        infra.get("migration", {}).get("required") is False
        and infra.get("current_release_sql_required") is False,
        f"migration_required={infra.get('migration', {}).get('required')!r}",
    )
    mutation = infra.get("mutation_policy", {})
    record(
        checks,
        "read_only_runtime_policy",
        mutation.get("d1_write") is False
        and mutation.get("r2_write") is False
        and mutation.get("provider_write") is False
        and mutation.get("destructive_probe_performed") is False,
        "D1/R2/provider writes disabled",
    )
    ...
    module_keys = sorted(
        str(row.get("module_key") or "").lower()
    ...
    record(
        ...
        "five_module_runtime",
        modules_status == 200
        and modules.get("ok") is True
        and int(modules.get("release") or 0) == EXPECTED_RELEASE
        and module_keys == sorted(EXPECTED_MODULES)
        and diagnostics.get("healthy") is True
        and int(diagnostics.get("role_access_count") or 0) == 10,
        f"HTTP {modules_status}; modules={module_keys}; roles={diagnostics.get('role_access_count')}",
    )
    record(
        checks,
        "socials_runtime_authority",
        "socials" in module_keys,
        "Socials is present in the canonical authenticated module authority; no publish mutation is executed.",
    )

    for name, expected_contract in (
        ("storefront", "catalog-read"),
        ("creators", "content-media"),
        ("financials", "accounting-read"),
    ):
        status, payload = get_json(base_url, AUTH_ENDPOINTS[name], cookie=cookie, timeout=timeout)
        passed = status == 200 and payload.get("ok") is True and payload.get("contract") == expected_contract
        if name == "financials":
            passed = passed and int(payload.get("release") or 0) == EXPECTED_RELEASE and payload.get("schema_ready") is True
        record(checks, f"{name}_read_contract", passed, f"HTTP {status}; contract={payload.get('contract')!r}")

    record(
        checks,
        "it_runtime_authority",
        infra_status == 200 and infra.get("ok") is True and "it-platform" in module_keys,
        "I.T. covered by authenticated infrastructure plus canonical module authority.",
    )
    ...
    return {
        ...
        "http_method": "GET",
        "credentials_source": SESSION_ENV,
        "credentials_emitted": False,
        "core_runtime": "PASS" if core_pass else "FAIL",
        "checks": checks,
        "provider_readiness": provider_readiness(payment_payload),
        "provider_transaction_acceptance": "NOT_PERFORMED",
        "caip_private_media_acceptance": "NOT_PERFORMED",
...
def self_check() -> int:
    ...
    for forbidden in (
        ...
        "https://example.com",
        "http://devilndove-site-dev.pages.dev",
    ):
        refused = False
        try:
    ...
    record(checks, "get_only_manifest", all(path.startswith("/api/") for path in [*AUTH_ENDPOINTS.values(), *PUBLIC_ENDPOINTS.values()]), "runtime endpoints are GET-only in this harness")
...
def main() -> int:
    ...
    parser.add_argument("--self-check", action="store_true", help="Run local safety/manifest checks only; no network.")
    parser.add_argument("--anonymous-check", action="store_true", help="GET protected Development routes without credentials and require 401/403.")
    ...
    try:
        ...
        if args.anonymous_check:
            evidence = run_anonymous_check(base_url, args.timeout)
        else:
            cookie = os.environ.get(SESSION_ENV, "").strip()
            if not cookie:
                raise AcceptanceError(
                    f"{SESSION_ENV} is required for authenticated acceptance. "
                    "Do not pass cookies/tokens on the command line or commit them to the repository."
                )
    ...
    if args.evidence_json:
        ...
        print(f"Sanitized evidence: {args.evidence_json}")
    ...
    if "provider_readiness" in evidence:
        print("Provider readiness is reported separately; no payment/provider mutation was performed.")
