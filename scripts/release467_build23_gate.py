#!/usr/bin/env python3
"""Fail-closed source contract for Release 467 Build 23 — Creator ↔ Finance Profitability Reconciliation."""
...
req(p.get('release')==467 and p.get('build')==23 and p.get('title')==TITLE,'Build 23 pointer identity drifted');req(p.get('state') in ('DEVELOPMENT_CANDIDATE','DEVELOPMENT_GREEN'),'Build 23 pointer state invalid')
req(p.get('source_base_sha')==BASE_SHA and p.get('source_base_tree_sha')==BASE_TREE,'Build 23 source base drifted');req(p.get('last_green_build')==22 and p.get('last_green_dev_sha')==BASE_SHA and p.get('last_green_dev_tree_sha')==BASE_TREE,'Build 22 final closure predecessor drifted');req(p.get('last_green_system_gate_run')==B22_SYSTEM and p.get('last_green_build_proof_run')==B22_PROOF and p.get('last_green_branch_hygiene_run')==B22_HYGIENE,'Build 22 final run evidence drifted')
req(p.get('main_source_head_last_verified')==PROD and p.get('production_tree_last_verified')==PROD_TREE and p.get('production_pages_deploy_last_verified')==PROD_DEPLOY,'Production Build 20 proof drifted');req(p.get('promotion_state')=='NO_AUTOMATIC_PROMOTION','automatic Production promotion forbidden')
...
req(m.get('release')==467 and m.get('build')==23 and m.get('title')==TITLE and m.get('role')=='READ_ONLY_CROSS_MODULE_PROFITABILITY_RECONCILIATION','Build 23 manifest identity drifted');pr=m.get('predecessor') or {};req(pr.get('merged_dev_sha')==BASE_SHA and pr.get('merged_dev_tree_sha')==BASE_TREE and pr.get('system_gate_run')==B22_SYSTEM and pr.get('build22_proof_run')==B22_PROOF and pr.get('branch_hygiene_run')==B22_HYGIENE,'Build 23 predecessor evidence drifted');req(pr.get('production_main_sha')==PROD and pr.get('production_tree_sha')==PROD_TREE and pr.get('production_pages_deploy_run')==PROD_DEPLOY,'Build 23 Production provenance drifted')
...
req('onRequestPost' not in api,'Build 23 endpoint must expose no POST handler');req(len(re.findall(r'<h1(?:\s|>)',page,re.I))==1,'Build 23 page must contain exactly one H1');hasall(page,['Release 467 Build 23','Creator ↔ Finance Profitability Reconciliation','does not post accounting entries'],'Build 23 page');hasall(client,['/api/admin/project-profitability-reconciliation','Read-only; no posting occurred.','A non-zero variance means review the evidence owners.'],'Build 23 client');req("method:'POST'" not in client.replace(' ',''),'Build 23 client must not POST');req('/admin/project-profitability-reconciliation/' in finance,'Finance must link Build 23 reconciliation');req('/admin/project-profitability-reconciliation/' in creator,'Creator completeness must link Build 23 reconciliation');req([x.get('file') for x in mig.get('migrations',[])]==MIGRATIONS,'canonical migrations drifted')
