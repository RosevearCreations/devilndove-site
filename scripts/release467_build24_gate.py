#!/usr/bin/env python3
"""Fail-closed contract for Release 467 Build 24 — Storefront ↔ Inventory Sellability Reconciliation."""
...
state=p.get('state'); req(state in ('DEVELOPMENT_CANDIDATE','DEVELOPMENT_GREEN'),'Build 24 pointer state invalid')
...
if state=='DEVELOPMENT_CANDIDATE':
    ...
    req(p.get('last_green_build')==24 and p.get('accepted_dev_sha')==ACCEPTED_SHA and p.get('accepted_dev_tree_sha')==ACCEPTED_TREE,'Build 24 accepted runtime pointer drifted')
    req(p.get('last_green_dev_sha')==ACCEPTED_SHA and p.get('last_green_dev_tree_sha')==ACCEPTED_TREE,'Build 24 latest green SHA/tree drifted')
    req(p.get('last_green_system_gate_run')==ACCEPTED_SYSTEM and p.get('last_green_build_proof_run')==ACCEPTED_PROOF and p.get('last_green_branch_hygiene_run')==ACCEPTED_HYGIENE,'Build 24 latest green run evidence drifted')
...
if state=='DEVELOPMENT_GREEN':
    req(m.get('state')=='DEVELOPMENT_GREEN' and m.get('accepted_dev_sha')==ACCEPTED_SHA and m.get('accepted_dev_tree_sha')==ACCEPTED_TREE,'Build 24 manifest accepted runtime drifted')
    a=m.get('acceptance') or {}; req(a.get('merged_system_gate_run')==ACCEPTED_SYSTEM and a.get('merged_build24_proof_run')==ACCEPTED_PROOF and a.get('merged_branch_hygiene_run')==ACCEPTED_HYGIENE,'Build 24 manifest accepted runs drifted')
else: req(m.get('state')=='DEVELOPMENT_CANDIDATE','Build 24 candidate manifest state drifted')
...
print('PASS Release 467 Build 24 Storefront Inventory Sellability Reconciliation gate'); print('accepted_runtime='+ACCEPTED_SHA if state=='DEVELOPMENT_GREEN' else 'candidate_base='+BASE_SHA); print('storefront_inventory_sellability=READ_ONLY'); print('sellability_supported_authorization=FALSE'); print('automatic_unpublish=FALSE'); print('public_offer_rule_changed=FALSE'); print('schema_d1_r2_provider_main_production_mutation=NONE')
