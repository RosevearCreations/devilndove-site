...
pointer=load('current-development-authority.json'); b102=load('release467-build102-product-work-manual-reorder.json'); b103=load('release467-build103-product-work-session-paging.json'); manifest=load('migrations/canonical/manifest.json')
...
req(pointer.get('accepted_dev_sha')==B102_SHA and pointer.get('accepted_dev_tree_sha')==B102_TREE,'accepted Build 102 SHA/tree drifted')
...
req(last.get('build')==102 and last.get('dev_sha')==B102_SHA and last.get('tree_sha')==B102_TREE and (last.get('proofs') or {})==B102_PROOFS,'Build 102 restart closure drifted')
req(prod.get('build')==102 and prod.get('main_sha')==B102_SHA and prod.get('tree_sha')==B102_TREE and prod.get('production_pages_deploy_run')==B102_PAGES and prod.get('production_live_resource_integrity_run')==B102_LIVE,'Build 102 Production baseline drifted')
req(cand.get('build')==103 and cand.get('authority')=='release467-build103-product-work-session-paging.json' and cand.get('state')=='AWAITING_EXTERNAL_EXACT_CLOSURE_HEAD_PROOF','Build 103 candidate pointer drifted')
...
req(f102.get('dev_sha')==B102_SHA and f102.get('tree_sha')==B102_TREE and (f102.get('proofs') or {})==B102_PROOFS and f102.get('ingested_by_build')==103,'Build 102 final closure not ingested by Build 103')
req(p102.get('main_sha')==B102_SHA and p102.get('tree_sha')==B102_TREE and p102.get('production_pages_deploy_run')==B102_PAGES and p102.get('production_live_resource_integrity_run')==B102_LIVE,'Build 102 Production closure drifted')
...
for text,label in ((it_api,'I.T. API'),(reliability,'Reliability'),(preflight,'Deployment Preflight')):
    req(B102_SHA in text and B102_TREE in text,f'{label} missing Build 102 SHA/tree')
...
for path in ('AI_HANDOFF.md','PROJECT_STATUS_AND_ROADMAP.md','SANITY_HEALTH_CHECK.md','MARKDOWN_INDEX.md','docs/operations/IT_PREFLIGHT_STARTUP_RELEASE_GUIDE.md','docs/operations/RELEASE_467_BUILD_103_PRODUCT_WORK_SESSION_PAGING.md'):
    body=read(path)
    for token in (B102_SHA,B102_TREE,*[str(x) for x in B102_PROOFS.values()],str(B102_PAGES),str(B102_LIVE)):req(token in body,f'{path} missing Build 102 closure token {token}')
...
print('Build 102 final Development + Production closure: INGESTED')
