...
q(p.get('build')==231 and p.get('title')=='Workshop Journal & Capability Case-Study Activation','current pointer must remain last verified Build 231 while Build 232 candidate is tested')
q(p.get('state')=='DEVELOPMENT_GREEN' and p.get('source_authority')=='dev','current pointer must remain verified Development GREEN')
