...
cc=compact(current); compatc=compact(compat)

pointer_release=int(pointer.get('release') or 0); pointer_build=int(pointer.get('build') or 0)
release_match=re.search(r'const RELEASE\s*=\s*(\d+)',current);build_match=re.search(r'const BUILD\s*=\s*(\d+)',current)
active_release=int(release_match.group(1)) if release_match else 0;active_build=int(build_match.group(1)) if build_match else 0
...
expected_prefix=['0001_release464_migration_authority.sql','0002_release464_operational_acceptance.sql','0003_release464_business_growth.sql','0004_release465_storefront_quality.sql','0005_release467_inventory_process_assignment.sql','0006_release467_product_media_publication_guard.sql','0007_release467_storefront_launch_remediation.sql','0008_release467_workshop_process_taxonomy.sql','0009_release467_workshop_capability_profiles.sql','0010_release467_custom_work_intake_2.sql','0011_release467_manufacturing_triage_route.sql','0012_release467_hybrid_creative_project_operations.sql','0013_release467_digital_proof_customer_approval.sql','0014_release467_prototype_sample_production_run.sql','0015_release467_small_batch_corporate_event_quoting.sql','0016_release467_customer_supplied_item_suitability_review.sql','0017_release467_production_cost_evidence_v2.sql','0018_release467_manufacturing_work_order_job_traveler.sql','0019_release467_production_run_qa_rework_scrap_evidence.sql','0020_release467_workshop_knowledge_library_foundation.sql','0021_release467_project_knowledge_recipe_history.sql','0022_release467_capability_profile_coverage_closure.sql']
req(canonical_files[:22]==expected_prefix,'current Deployment Preflight lost the canonical 0001-0022 prefix')
if active_build>=234:
    req(len(canonical_files)>=23 and canonical_files[22]=='0023_release467_cupcake_soap_label_templates.sql','Build 234 Deployment Preflight expects canonical migration 0023')
for filename in canonical_files: req(filename in current,f'current Deployment Preflight missing canonical migration: {filename}')

for stale in ('database_upgrade_current_pass.sql','database_build171_ledger_repair.sql','database_build173_deployment_preflight.sql','database_build174_deployment_preflight_detail.sql','database_build175_release_control.sql','database_build176_release_safety_controls.sql','build_171_admin_safety_release_readiness','build_173_deployment_preflight_release_safety','build_174_preflight_detail_manifest','build_175_release_control_center','build_176_release_safety_controls'):
...
req('exportasyncfunctiononRequestGet' in cc,'current Deployment Preflight must expose GET')
for method in ('onRequestPost','onRequestPut','onRequestPatch','onRequestDelete'): req(method not in current,f'current Deployment Preflight must remain GET-only ({method} found)')
req("mutation_capability:'none'" in cc,'current Deployment Preflight must explicitly expose no mutation capability')
req('request_time_schema_mutation:false' in cc,'request-time schema mutation boundary missing')
req("manifest_path:'migrations/canonical/manifest.json'" in cc,'canonical manifest authority missing from current endpoint')
req("applicator:'scripts/d1_migrate.py'" in cc,'canonical migration applicator authority missing')
req("native_ledger:'d1_migrations'" in cc,'native d1_migrations authority missing')
req("proof_table:'app_schema_migration_proofs'" in cc,'canonical migration proof-table authority missing')
req('PRAGMA foreign_key_check' in current,'foreign-key integrity check missing')
for proof in ('System Gate','Current Application Quality Proof','I.T. Admin Runtime Proof','Repository Branch Hygiene'): req(proof in current,f'four-proof Development promotion requirement missing: {proof}')
for key in ('main_sha','tree_sha','production_pages_deploy_run'):
    value=str(prod.get(key) or '');req(value and value in current,f'current Deployment Preflight missing current Production {key}')
req("historical_feature_authority:'release467-build37-deployment-preflight-canonical-migration.json'" in cc,'Build 37 Deployment Preflight feature provenance must remain explicit')

req('getCurrentDeploymentPreflight' in compat and "from'./current-deployment-preflight.js'" in compatc,'compatibility GET route must delegate to current Deployment Preflight')
req('exportasyncfunctiononRequestPost' in compatc and '405' in compat and "Allow:'GET'" in compatc,'historical Deployment Preflight POST must fail closed with 405')
req("mutation_capability:'none'" in compatc,'compatibility route must expose no mutation capability')
...
req(f'Release {active_release} Build {active_build}' in page,'Deployment Preflight page identity must match the active current projection')
...
req("method:'POST'" not in compact(client),'current client must not POST')
for stale_label in ('Build 175','Build 176','build-174'): req(stale_label not in client,f'current client contains stale build label: {stale_label}')
...
print(f'Active Deployment Preflight: RELEASE {active_release} BUILD {active_build} READ-ONLY')
