...
for token in ('/api/admin/product-browser','/admin/product-editor/?product_id=','/media/product?key=','Automatic retries are stopped'):
...
for token in ('/api/admin/product-editor-detail?product_id=','/api/admin/product-editor-media?product_id=','/media/product?key=','No catalog refresh or readiness scan'):
...
for token in ('MAX_LIMIT=50','FROM products','ORDER BY product_id DESC','LIMIT ?','X-DD-D1-Read-Contract'):
...
for token in ('WHERE p.product_id=?','LEFT JOIN product_seo','LIMIT 1','lazy_sections'):
...
for token in ('FROM product_images','WHERE product_id=?','LIMIT 12','r2_listing:false'):
...
print('Products: BOUNDED BROWSER / NO EAGER EDITOR SUBSYSTEMS')
print('Editor: ONE PRODUCT STARTUP READ / MEDIA ON DEMAND / NO AUTOSAVE LOOP')
print('Media: SAME-ORIGIN R2 TRANSPORT / ZERO D1 AUTHORITY READS')
