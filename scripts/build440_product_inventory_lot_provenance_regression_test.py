...
Local-only. Executes the additive migration against an in-memory SQLite fixture and
source-checks production/reversal contracts. No Cloudflare, D1, R2, or provider access.
"""
...
CREATE TABLE orders(
  order_id INTEGER PRIMARY KEY AUTOINCREMENT,order_status TEXT DEFAULT 'pending',created_at TEXT DEFAULT CURRENT_TIMESTAMP
);
...
# Re-run to prove additive/idempotent behavior and stable cutover.
...
check(cutover_before == cutover_after and bool(cutover_before), 'forward-provenance cutover is stable across migration re-run')

raw = conn.execute("SELECT lot_code,quantity_remaining FROM inventory_purchase_lots WHERE site_item_inventory_id=1 ORDER BY inventory_purchase_lot_id").fetchall()
check(len(raw) == 2 and abs(sum(float(r[1]) for r in raw) - 10.0) < 1e-6, 'raw purchase-lot bootstrap preserves known lot and adds only missing opening balance')
...
# Commitment trigger: first pending order reserves 3 of 5; second request for 3 must fail.
...
commitment = conn.execute("SELECT COALESCE(committed_quantity,0) FROM product_inventory_active_commitments WHERE product_id=1").fetchone()
check(commitment is None or abs(float(commitment[0])) < 1e-6, 'cancelled order automatically releases forward commitment attribution')

# Add a new production lot after the legacy opening balance, then prove FIFO attribution reaches it only after legacy is consumed.
...
check(attribution and abs(float(attribution[0]) - 1.0) < 1e-6 and abs(float(attribution[1]) - 1.0) < 1e-6, 'FIFO attribution consumes legacy opening stock first and identifies downstream commitment on new production lot')

# Reactivating a cancelled oversized order must be blocked.
...
check(reactivation_blocked, 'order reactivation guard prevents cancelled/refunded order from reclaiming unavailable finished stock')
...
check('product_finished_lot_commitment_attribution' in migration and 'product_inventory_active_commitments' in migration, 'migration exposes bounded forward commitment attribution views')
check('trg_order_items_build440_inventory_commit_guard_insert' in migration and 'trg_orders_build440_inventory_commit_guard_reactivate' in migration, 'migration owns checkout and status-reactivation commitment guards')
...
print('Downstream commitment attribution: FIFO / POST-CUTOVER ONLY')
