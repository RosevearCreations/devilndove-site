...
if not successor_active:
    ...
    q(int(p.get('build') or 0)>=261 and p.get('state')=='DEVELOPMENT_GREEN','Verified Build 261+ successor authority must preserve Build 260 provenance')
    q(bool(re.fullmatch(r'[0-9a-f]{40}',str(p.get('accepted_dev_sha') or ''))),'Verified successor must preserve an exact accepted Development SHA')
    prod_now=p.get('production_checkpoint') or {}
    q(prod_now.get('state')=='PRODUCTION_GREEN' and bool(re.fullmatch(r'[0-9a-f]{40}',str(prod_now.get('main_sha') or ''))),'Verified successor must preserve an exact Production baseline')
