...
public_help=read('help/index.html');public_help_lower=public_help.lower();req(len(re.findall(r'<h1(?:\s|>)',public_help,re.I))==1,'public help must contain exactly one H1');req(all(topic in public_help_lower for topic in ('shopping','product images','wishlist','creators','contextual help')),'public help is missing user/creator topics')
