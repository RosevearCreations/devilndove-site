#!/usr/bin/env python3
"""Canonical Release 455 current-release and forward-sanity authority."""
...
req(release.get('release')==455,'current Development release must be 455')
...
req(previous.get('release')==454 and str(previous.get('state','')).startswith('complete_'),'Release 454 must be the completed previous release')
...
req(d1.get('schema_current_through_release')==453,'Release 455 must carry D1 schema through 453, not invent a Release 455 migration')
req(release.get('current_release_migrations')==[],'Release 455 current migration list must be empty')
...
req(history.get(452,{}).get('state')=='complete_source_proven_no_new_d1_migration','Release 452 completed history missing')
req(history.get(454,{}).get('state')=='complete_source_proven_no_new_d1_migration','Release 454 completed history missing')
req(history.get(455,{}).get('state')=='complete_source_proven_no_new_d1_migration','Release 455 history row missing')
policy=release.get('release_policy',{})
req(policy.get('current_release_d1_migration_required') is False,'Release 455 must be source-only')
...
for p in (
 'public/js/storefront-discovery-runtime.js','css/storefront-discovery.css','assets/visual-placeholders/storefront-media-fallback.svg',
 'scripts/release455_storefront_discovery_gate.py','docs/operations/RELEASE_455_STOREFRONT_DISCOVERY.md',
 'scripts/release454_admin_convergence_gate.py','docs/operations/RELEASE_454_ADMIN_CONVERGENCE.md',
 'AI_HANDOFF.md','PROJECT_STATUS_AND_ROADMAP.md','MARKDOWN_INDEX.md','SANITY_HEALTH_CHECK.md',
 'docs/operations/DEVELOPMENT_CLOUDFLARE_CONNECTION_AUTHORITY.md','.github/workflows/release455-source-gate.yml'
):
 req((ROOT/p).exists(),f'current/carried-forward authority missing: {p}')
for p in ('AI_HANDOFF.md','PROJECT_STATUS_AND_ROADMAP.md','MARKDOWN_INDEX.md','SANITY_HEALTH_CHECK.md','docs/operations/DEVELOPMENT_CLOUDFLARE_CONNECTION_AUTHORITY.md','docs/operations/RELEASE_455_STOREFRONT_DISCOVERY.md'):
 t=read(p);req('Release 455' in t,f'{p} must identify Release 455');req('Release 453' in t and '33258377328' in t and '33258415391' in t,f'{p} must preserve Release 453 D1 evidence')
...
req('CURRENT_RELEASE = 455' in authority and 'Storefront Discovery, Media Fallback & SEO Depth' in authority,'shared runtime release authority drifted')
...
req('node --check public/js/storefront-discovery-runtime.js' in workflow,'System Gate must syntax-check Release 455 runtime')
...
for p in list(ROOT.glob('*.html'))+list((ROOT/'admin').rglob('*.html'))+list((ROOT/'js').rglob('*.js'))+list((ROOT/'public/js').rglob('*.js'))+list((ROOT/'css').rglob('*.css')):
 for m in version_pattern.finditer(p.read_text(encoding='utf-8',errors='replace')):
  if int(m.group(2))>455:future.append(f'{p.relative_to(ROOT)}:{m.group(2)}')
req(not future,f'future cache majors found: {future[:12]}')
print('PLATFORM FORWARD SANITY')
print('Current release: 455 — Storefront Discovery, Media Fallback & SEO Depth')
print('Development Pages target: devilndove-site-dev / devilndove-site-dev.pages.dev')
print('Release 455 D1 migration: NONE')
