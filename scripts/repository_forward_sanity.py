...
def read(path):
    p = ROOT / path
    if not p.is_file():
        FAIL.append(f'missing current authority: {path}')
        return ''
...
req("'current-development-authority.json'" in system_gate, 'System Gate must trigger on current-development-authority.json')
req("'release467-*.json'" in system_gate, 'System Gate must trigger on current Release 467 authority JSON files')
...
print('Active workflows/navigation/help/provenance: RELEASE-NEUTRAL')
