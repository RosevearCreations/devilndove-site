...
check('AUTHORIZED_EXPECTED_KEYS = 498' in restore, 'restore authorization is fixed to the proven 498-key parity gap')
check('APPLY_TOKEN = "BUILD440_DEV_R2_RESTORE"' in restore, 'apply requires an explicit Development restore authorization token')
check('supplies_images_inventory.csv' in restore and 'r2_object_key' in restore and 'file_size_bytes' in restore and 'sha256' in restore, 'restore uses recorded canonical key, byte-size and SHA metadata')
...
check('missing_from_manifest = sorted(d1_keys - manifest_keys)' in restore and 'stale_manifest = sorted(manifest_keys - d1_keys)' in restore, 'restore requires exact current D1/source-manifest key equality')
check('verify_all_sources(assets' in restore and 'validate_file(temp, asset)' in restore, 'all public sources are size/SHA verified before any R2 write phase')
check('mode.add_argument("--dry-run"' in restore and 'Development R2 mutation executed: NO' in restore, 'dry-run is explicit and reports zero Development R2 mutation')
...
print('Authorized scope: 498 CURRENT DEVELOPMENT TOOL/SUPPLY IMAGE KEYS')
