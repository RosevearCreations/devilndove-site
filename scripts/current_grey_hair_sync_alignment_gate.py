...
api = API.read_text(encoding='utf-8'); page = PAGE.read_text(encoding='utf-8'); client = CLIENT.read_text(encoding='utf-8'); b45_page = B45_PAGE.read_text(encoding='utf-8'); schema = LEGACY_SCHEMA.read_text(encoding='utf-8')
combined=f'{api}\n{page}\n{client}'
def req(condition,message):
    if not condition: raise SystemExit(f'BUILD 46 GREY HAIR SYNC ALIGNMENT GATE: FAIL — {message}')
...
for name in ('caip_capture_groups','caip_capture_tracks'):
    req(name in api,f'existing {name} authority is not reused'); req(f'CREATE TABLE IF NOT EXISTS {name}' in schema,f'historical persistence definition missing for {name}')
...
req(canonical_names[:4]==baseline,f'Build 46 canonical baseline changed: {canonical_names}')
for forbidden in ('create table','alter table','drop table','create index','drop index'): req(forbidden not in api.lower(),f'Build 46 endpoint contains request-time DDL token: {forbidden}')
