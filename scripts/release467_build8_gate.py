...
req(pointer.get("release") == 467 and pointer.get("build") == 8, "current authority pointer must identify Release 467 Build 8")
...
for path, tokens in required_docs.items():
    body = read(path)
    for token in tokens:
        req(token in body, f"{path} missing current authority token: {token}")
...
print(f"predecessor_build7_proof={BUILD7_PROOF}")
