...
assert.equal(items.length,37,'exactly 37 gate definitions are required');
assert.equal(new Set(items.map((row)=>row.key)).size,37,'gate keys must be unique');
assert.match(api,/const BUILD = '227'/);assert.match(api,/const GATE_FIX_FOCUS/);assert.match(api,/correction_guidance/);assert.match(api,/evidence_guidance/);assert.match(api,/retest_guidance/);
...
for(const response of [new Response('<!doctype html><title>Fallback page</title>',{status:200,headers:{'Content-Type':'text/html'}}),Response.json({ok:true,build:'227',expected_total:37,items:[]})]){
  const nodes=await renderWithResponse(response);assert.equal((nodes.startupReadinessMount.innerHTML.match(/data-key=/g)||[]).length,37);assert.match(nodes.startupReadinessMount.innerHTML,/If any step fails: how to correct it/);assert.match(nodes.startupReadinessMount.innerHTML,/Evidence to save/);assert.match(nodes.startupReadinessMount.innerHTML,/Retest and reopen rule/);assert.doesNotMatch(nodes.startupReadinessMount.innerHTML,/No readiness items match these filters/);
}
