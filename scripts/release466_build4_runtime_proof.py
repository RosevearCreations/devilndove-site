...
REQUIRED_TABLE_FLAGS=(
 ...
 'creative_asset_access_grants_table','creative_asset_access_audit_table','oauth_provider_connections_table','oauth_security_events_table'
)
...
def main():
 ...
 stripe_total=n(row,'stripe_required_checks');stripe_passed=n(row,'stripe_passed_checks')
 paypal_total=n(row,'paypal_required_checks');paypal_passed=n(row,'paypal_passed_checks')
 ...
 stripe_ok=stripe_total==5 and stripe_passed==5
 paypal_ok=paypal_total==5 and paypal_passed==5
 ...
 evidence={
  ...
  'acceptance_observations':{
   'stripe':{'required_checks':stripe_total,'passed_checks':stripe_passed,'accepted_by_ledger':stripe_ok},
   'paypal':{'required_checks':paypal_total,'passed_checks':paypal_passed,'accepted_by_ledger':paypal_ok},
  ...
  'launch_state':state,
  'interpretation':'PASS means Build 4 can truthfully read the existing Development acceptance authorities. HOLD is the required result while real browser/provider evidence is incomplete; configuration alone is never promoted to acceptance.',
  ...
  'safety':{
   ...
   'automatic_payment_execution':False,'automatic_oauth_action':False,'raw_r2_delete':False,'secrets_emitted':False,
  }
 }
