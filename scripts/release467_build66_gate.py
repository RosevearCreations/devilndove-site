#!/usr/bin/env python3
"""Retained source contract for Release 467 Build 66 — Product Workspace Split.

Later builds may advance the visible/versioned Product workspace bundle, but they must not
remove Build 66's one-Product/six-workspace behavior or turn it into a data authority.
"""
...
for token in ("one Product authority","Products","Editor","Inventory Links","Media","SEO / Publishing","Cleanup / Archive","workspace=","schema-neutral","Build 67 — Product Editor Recovery & Autosave"):
...
for token in ("admin-product-workspaces.js?v=","product-workspace-split","element.closest?.('[hidden], [inert]')","style.display === 'none'","style.visibility === 'hidden'"):
    req(token in admin, f"Admin Product-workspace integration missing retained Build 66 token: {token}")

for token in ("data-product-workspace-tab","dataset.productWorkspacePanel","role=\"tablist\"","setAttribute('role', 'tab')","setAttribute('role', 'tabpanel')","aria-selected","workspace","window.history.pushState","window.history.replaceState","window.DDProductWorkspaces","dd:product-workspace-changed","dd:product-editor-target","#productResourcesAdminMount","#siteInventoryAdminMount","#productStockReportMount","#productMediaAdminMount","#adminProductImageAnnotationsMount","#mediaLibraryAdminMount","#productStoryNotesAdminMount","#productSeoAdminMount","#catalogSyncAdminMount","#productCleanupCenter","productCorrectionMount","[data-edit-product-id]","[data-open-product-correction]"):
    req(token in workspace, f"Product workspace retained authority missing token: {token}")
...
for forbidden in ("/api/","apifetch(","fetch(",".prepare(",".exec(","stripe.com","paypal.com"):
    req(forbidden not in workspace_lower, f"Product workspace script gained forbidden data/provider authority token: {forbidden}")
...
print("Historical visible asset revision: MAY ADVANCE / BEHAVIOR RETAINED")
