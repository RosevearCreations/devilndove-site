#!/usr/bin/env python3
"""Build 436 guarded Membership Build 395 Production rebuild controller.

Prepared only. Source creation does not authorize execution.

The controller requires:
- exact Membership-specific authorization token;
- fresh Build 436 read-only dependency/mapping preflight;
- a fresh full Production D1 export;
- byte/SHA/UUID/age backup verification;
- exact post-backup source-row fingerprint preservation;
- one D1 execute-file batch containing canonical shadow creation, exact mapped
  copy, SQL assertions, legacy drop, and shadow rename;
- independent canonical row/schema/value postchecks.

Cloudflare D1 remote SQL-file execution is transaction-managed. Do not add an
explicit BEGIN/COMMIT wrapper here; D1 import guidance warns that doing so can
cause a nested-transaction error.
...
def fresh_preflight() -> dict:
    ...
    if result.returncode != 0 or not PREFLIGHT_ARTIFACT.exists():
        fail('fresh Build 436 Membership rebuild authorization preflight is not green.')
    ...
    if payload.get('safe_to_request_membership_rebuild_authorization') is not True:
        fail('fresh Build 436 Membership rebuild preflight is not safe.')
    return payload
...
def exact_prewrite_match(expected: dict, current: dict) -> bool:
    return all([
        ...
        current.get('no_user_defined_indexes_or_triggers') is True,
...
def backup(confirm: str | None) -> None:
    ...
    payload = {
        'artifact': 'Build 436 Membership Build 395 backup/fingerprint boundary',
    ...
    print('BUILD 436 MEMBERSHIP BACKUP/FINGERPRINT BOUNDARY: PASS')
...
def apply(confirm: str | None) -> None:
    ...
    payload = {
        'artifact': 'Build 436 Production Membership Build 395 rebuild postcheck',
        'stage': 'membership',
        'scope': 'membership_build395_lossless_shadow_rebuild',
        'pass': passed,
    ...
    print('BUILD 436 PRODUCTION MEMBERSHIP BUILD 395 REBUILD POSTCHECK:', 'PASS' if passed else 'FAIL')
...
def postcheck() -> None:
    ...
    print('BUILD 436 PRODUCTION MEMBERSHIP BUILD 395 READ-ONLY POSTCHECK:', 'PASS' if passed else 'FAIL')
