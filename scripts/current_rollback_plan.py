#!/usr/bin/env python3
"""Build a release-neutral, fail-closed Production rollback readiness plan without mutation."""
...
def load_json_at(sha: str, path: str) -> dict:
    return json.loads(git_text("show", f"{sha}:{path}"))
...
def main() -> int:
    ...
    if run("git", "merge-base", "--is-ancestor", target, current, check=False).returncode != 0:
        raise SystemExit("rollback target must be an ancestor of current Production source")

    target_release = load_json_at(target, "development-release.json")
    current_release = load_json_at(current, "development-release.json")
    current_authority = load_json_at(current, "current-development-authority.json")
    ...
    if current_authority.get("state") != "DEVELOPMENT_GREEN":
        raise SystemExit("current canonical authority is not Development GREEN")

    plan = {
        "kind": "current-production-rollback-readiness-plan",
        ...
        "current_authority_release": int(current_authority.get("release") or 0),
        "current_authority_build": int(current_authority.get("build") or 0),
    ...
    print("CURRENT PRODUCTION ROLLBACK READINESS: PASS")
