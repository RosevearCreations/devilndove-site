#!/usr/bin/env python3
"""Release 465 Build 2 historical acceptance, append-safe for Build 3."""
...
has(helper,'loadMaterialLotPlan','loadProductAvailabilityIntelligence','loadRelatedProductIntelligence','loadGenealogyExceptions','loadCreativeReadinessIntelligence','chooseNextSafeAction','actual_planned_quantity_claimed:false','historical_reconstruction_claimed:false','automatic_relationship_write:false','inventory_mutation_active:false','accounting_posting_active:false',label='Build 2 helper');has(helper+'\n'+lot,'product_resource_links','site_item_inventory','inventory_purchase_lots','inventory_lot_policies','product_production_runs','product_finished_inventory_lots','creative_projects','creative_media_evidence_ranges','content_projects','creative_business_pipelines',label='Build 2 reused authority');req('onRequestPost' not in endpoint and 'onRequestPut' not in endpoint and 'onRequestDelete' not in endpoint,'Build 2 API must remain GET-only');req(page.lower().count('<h1')==1,'Build 2 cockpit must retain one H1');has(client,'/api/admin/inventory-creator-intelligence','renderAvailability','renderRelated','renderGenealogy','renderCreative','renderNext',label='Build 2 client')
release=json.loads(read('development-release.json') or '{}');req(release.get('convergence_state') in ('release465_build2_complete_development_green','release465_build3_source_candidate','release465_complete_development_green'),'later Release 465 state must remain append-safe');req(int((release.get('current_release_database_state') or {}).get('development_native_migration_rows') or 0)==4 and int((release.get('current_release_database_state') or {}).get('development_migration_proof_rows') or 0)==4,'Build 2 must preserve 4/4 migration proof')
print('RELEASE 465 BUILD 2 — INVENTORY & CREATOR INTELLIGENCE — APPEND SAFE')
if FAIL:
