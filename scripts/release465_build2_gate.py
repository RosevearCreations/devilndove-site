...
release=json.loads(read('development-release.json') or '{}');req(int(release.get('release') or 0)>=465,'current release must not regress below 465');req(all(x.get('status')=='complete_development_green' for x in release.get('release465_build2',[])),'current authority must preserve Release 465 Build 2 completion');req(int((release.get('current_release_database_state') or {}).get('development_native_migration_rows') or 0)>=4 and int((release.get('current_release_database_state') or {}).get('development_migration_proof_rows') or 0)>=4,'Build 2 must preserve its historical 4/4 migration proof')
