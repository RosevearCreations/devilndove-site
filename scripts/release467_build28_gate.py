...
req(p.get('accepted_dev_sha')==PREV_ACCEPTED_SHA and p.get('accepted_dev_tree_sha')==PREV_ACCEPTED_TREE,'current authority must retain accepted Build 27 runtime evidence')
...
if m.get('state')=='DEVELOPMENT_GREEN':
    req(bool(m.get('accepted_dev_sha')) and bool(m.get('accepted_dev_tree_sha')),'green Build 28 manifest must freeze accepted SHA/tree')
    ma=m.get('acceptance') or {}
    req(all(ma.get(k) for k in ('merged_system_gate_run','merged_build28_proof_run','merged_branch_hygiene_run')),'green Build 28 manifest must freeze accepted run evidence')
