...
req(int(final.get('canonical_migrations') or 0)==21 and int(final.get('foreign_key_violations') or -1)==0,'Build 222 Development migration closure drifted')
...
req(prod.get('business_counts')=={'users':2,'products':45,'site_item_inventory':1041,'orders':0} and int(prod.get('canonical_migrations') or 0)==21 and int(prod.get('foreign_key_violations') or -1)==0,'Build 222 Production D1/business preservation drifted')
