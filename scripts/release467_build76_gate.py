...
story_html = read("workshop-journal/story/index.html")
story_js = read("public/js/workshop-journal-story.js")
...
for token in ("all checked-in public HTML routes", "exactly one source H1", "canonical", "sitemap", "Open Graph", "JSON-LD", "internal-link", "Build 77 — Canada-Only Commerce Rules"):
...
for token in (
    ...
    '"og:site_name"', '"og:type"', '"og:title"', '"og:description"', '"og:url"', '"og:image"', '"twitter:card"',
    "exactly one source H1 required", "indexable route must explicitly declare index,follow", "legacy alias must canonicalize exactly", "canonical must be exactly",
    "indexable route missing valid JSON-LD", "sitemap.xml missing indexable routes", "sitemap.xml includes noindex routes", "sitemap.xml includes legacy canonical aliases",
    "indexable sitemap routes without an inbound link", "robots.txt missing canonical sitemap declaration",
):
    req(token in crawl, f"Build 76 crawler missing enforcement token: {token}")

for forbidden in ("wrangler d1 execute", "INSERT INTO", "UPDATE products", "DELETE FROM", "setInterval(", "fetch(", "stripe.com", "paypal.com"):
    req(forbidden not in crawl, f"Build 76 crawler gained forbidden runtime/mutation behavior: {forbidden}")

req('content="noindex,follow" name="robots"' in story_html, "generic Workshop Journal story template must be source noindex,follow")
for token in (
    "const PRODUCTION_ORIGIN = 'https://devilndove.com'",
    "safeCanonicalPath",
    "setMetaForPublishedStory",
    "setMeta('meta[name=\"robots\"]', 'index,follow')",
    "setMeta('meta[property=\"og:url\"]', canonical)",
    "document.querySelector('link[rel=\"canonical\"]')?.setAttribute('href', canonical)",
):
    req(token in story_js, f"dynamic Workshop Journal story SEO missing token: {token}")
req("`${location.origin}" not in story_js, "dynamic Workshop Journal story must not derive canonical URLs from Preview origin")
...
print("Canonical / aliases / sitemap parity: ENFORCED")
