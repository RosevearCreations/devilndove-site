...
e=a.get('development_evidence') or {};req(int(e.get('native_migration_rows') or 0)==4 and int(e.get('proof_rows') or 0)==4 and int(e.get('foreign_key_violations',-1))==0,'Build 1 historical migration/FK evidence drifted');req(int(e.get('release465_publication_triggers') or 0)==4,'Build 1 publication-trigger evidence drifted');req(e.get('preview_smoke_mode')=='CLOUDFLARE_ACCESS_PROTECTED' and int(e.get('preview_smoke_auth_headers_used') or 0)==0 and e.get('preview_access_weakened') is False,'Build 1 Access-safe evidence drifted')
manifest=json.loads(read('migrations/canonical/manifest.json') or '{}');files=[x.get('file') for x in manifest.get('migrations',[]) if isinstance(x,dict)];baseline=['0001_release464_migration_authority.sql','0002_release464_operational_acceptance.sql','0003_release464_business_growth.sql','0004_release465_storefront_quality.sql'];req(files[:4]==baseline,'Build 1 canonical migration baseline 0001-0004 drifted');req(len(files)>=4,'Build 1 canonical migration baseline is incomplete')
