...
RUNTIME_TREE='550272841e764d77fc21297abede3d4cae1aaea0'; PROD_MAIN='055cbc973c667b35a209c7ea207779089f6fed3a'; PROD_DEPLOY=33688892602; RETIRED=list(range(1,21)); EXTRA='.github/workflows/release467-build5-promotion-proof.yml'
def req(ok,msg):
...
rg=p.get('repository_governance') or {}; req(rg.get('persistent_branches')==['main','dev'],'persistent branch policy drifted'); req(rg.get('retired_automatic_proof_builds')==RETIRED,'retired historical proof range drifted'); req(rg.get('retired_extra_automatic_proof_workflows')==[EXTRA],'extra historical workflow retirement drifted')
...
wh=m.get('workflow_hygiene') or {}; req(wh.get('retained_proofs_dispatch_only_builds')==RETIRED,'Build 21 manifest historical workflow range drifted'); req(wh.get('retained_extra_dispatch_only_workflows')==[EXTRA],'Build 5 promotion workflow retirement drifted')
...
ch=changed(); allowed={f'.github/workflows/release467-build{x}-proof.yml' for x in range(1,16)} | {EXTRA,'current-development-authority.json','release467-build21-release-state-branch-ci-hygiene.json','scripts/release467_build21_gate.py','AI_HANDOFF.md','PROJECT_STATUS_AND_ROADMAP.md','SANITY_HEALTH_CHECK.md','MARKDOWN_INDEX.md','docs/operations/RELEASE_467_BUILD_21_RELEASE_STATE_BRANCH_CI_HYGIENE.md'}
