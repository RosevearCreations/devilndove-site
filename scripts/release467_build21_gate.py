...
if pointer==21:
    ...
    req(pred.get('build')==21 and pred.get('merged_dev_sha')==B21_SHA and pred.get('merged_dev_tree_sha')==B21_TREE,'Build 22 must retain exact Build 21 predecessor')
    ...
    if p.get('state')=='DEVELOPMENT_CANDIDATE':
        req(p.get('last_green_build')==21 and p.get('last_green_dev_sha')==B21_SHA and p.get('last_green_dev_tree_sha')==B21_TREE,'Build 22 candidate must retain Build 21 last-green predecessor')
    else:
        req(p.get('state')=='DEVELOPMENT_GREEN' and p.get('last_green_build')==22,'Build 22 closure must be DEVELOPMENT_GREEN')
