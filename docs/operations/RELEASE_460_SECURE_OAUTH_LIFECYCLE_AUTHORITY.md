...
- Diagnostics expose configuration/readiness booleans and safe diagnostic codes only.
...
- `GET /api/admin/oauth-connections` is redacted diagnostics.
...
The migration is additive, checks foreign keys, and includes proof that forbidden plaintext OAuth columns do not exist.
...
- `.github/workflows/release460-source-gate.yml` runs both proofs plus the carried Release 459 gate.
- `.github/workflows/development-d1-release460.yml` may mutate only exact `devilndove-dev` and applies only the Release 460 migration after re-running the safety proofs.

## Manual boundary
Do not authorize Etsy/Pinterest/Meta/X/TikTok/YouTube yet. Do not set `OAUTH_PROVIDER_AUTHORIZATION_MODE=development-explicit` yet. After Release 460 source, System Gate, Development D1, and read-only verification are green, real provider authorization remains a separate deliberate acceptance activity.
