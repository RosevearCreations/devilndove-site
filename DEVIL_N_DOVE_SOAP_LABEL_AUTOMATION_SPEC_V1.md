...
**Version:** 1.1 (Build 222 integration)  
**Project:** Devil n Dove  
**Subsystem:** Creative Asset Intelligence Platform (CAIP)  
**Status:** Active implementation foundation; production prepress and physical/regulatory approval remain gated  
...
## 19. Build 222 Integration Appendix

**Integrated status:** Active implementation foundation  
**Implementation build:** 222  
**Admin routes:** `/admin/packaging-studio/` and `/admin/packaging/soap-labels/`  
**Migration:** `database_build222_soap_label_startup_readiness.sql`

### 19.1 Implemented mapping

The specification is implemented as a first-class CAIP packaging workflow with:

- a continuous SVG ribbon in the required section order;
- a photo-matched Glacial Purple starting profile;
- English and French structured ingredient rows;
- bilingual structured claim/icon rows;
- reusable rose assets and product-specific palettes;
- exact SVG document dimensions;
- fold, safe-area, bleed and overlap guides;
- immutable-style review versions;
- export filenames and SHA-256 evidence;
- browser-local recovery when D1 is unavailable;
- a required physical 100%-scale print-test record before approval.

The approved image is stored as a reference at:

```text
/assets/packaging/soap/reference/glacial-purple-aloe-soap-approved-reference.png
```

It is not embedded as the final production label. Text, colours, roses and fields are rendered from structured records.

### 19.2 Normalized data model

Build 222 adds the proposed soap-label records:
...
These extend the broader Packaging Studio tables:
...
Ingredients, claims, print tests and export evidence are relational because they require ordered row editing, validation and independent audit. Small visual settings may remain inside versioned JSON subdocuments.

### 19.3 Dimension conflict and controlled profiles

The original specification simultaneously requires:

- an overall artboard height of **1.50 inches / 38.1 mm**; and
- a rear circle of **50 mm**.

A 50 mm circle cannot fit inside a 38.1 mm-high artboard without clipping. Build 222 therefore exposes two deliberate profiles:

1. **Photo-fit profile** — 11 × 1.5-inch artboard, 0.75-inch centred band, 2 × 1.5-inch front oval and 38.1 mm rendered rear seal.
2. **50 mm seal profile** — 11-inch-wide × 50 mm-high artboard, 0.75-inch band, 2 × 1.5-inch front oval and true 50 mm rear seal.

Neither profile is automatically declared production-correct. Print both at 100%, measure them and choose the profile that fits the physical soap and approved visual design.

### 19.4 Current export boundary

Implemented:

- exact-source SVG;
- PNG, WebP and JPG previews;
- browser Print/Save PDF preparation;
- predictable filenames;
- version and checksum evidence.

Not yet represented as complete:

- server-generated prepress PDF;
- embedded or outlined fonts;
- verified CMYK conversion and printer profile;
- crop and calibration marks;
- automated source-versus-raster visual-difference proof.

Browser Print/Save PDF must not be described as a final CMYK production file.

### 19.5 Approval requirements

The application blocks label approval until:

- the required English and French ingredient panels remain within the conservative eight-line narrow-band capacity estimate;
- a review version is saved;
- required structured content passes the application preflight;
- a label is printed at 100% scale;
- physical measurements are recorded;
- wrap fit, legibility and overlap are reviewed;
- the print-test status is recorded as passed.

The current line-capacity check is deliberately conservative and does not replace exact SVG glyph measurement or a physical proof. Application approval remains separate from formula, bilingual, cosmetic, consumer-packaging, net-quantity and claim compliance review.

### 19.6 Startup dependency

A soap is not ready for public sale until Gate 13 and the applicable regulatory gate in `STARTUP_GO_LIVE_GUIDE.md` are complete. The label editor existing or producing an SVG is not sufficient evidence by itself.
