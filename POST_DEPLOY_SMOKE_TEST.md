# Post-Deploy Smoke Test — Build 233 current entry

Use the complete stage 4 procedure in `PRELAUNCH_PROCESS_PLAYBOOKS.md`. On production, confirm domain/HTTPS/canonical routes; exactly one H1 and no mobile/desktop overlap; public catalog/product/gallery; structured API errors/runtime incidents; Creative Automation fallback; Packaging source checks; read-only Meta tests; and all 43 Startup gates with All statuses. First run the fourteen-step Build 233 login gate: the normal login diagnostic is binding-only, a successful POST returns `auth_login_bounded_v1`, Cloudflare reports no resource-limit outcome, a wrong password returns structured 401, a blocked `/api/auth/me` retains the existing session until verification recovers, and logout/reset/logout-all/expiry work explicitly. Retest Build 231 product load/autosave recovery. Then run the Build 232 archived-product proof: one unused archived record must return `bounded_registry_v1` and reach reviewed removal, while a separate order/packaging/project-history record must remain Archive-only; check inventory and audits exactly once and review Cloudflare invocation outcomes. Open `missing_launch_images`, load all 20 D1 manifest rows, test one saved/history update, and force a safe API failure to prove the read-only Unsynced fallback shows 20 rows with saving disabled. Check changed interfaces at phone/desktop widths and confirm generated art is disclosed and absent from Product/Offer schema. Record deployment ID, route, device/browser, expected/actual result and non-secret evidence. Any failure pauses promotion and requires the full suite after correction or rollback.
