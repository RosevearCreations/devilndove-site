// Release 467 Build 114 — Business Health Action Queue & Owner Routing client.
(()=>{'use strict';
...
function renderQueue(q={}){
  ...
  const mount=id('b114ActionQueue');
  if(!mount)return;
  if(!list.length){
    mount.innerHTML='<p>No Business Health actions are currently queued. This READY state is review-only and does not authorize automatic business mutation.</p>';
    return;
  }
...
function render(d){const p=d.profitability||{},a=d.financial_anomalies||{},m=d.month_end_readiness||{},h=d.it_health||{},q=d.business_action_queue||{};id('b3Metrics').innerHTML=`<div class="card"><div class="small">Action queue</div><strong>${Number(q.summary?.action_count||0)}</strong><div class="small">${esc(String(q.state||'ready').toUpperCase())}</div></div><div class="card"><div class="small">Month-end readiness</div><strong>${Number(m.score||0)}/100</strong></div><div class="card"><div class="small">I.T. health</div><strong>${Number(h.score||0)}/100</strong></div><div class="card"><div class="small">Financial anomalies</div><strong>${Number(a.summary?.total||0)}</strong><div class="small">high ${Number(a.summary?.high||0)}</div></div><div class="card"><div class="small">Profitability projects</div><strong>${Number(p.summary?.projects||0)}</strong><div class="small">high risk ${Number(p.summary?.high||0)}</div></div>`;renderQueue(q);renderProfitability(p);renderAnomalies(a);renderMonth(m);renderIt(h);}
async function load(){const period=id('b3Period')?.value||new Date().toISOString().slice(0,7);status('Loading Release 467 Build 114 Business Health Action Queue…');try{const d=await read(await api(`/api/admin/business-health-action-queue?period_month=${encodeURIComponent(period)}`));render(d);status(`Build 114 Business Health loaded for ${d.period_month||period}. Routing only; no business mutation was executed.`);}catch(e){status(e.message,true);}}
...
window.Release467Build114BusinessHealthActionQueue={renderQueue,renderProfitability,renderAnomalies,renderMonth,renderIt};
})();
