...
// do-not-reuse flags, and movement history with visual item references.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function render() {
    ...
    mountEl.innerHTML = `
      <div class="card" style="margin-top:18px">
        ...
        <div class="grid cols-2" style="gap:12px;margin-top:16px"><div><label class="small" for="siteInventorySearch">Search</label><input id="siteInventorySearch" type="text" placeholder="name, category, supplier" /></div><div style="align-self:end"><button class="btn" type="button" id="siteInventoryRefreshButton">Refresh List</button></div></div>
        <div class="admin-table-wrap" style="margin-top:12px"><table><thead><tr><th>Item</th><th>Stock</th><th>Rules</th><th>Supplier</th><th>Cost</th><th>Actions</th></tr></thead><tbody id="siteInventoryList"><tr><td colspan="6" style="padding:8px">Loading inventory...</td></tr></tbody></table></div>
  ...
  function readForm() {
    return {
      source_type: document.getElementById('siteInventorySourceType')?.value || 'tool', external_key: document.getElementById('siteInventoryExternalKey')?.value || '',
      item_name: document.getElementById('siteInventoryItemName')?.value || '', category: document.getElementById('siteInventoryCategory')?.value || '',
      image_url: document.getElementById('siteInventoryImageUrl')?.value || '', on_hand_quantity: Number(document.getElementById('siteInventoryOnHand')?.value || 0),
      reserved_quantity: Number(document.getElementById('siteInventoryReservedInput')?.value || 0), incoming_quantity: Number(document.getElementById('siteInventoryIncomingInput')?.value || 0),
      reorder_level: Number(document.getElementById('siteInventoryReorder')?.value || 0), preferred_reorder_quantity: Number(document.getElementById('siteInventoryPreferredReorderQty')?.value || 0),
      unit_cost_cents: Number(document.getElementById('siteInventoryUnitCost')?.value || 0), supplier_name: document.getElementById('siteInventorySupplierName')?.value || '',
      supplier_sku: document.getElementById('siteInventorySupplierSku')?.value || '', amazon_url: document.getElementById('siteInventoryAmazonUrl')?.value || '',
      source_url: document.getElementById('siteInventorySourceUrl')?.value || '', reorder_notes: document.getElementById('siteInventoryNotes')?.value || '',
      movement_note: document.getElementById('siteInventoryMovementNote')?.value || '', is_active: Number(document.getElementById('siteInventoryIsActive')?.value || 1),
      is_on_reorder_list: document.getElementById('siteInventoryOnReorderList')?.checked ? 1 : 0, do_not_reorder: document.getElementById('siteInventoryDoNotReorder')?.checked ? 1 : 0,
      do_not_reuse: document.getElementById('siteInventoryDoNotReuse')?.checked ? 1 : 0, reuse_status: document.getElementById('siteInventoryReuseStatus')?.value || ''
    };
  }
  function renderMovements(movements) {
    const body = document.getElementById('siteInventoryMovementList'); if (!body) return;
    if (!Array.isArray(movements) || !movements.length) { body.innerHTML = '<tr><td colspan="5" style="padding:8px">No movement history recorded yet.</td></tr>'; return; }
  ...
  async function saveItem(event) { event.preventDefault(); try { setMessage('Saving inventory item...'); const response = await window.DDAuth.apiFetch('/api/admin/site-item-inventory', { method: 'POST', body: JSON.stringify(readForm()) }); const data = await response.json(); if (!response.ok || !data?.ok) throw new Error(data?.error || 'Failed to save inventory item.'); setMessage('Inventory item saved.'); document.getElementById('siteInventoryForm')?.reset(); await loadList(); } catch (err) { setMessage(err.message || 'Failed to save inventory item.', true); } }
  async function loadList() {
    try {
      ...
      const data = await response.json(); if (!response.ok || !data?.ok) throw new Error(data?.error || 'Failed to load inventory list.');
      const summary = data.summary || {}; setValue('siteInventoryTotalItems', summary.total_items || 0); setValue('siteInventoryActiveItems', summary.active_items || 0); setValue('siteInventoryLowStock', summary.low_stock_items || 0); setValue('siteInventoryReserved', summary.total_reserved || 0); setValue('siteInventoryIncoming', summary.total_incoming || 0); setValue('siteInventoryReorderListCount', summary.reorder_list_items || 0);
      const items = Array.isArray(data.items) ? data.items : []; const body = document.getElementById('siteInventoryList'); if (!body) return;
      if (!items.length) body.innerHTML = '<tr><td colspan="6" style="padding:8px">No site inventory items saved yet.</td></tr>';
      else body.innerHTML = items.map((x) => `<tr><td style="padding:8px;border-bottom:1px solid #ddd">${x.image_url ? `<img src="${escapeHtml(x.image_url)}" alt="${escapeHtml(x.item_name)}" style="width:52px;height:52px;object-fit:cover;border-radius:10px;display:block;margin-bottom:8px"/>` : ''}${x.needs_reorder ? '⚠️ ' : ''}<strong>${escapeHtml(x.item_name)}</strong><div class="small">${escapeHtml(x.source_type)} • ${escapeHtml(x.category || '—')}</div></td><td style="padding:8px;border-bottom:1px solid #ddd">On hand ${x.on_hand_quantity}<div class="small">Reserved ${x.reserved_quantity} • Incoming ${x.incoming_quantity} • Reorder ${x.reorder_level}</div><div class="small">Preferred reorder ${x.preferred_reorder_quantity || 0}</div></td><td style="padding:8px;border-bottom:1px solid #ddd"><div class="small">${x.is_on_reorder_list ? 'On reorder list' : 'Not queued'}</div><div class="small">${x.do_not_reorder ? 'Do not reorder' : 'Can reorder'}</div><div class="small">${x.do_not_reuse ? 'Do not reuse' : (x.reuse_status || 'Reusable/normal')}</div></td><td style="padding:8px;border-bottom:1px solid #ddd">${escapeHtml(x.supplier_name || '—')}<div class="small">${escapeHtml(x.supplier_sku || '')}</div></td><td style="padding:8px;border-bottom:1px solid #ddd">${fmtMoney(x.unit_cost_cents || 0)}</td><td style="padding:8px;border-bottom:1px solid #ddd"><button class="btn" type="button" data-edit-id="${x.site_item_inventory_id}" data-item='${escapeHtml(JSON.stringify(x))}'>Quick Update</button> <button class="btn" type="button" data-delete-id="${x.site_item_inventory_id}">Delete</button></td></tr>`).join('');
      renderMovements(data.movements || []); setMessage('');
    } catch (err) { setMessage(err.message || 'Failed to load inventory list.', true); }
  }
  async function onTableClick(event) {
    const editBtn = event.target.closest('[data-edit-id]'); const deleteBtn = event.target.closest('[data-delete-id]');
    if (editBtn) {
      let item = null; try { item = JSON.parse(editBtn.getAttribute('data-item') || '{}'); } catch {}
      const id = Number(item?.site_item_inventory_id || editBtn.getAttribute('data-edit-id') || 0); if (!id) return;
      const onHand = Number(window.prompt('New on-hand quantity?', String(item?.on_hand_quantity ?? 0))); if (Number.isNaN(onHand)) return;
      ...
      try { setMessage('Updating inventory item...'); const response = await window.DDAuth.apiFetch('/api/admin/site-item-inventory', { method: 'PATCH', body: JSON.stringify({ site_item_inventory_id: id, item_name: item?.item_name || '', on_hand_quantity: onHand, is_on_reorder_list: reorderList ? 1 : 0, do_not_reuse: doNotReuse ? 1 : 0, movement_note: movementNote || 'Inventory quantity updated.' }) }); const data = await response.json(); if (!response.ok || !data?.ok) throw new Error(data?.error || 'Failed to update inventory item.'); await loadList(); } catch (error) { setMessage(error.message || 'Failed to update inventory item.', true); }
      return;
    }
    if (deleteBtn) {
      const id = Number(deleteBtn.getAttribute('data-delete-id') || 0); if (!id || !window.confirm('Delete this inventory item?')) return;
      try { setMessage('Deleting inventory item...'); const response = await window.DDAuth.apiFetch(`/api/admin/site-item-inventory?site_item_inventory_id=${encodeURIComponent(id)}`, { method: 'DELETE' }); const data = await response.json(); if (!response.ok || !data?.ok) throw new Error(data?.error || 'Failed to delete inventory item.'); await loadList(); } catch (error) { setMessage(error.message || 'Failed to delete inventory item.', true); }
    }
  }
