...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function productRows(data) {
    return arr(data,'products').slice(0,80).map((row) => `<tr><td><a href="/admin/products/?focus_product=${Number(row.product_id || 0)}">${esc(row.product_name)}</a><div class="small">${esc(row.product_slug || '')}</div></td><td>${pill(row.readiness_status)}</td><td>${row.missing_real_photo ? pill('missing_real_photo') : pill('photo_present')}</td><td>${Number(row.missing_alt_text || 0)}</td><td>${row.low_stock ? pill(`low_stock_${row.inventory_quantity}`) : esc(row.inventory_quantity)}</td><td>${esc(row.product_family_key || '')}<div class="small">${row.cost_configured ? 'cost configured' : 'cost needs setup'}</div></td><td>${esc(row.channel_key || '')}<div class="small">${row.fee_configured ? 'fees configured' : 'fees need setup'}</div></td><td>${money(row.price_cents)}</td><td>${money(row.estimated_cost_cents)}</td><td>${money(row.estimated_margin_cents)}<div class="small">${Number(row.estimated_margin_percent || 0).toFixed(1)}%</div></td><td>${pill(row.margin_status)}</td><td>${pill(row.marketplace_export_status)}</td></tr>`).join('');
  }
  ...
  function render(data) {
    ...
    mount.innerHTML = `
      ...
      <section class="card"><h2>Product readiness, real-photo, stock, and margin warnings</h2><p class="small">Marketplace fees are estimates. Review actual channel fees before publishing/exporting.</p>${table(['Product','Readiness','Real photo','Missing alt','Stock','Family','Channel','Price','Est. cost','Est. margin','Margin status','Export'],productRows(data),'No product rows yet.')}</section>
