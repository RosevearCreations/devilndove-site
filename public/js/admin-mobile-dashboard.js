document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderTodayTasks(payload) {
    ...
    todayTaskStats.querySelectorAll('[data-today-task-action]').forEach((button) => button.addEventListener('click', async () => {
      ...
      if (action === 'snoozed') payload.snooze_until = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
      try { const res = await window.DDAuth.apiFetch('/api/admin/today-task-actions', { method: 'POST', body: JSON.stringify(payload) }); const data = await res.json().catch(() => null); if (!res.ok || !data?.ok) throw new Error(data?.error || 'Task action failed.'); button.closest('.mobile-summary-list-item')?.remove(); } catch (error) { setMessage(error.message || 'Task action failed.', 'error'); }
    }));
  }
