(() => {
  ...
  function renderGovernance(r){
    const g=r.governance||{},rec=r.recovery||{},drift=r.drift||{};
  ...
  async function load(){
    ...
    try{
      const response=await api('/api/admin/release466-reliability');
      const data=await response.json();
      if(!response.ok||!data.ok)throw new Error(data.error||`HTTP ${response.status}`);
      const r=data.reliability||{};
      msg.textContent=`Release 466 Build 1 reliability snapshot loaded for ${r.environment||'unknown'}; mutation capability: none.`;
      ...
      renderChecks(r.checks);renderGovernance(r);renderSlo(r);
    }catch(error){
      msg.textContent=`Reliability snapshot unavailable: ${error?.message||error}`;
    }
  }
