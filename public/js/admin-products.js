...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function renderRows(products) {
    ...
    tableBody.innerHTML = products.map(product => {
      ...
      const contextImageCount = Number(product.context_image_count || 0);
      const recordImageCount = Number(product.record_image_count || 0);
      const duplicateAngleGroupCount = Number(product.duplicate_angle_group_count || 0);
      const duplicateImageCount = Number(product.duplicate_image_count || 0);
