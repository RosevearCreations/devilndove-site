// Release 467 Build 104 — browser-local Product Work Session Focus Views.
// Read-only planner over already-rendered Product/readiness state; no Product authority mutation.
(() => {
  ...
  const FOCUS_MODES = ['all','active','blocked','ready','done'];
  ...
  ready(() => {
    ...
    function addProduct(id) { id=Number(id)||0; if(!id||sessionItem(id)||session.items.length>=MAX_ITEMS)return false; session.items.push({product_id:id,added_at:new Date().toISOString(),completed_at:null,priority:'normal'}); saveSession(); return true; }
    ...
    function setFocusMode(mode) { if(!FOCUS_MODES.includes(mode))return; sessionFocus=mode; sessionPage=0; }
    function moveItem(id,delta) {
      if(session.sort_mode!=='manual'||sessionFocus!=='all')return -1;
      const index=session.items.findIndex((item)=>item.product_id===Number(id)),target=index+Number(delta||0);
      if(index<0||target<0||target>=session.items.length||target===index)return -1;
      const [item]=session.items.splice(index,1); session.items.splice(target,0,item); saveSession(); return target;
    }
    ...
    function matchesFocus(item,mode=sessionFocus) {
      if(mode==='all')return true;
      if(mode==='done')return Boolean(item.completed_at);
      if(item.completed_at)return false;
      if(mode==='active')return true;
      const readiness=readinessForRow(rowForProduct(item.product_id));
      if(mode==='blocked')return readiness.blocked;
      if(mode==='ready')return readiness.ready;
      return true;
    }
    const focusedItems = (items=orderedItems()) => items.filter((item)=>matchesFocus(item));
    ...
    function focusCounts(items=orderedItems()) { return Object.fromEntries(FOCUS_MODES.map((mode)=>[mode,items.filter((item)=>matchesFocus(item,mode)).length])); }
    ...
    function ensureStyle() {
      ...
      style.textContent='.dd-work-session-card{margin:14px 0;padding:16px;border:1px solid var(--border);border-radius:14px;background:rgba(255,255,255,.03)}.dd-work-session-head,.dd-work-session-actions,.dd-work-session-pager{display:flex;gap:10px;align-items:center;justify-content:space-between;flex-wrap:wrap}.dd-work-session-actions{justify-content:flex-start;margin-top:10px}.dd-work-session-pager{margin-top:10px;padding:8px 0;border-top:1px solid var(--border);border-bottom:1px solid var(--border)}.dd-work-session-progress,.dd-work-session-page-summary{font-weight:800}.dd-work-session-list{display:grid;gap:8px;margin-top:12px}.dd-work-session-item{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:center;padding:10px 12px;border:1px solid var(--border);border-radius:12px}.dd-work-session-item.is-done{opacity:.65}.dd-work-session-item-actions{display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end}.dd-work-session-toggle{margin-left:6px}.dd-work-session-locate{outline:3px solid currentColor;outline-offset:-3px}.dd-work-session-priority,.dd-work-session-sort,.dd-work-session-focus{min-height:36px;border-radius:8px;padding:4px 8px;background:var(--panel);color:inherit;border:1px solid var(--border)}.dd-work-session-reorder{display:inline-flex;gap:4px}.dd-work-session-reorder .btn[disabled],.dd-work-session-pager .btn[disabled]{opacity:.45}@media(max-width:720px){.dd-work-session-item{grid-template-columns:1fr}.dd-work-session-item-actions{justify-content:flex-start}.dd-work-session-actions .btn,.dd-work-session-pager .btn{flex:1 1 140px}}'; document.head.appendChild(style);
    }
    ...
    function render() {
      readSnapshot(); ensureStyle(); const panel=ensurePanel(),ordered=orderedItems(),focused=focusedItems(ordered),counts=focusCounts(ordered),total=session.items.length,complete=session.items.filter((item)=>item.completed_at).length,active=total-complete,next=nextIncomplete(),blocked=nextBlocked(),manual=session.sort_mode==='manual',manualEnabled=manual&&sessionFocus==='all';
      clampPage(focused.length); const pageCount=pageCountFor(focused.length),pageStart=sessionPage*PAGE_SIZE,pageEnd=Math.min(focused.length,pageStart+PAGE_SIZE),pageItems=focused.slice(pageStart,pageEnd);
      const rows=pageItems.map((item)=>{const row=rowForProduct(item.product_id),meta=rowMeta(row,item.product_id),readiness=readinessForRow(row),state=readiness.known?`${readiness.ready?'Ready':readiness.blocked?'Blocked':'Readiness'}${Number.isFinite(readiness.score)?` ${readiness.score}%`:''}${readiness.blocker?` · ${readiness.blocker}`:''}`:'Readiness unavailable',storedIndex=session.items.findIndex((entry)=>entry.product_id===item.product_id),moveUpDisabled=!manualEnabled||storedIndex<=0,moveDownDisabled=!manualEnabled||storedIndex<0||storedIndex>=session.items.length-1;return `<article class="dd-work-session-item ${item.completed_at?'is-done':''}"><div><strong>DD${esc(meta.number)} — ${esc(meta.name)}</strong><div class="small">${esc(state)} · ${esc(item.priority)} priority${item.completed_at?' · completed in this browser':''}${manual?` · manual position ${storedIndex+1} of ${session.items.length}`:''}</div></div><div class="dd-work-session-item-actions"><span class="dd-work-session-reorder" aria-label="Manual order controls"><button class="btn" type="button" data-work-session-action="move-up" data-product-id="${item.product_id}" ${moveUpDisabled?'disabled':''} aria-label="Move ${esc(meta.name)} up in manual order">↑ Up</button><button class="btn" type="button" data-work-session-action="move-down" data-product-id="${item.product_id}" ${moveDownDisabled?'disabled':''} aria-label="Move ${esc(meta.name)} down in manual order">↓ Down</button></span><label class="small">Priority <select class="dd-work-session-priority" data-work-session-priority="${item.product_id}">${PRIORITIES.map((p)=>`<option value="${p}" ${p===item.priority?'selected':''}>${p[0].toUpperCase()+p.slice(1)}</option>`).join('')}</select></label><button class="btn" type="button" data-work-session-action="locate" data-product-id="${item.product_id}">Locate</button><button class="btn" type="button" data-work-session-action="blocker" data-product-id="${item.product_id}" ${readiness.blocked?'':'disabled'}>Open blocker</button><button class="btn" type="button" data-work-session-action="done" data-product-id="${item.product_id}">${item.completed_at?'Undo done':'Mark done'}</button><button class="btn" type="button" data-work-session-action="remove" data-product-id="${item.product_id}">Remove</button></div></article>`;}).join('');
      const pageSummary=focused.length?`Page ${sessionPage+1} of ${pageCount} · Focused Products ${pageStart+1}-${pageEnd} of ${focused.length} · Session ${total}`:`${sessionFocus[0].toUpperCase()+sessionFocus.slice(1)} focus has no Products · Session ${total}`;
      const pager=focused.length?`<nav class="dd-work-session-pager" aria-label="Product work session pages"><button class="btn" type="button" data-work-session-command="page-prev" ${sessionPage<=0?'disabled':''} aria-label="Previous Product work session page">← Previous 20</button><span class="dd-work-session-page-summary small" aria-live="polite">${esc(pageSummary)}</span><button class="btn" type="button" data-work-session-command="page-next" ${sessionPage>=pageCount-1?'disabled':''} aria-label="Next Product work session page">Next 20 →</button></nav>`:`<div class="dd-work-session-pager"><span class="dd-work-session-page-summary small" aria-live="polite">${esc(pageSummary)}</span></div>`;
      const focusOptions=FOCUS_MODES.map((mode)=>`<option value="${mode}" ${mode===sessionFocus?'selected':''}>${mode[0].toUpperCase()+mode.slice(1)} (${counts[mode]||0})</option>`).join('');
      const emptyCopy=total?`No Products match the ${sessionFocus} focus. Choose another focus or update Product completion/readiness state.`:'No Products are pinned yet. Use Add to work on a Product row or add the currently visible Products.';
      panel.innerHTML=`<div class="dd-work-session-head"><div><p class="eyebrow">Build 104 · browser-local planning</p><h3 style="margin:0">Product work session</h3><p class="small">Focus the existing ${MAX_ITEMS}-Product session on All, Active, Blocked, Ready or Done items without changing Product records. Focused results keep Build 103 paging; next actions still scan the complete ordered session.</p></div><div class="dd-work-session-progress">${complete}/${total} done · ${active} active</div></div><div class="dd-work-session-actions"><label class="small">Session order <select class="dd-work-session-sort" data-work-session-sort>${SORT_MODES.map((mode)=>`<option value="${mode}" ${mode===session.sort_mode?'selected':''}>${mode[0].toUpperCase()+mode.slice(1)}</option>`).join('')}</select></label><label class="small">Session focus <select class="dd-work-session-focus" data-work-session-focus>${focusOptions}</select></label><button class="btn" type="button" data-work-session-command="add-visible">Add visible Products</button><button class="btn" type="button" data-work-session-command="next" ${next?'':'disabled'}>Locate next Product</button><button class="btn" type="button" data-work-session-command="next-blocker" ${blocked?'':'disabled'}>Open next blocker</button><button class="btn" type="button" data-work-session-command="clear-completed" ${complete?'':'disabled'}>Clear completed</button><button class="btn" type="button" data-work-session-command="clear" ${total?'':'disabled'}>Clear session</button></div>${manual&&!manualEnabled?'<p class="small">Manual Move Up/Down is available when Session focus is All, so hidden focused items cannot make the stored sequence ambiguous.</p>':''}${pager}<div id="catalogWorkSessionMessage" class="small" role="status" aria-live="polite"></div><div class="dd-work-session-list">${rows||`<p class="small">${esc(emptyCopy)}</p>`}</div>`; syncRowButtons();
    }
    ...
    document.addEventListener('change',(event)=>{const priority=event.target.closest('[data-work-session-priority]');if(priority){setPriority(Number(priority.dataset.workSessionPriority||0),String(priority.value||''));render();return;}const sort=event.target.closest('[data-work-session-sort]');if(sort){setSortMode(String(sort.value||''));render();setMessage(sort.value==='manual'?'Manual order is active. Choose All focus to use Move Up/Down.':'Session order updated and paging returned to page 1.','green');return;}const focus=event.target.closest('[data-work-session-focus]');if(focus){setFocusMode(String(focus.value||''));render();setMessage(`Session focus changed to ${sessionFocus}. Focused paging returned to page 1; next actions still scan the full session.`,'green');}});
    document.addEventListener('click',(event)=>{const command=event.target.closest('[data-work-session-command]');if(command){const action=command.dataset.workSessionCommand;if(action==='add-visible')addVisibleProducts();if(action==='next'){const item=nextIncomplete();if(item)locateProduct(item.product_id);}if(action==='next-blocker'){const item=nextBlocked();if(item)openBlocker(item.product_id);}if(action==='page-prev'){sessionPage=Math.max(0,sessionPage-1);render();setMessage(`Showing focused work-session page ${sessionPage+1}.`,'green');}if(action==='page-next'){sessionPage=Math.min(pageCountFor(focusedItems().length)-1,sessionPage+1);render();setMessage(`Showing focused work-session page ${sessionPage+1}.`,'green');}if(action==='clear-completed'){session.items=session.items.filter((item)=>!item.completed_at);saveSession();render();}if(action==='clear'){session={sort_mode:session.sort_mode,items:[]};sessionPage=0;sessionFocus='all';saveSession();render();}return;}const button=event.target.closest('[data-work-session-action]');if(!button)return;const id=Number(button.dataset.productId||0)||0,action=button.dataset.workSessionAction;if(action==='locate')locateProduct(id);if(action==='blocker')openBlocker(id);if(action==='done'){const item=sessionItem(id);if(item)setDone(id,!item.completed_at);render();}if(action==='remove'){removeProduct(id);render();}if(action==='move-up'){const target=moveItem(id,-1);if(target>=0)sessionPage=Math.floor(target/PAGE_SIZE);render();setMessage(target>=0?'Moved Product up in the browser-local manual order and kept it visible.':'Move Up requires Manual order with All focus. ',target>=0?'green':'review');}if(action==='move-down'){const target=moveItem(id,1);if(target>=0)sessionPage=Math.floor(target/PAGE_SIZE);render();setMessage(target>=0?'Moved Product down in the browser-local manual order and kept it visible.':'Move Down requires Manual order with All focus. ',target>=0?'green':'review');}});
