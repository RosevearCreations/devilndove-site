// Release 467 Build 102 — browser-local Product Work Manual Reorder & Accessibility.
...
(() => {
  ...
  ready(() => {
    ...
    function moveItem(id, delta) {
      if (session.sort_mode !== 'manual') return false;
      const index = session.items.findIndex((item)=>item.product_id===Number(id));
      const target = index + Number(delta||0);
      if (index < 0 || target < 0 || target >= session.items.length || target === index) return false;
