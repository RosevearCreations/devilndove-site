// Release 466 Build 5 — four-module admin navigation renderer.
// Navigation grouping only: canonical runtime module authority remains Storefront, Creators, Socials, Financials and I.T.
  ...
  const load = async () => {
    try {
      ...
      cardCounts.forEach((card) => {
        ...
        if (!module) return;
        const count = (module.sections || []).reduce((total, section) => total + (Array.isArray(section?.links) ? section.links.length : 0), 0);
      ...
      const toolCount = sections.reduce((total, section) => total + (Array.isArray(section?.links) ? section.links.length : 0), 0);
      const intro = document.getElementById('adminModuleHubIntro');
      if (intro) intro.textContent = `${module.summary || ''} ${toolCount} grouped tool${toolCount === 1 ? '' : 's'}.`;

      hubMount.innerHTML = sections.map((section) => {
        const links = Array.isArray(section?.links) ? section.links : [];
        return `<section class="card" style="margin-top:18px">
          <h2 style="margin-top:0">${escapeHtml(section?.label || 'Tools')}</h2>
          <div class="admin-compact-tool-grid">
            ${links.map((link) => `<a href="${escapeHtml(link?.href || '/admin/')}"><strong>${escapeHtml(link?.label || link?.href || 'Open')}</strong><small>Open ${escapeHtml(module.label)} workspace</small></a>`).join('')}
          </div>
        </section>`;
      }).join('');
    } catch (error) {
