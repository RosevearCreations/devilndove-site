// Build 247 - Truth-reference soap layout, full botanical rose palette, persistent packaging libraries, reusable templates, and explicit project deletion.
(() => {
  const STORAGE_KEY = 'dd_packaging_studio_local_draft_v4';
  const state = { projects: [], templates: [], products: [], inventory: [], referenceSources: [], formulaLibrary: [], contentLibrary: [], librarySchemaReady: true, detail: null, loading: false, activeTab: 'product' };
  ...
  function detailMarkup() {
    ...
    return `<section class="card packaging-editor-card"><div class="section-heading-row"><div><p class="eyebrow">${esc(project.project_key || 'New packaging system')}</p><h2 style="margin:0">${esc(project.project_name || project.product_name || 'Labeling and packaging project')}</h2></div><div class="packaging-project-actions"><button class="btn" id="duplicatePackagingProject" type="button">Duplicate</button><button class="btn" id="archivePackagingProject" type="button">Archive</button><button class="btn danger" id="deletePackagingProject" type="button">Delete label</button></div></div>
      ...
      ${tabPanel('library',`<h3>Reusable soap, ingredient, fragrance, colourant and claims library</h3>
        ${state.librarySchemaReady?'':`<div class="packaging-warning-list"><strong>Build 247 database migration required</strong><p class="small">The editor remains usable, but persistent library controls require <code>database_build247_packaging_library_truth_layout_rose_palette.sql</code>.</p></div>`}
        <section class="packaging-library-section"><div class="section-heading-row"><div><h4>Soap type / formula library</h4><p class="small">Apply a saved product formula to populate the structured ingredients and its default rose direction. The built-in Health Oatmeal & Goat Milk entry preserves the supplied ingredient breakdown and the “*Organic” source note without guessing which individual ingredient is organic.</p></div></div>
          <div class="grid cols-3"><label><span class="small">Saved formula</span><select class="input" id="packagingFormulaLibraryId">${formulaLibraryOptions()}</select></label><label><span class="small">New/custom formula name</span><input class="input" id="packagingFormulaLibraryName" placeholder="Example: Honey Goat Milk Soap"/></label><label><span class="small">Formula notes</span><input class="input" id="packagingFormulaLibraryNotes" placeholder="Supplier/source, organic marker, review notes…"/></label></div>
          <div class="packaging-save-actions"><button class="btn primary" id="applyPackagingFormulaLibrary" type="button">Apply selected formula</button><button class="btn" id="savePackagingFormulaLibrary" type="button">Save current ingredients as formula</button><button class="btn danger" id="deletePackagingFormulaLibrary" type="button">Delete selected custom formula</button></div>
        </section>
        ...
        <section class="packaging-library-section"><h4>Ingredients, fragrance oils and safe colourants</h4><p class="small">Store supplier-verified INCI/display wording here, then add it to a label with one click. “Safe colourant” is a library category only; suitability still depends on the supplier documentation and the finished product.</p><div class="packaging-library-grid">${state.contentLibrary.filter((row)=>['ingredient','fragrance_oil','colourant'].includes(row.content_type)).map(libraryContentCard).join('')||'<p class="small">No reusable ingredient/additive entries are available yet.</p>'}</div>
          <div class="packaging-library-entry-form"><div class="grid cols-3"><label><span class="small">Library type</span><select class="input" id="packagingLibraryContentType"><option value="ingredient">Ingredient</option><option value="fragrance_oil">Fragrance oil</option><option value="colourant">Safe colourant</option><option value="claim">Claim</option></select></label><label><span class="small">Library name</span><input class="input" id="packagingLibraryItemName" placeholder="Supplier/product or claim name"/></label><label><span class="small">INCI (ingredient/additive)</span><input class="input" id="packagingLibraryInci" placeholder="Supplier-verified INCI"/></label></div><div class="grid cols-3"><label><span class="small">English / claim text</span><input class="input" id="packagingLibraryTextEn"/></label><label><span class="small">French / claim text</span><input class="input" id="packagingLibraryTextFr"/></label><label><span class="small">Icon</span><select class="input" id="packagingLibraryIcon">${['leaf','hands','recycle','heart','none'].map((name)=>`<option>${name}</option>`).join('')}</select></label></div><label><span class="small">Supplier / allergen / usage notes</span><input class="input" id="packagingLibraryNotes"/></label><button class="btn" id="savePackagingLibraryContent" type="button">Save library item</button></div>
        </section>`)}
  ...
  async function deleteProject() {
    ...
    try{message('Deleting the selected label/project…');const data=await api({action:'delete_project',packaging_project_id:projectId,confirm_project_key:key});state.projects=data.projects||[];state.templates=data.templates||state.templates;state.formulaLibrary=data.formula_library||state.formulaLibrary;state.contentLibrary=data.content_library||state.contentLibrary;state.detail=null;state.activeTab='product';try{localStorage.removeItem(STORAGE_KEY);}catch{}renderProjects();renderMain();message(data.message,'success');}
    catch(error){message(error.message||'The packaging project could not be deleted.','error');}
  }

  function refreshLibraryState(data) {
    state.projects=data.projects||state.projects; state.templates=data.templates||state.templates; state.formulaLibrary=data.formula_library||[]; state.contentLibrary=data.content_library||[]; state.librarySchemaReady=data.library_schema_ready!==false; if(data.detail!==undefined)state.detail=data.detail;
  }
  ...
  function applyFormulaLibrary() {
    ...
    bindDynamicRows(); renderPreview(); message(`Formula “${formula.formula_name}” applied to this draft. Review it, then save the project.`,'success');
  }
