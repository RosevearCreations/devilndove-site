// Build 255 - Material Library hub, independent source Master INCI editor, packaging templates, and retained label controls.
(() => {
  const STORAGE_KEY = 'dd_packaging_studio_local_draft_v5';
  const state = { projects: [], templates: [], products: [], inventory: [], referenceSources: [], formulaLibrary: [], contentLibrary: [], sourceMaterialLibrary: [], librarySchemaReady: true, sourceMaterialSchemaReady: true, sourceMaterialMetadataReady: true, detail: null, loading: false, activeTab: 'product' };
  ...
  function sourceMaterialManagerMarkup({attachedIds=new Set(),sourceMaterials=[],allowAttach=true,standalone=false}={}) {
    ...
    return `<section class="packaging-library-section packaging-material-library-hub"><div class="section-heading-row packaging-library-heading"><div><p class="eyebrow">Material Library</p><h4>${standalone?'Manage purchased materials before creating a label':'Purchased/source material templates'}</h4><p class="small">This is where we enter soap bases, candle waxes, fragrance and essential-oil blends, colours/micas/dyes, carrier oils/butters and other additives. Each purchased product keeps its own supplier ingredient/INCI list, claims, benefits and allergen evidence.</p></div><span class="status-pill">${rows.length} template${rows.length===1?'':'s'}</span></div>
      ${!state.sourceMaterialMetadataReady?`<div class="packaging-warning-list"><strong>Build 255 database migration required</strong><p class="small">The new product-family/source-category metadata needs <code>database_build255_packaging_material_library_hub.sql</code>. Existing Build 248 templates remain readable.</p></div>`:''}
      ${allowAttach?`<div class="grid cols-2 packaging-source-picker"><label><span class="small">Source material to attach/use</span><select class="input" id="packagingSourceMaterialTemplateId">${sourceMaterialOptions(sourceMaterials.find((row)=>String(row.material_role)==='base')?.packaging_source_material_template_id||'')}</select></label><label><span class="small">Attached sources</span><input class="input" readonly value="${esc(sourceMaterials.length?sourceMaterials.map((row)=>`${row.material_role}: ${row.material_name}`).join(' · '):'None yet')}"/></label></div>`:''}
      <div class="packaging-library-grid packaging-source-grid">${rows.map((row)=>sourceMaterialCard(row,attachedIds,allowAttach)).join('')||'<p class="small">No purchased/source material templates are available yet. Use the editor below to create the first one.</p>'}</div>
      <details class="packaging-source-editor" open><summary>Add or edit a purchased/source material template</summary><div class="packaging-source-editor-body"><input type="hidden" id="packagingSourceMaterialEditId"/>
        <div class="packaging-source-editor-intro"><strong>1. Identify what we bought</strong><span class="small">Choose the product area and exact source category. Candle wax can be a base just like a soap base; fragrance/essential oils and colourants remain separate additions.</span></div>
        ...
        <div class="grid cols-2"><label><span class="small">Supplier/source URL</span><input class="input" id="packagingSourceUrl" type="url"/></label><label><span class="small">Supplier INCI/SDS/document URL</span><input class="input" id="packagingSourceDocumentUrl" type="url"/></label></div><label><span class="small">Supplier/product image URL</span><input class="input" id="packagingSourceImageUrl" type="url" placeholder="Optional R2/site image; placeholder is used when blank"/></label>
        <div class="grid cols-3"><label><span class="small">Intended use</span><select class="input" id="packagingSourceIntendedUse"><option value="rinse_off">Rinse-off</option><option value="leave_on">Leave-on</option><option value="both">Both</option><option value="not_applicable">Not applicable</option></select></label><label><span class="small">Source verification</span><select class="input" id="packagingSourceVerification"><option value="needs_review">Needs review</option><option value="supplier_verified">Supplier verified</option><option value="owner_verified">Owner verified</option><option value="blocked">Blocked</option></select></label><label><span class="small">Fragrance-allergen review</span><select class="input" id="packagingSourceFragranceReview"><option value="not_applicable">Not applicable</option><option value="needs_supplier_data">Needs supplier data</option><option value="needs_review">Needs review</option><option value="reviewed">Reviewed</option></select></label></div>
        <div class="packaging-source-editor-intro"><strong>2. Enter the supplier composition / Master INCI</strong><span class="small">These rows belong to the purchased material itself—not the current soap/candle project. Paste the supplier ingredient list, build draft rows, then correct them to the supplier-verified INCI.</span></div>
        <label><span class="small">Supplier ingredient declaration exactly as received</span><textarea class="input" id="packagingSourceIngredientRaw" rows="3" placeholder="Example: Coconut Oil, Sorbitol, Vegetable Glycerin…"></textarea></label><div class="packaging-save-actions"><button class="btn" id="importPackagingSourceIngredients" type="button">Create draft rows from supplier text</button><button class="btn" id="addPackagingSourceInci" type="button">Add source ingredient row</button></div>
        <div class="admin-table-wrap packaging-row-editor packaging-source-inci-editor"><table><thead><tr><th>#</th><th>INCI / supplier name</th><th>English/common</th><th>French/common</th><th>Organic</th><th>Allergen/review</th><th>Label</th><th></th></tr></thead><tbody id="packagingSourceInciRows">${sourceInciRow({},0)}</tbody></table></div>
        <div class="packaging-source-editor-intro"><strong>3. Supplier evidence and reusable characteristics</strong><span class="small">Benefits and supplier claims remain source evidence until deliberately reviewed for a finished label.</span></div>
        ...
        <div class="packaging-save-actions"><button class="btn primary" id="savePackagingSourceMaterial" type="button">Save material template</button><button class="btn" id="clearPackagingSourceMaterialEditor" type="button">Clear editor</button></div>
      </div></details></section>`;
  }
  ...
  function bindSourceLibraryControls(){
    id('savePackagingSourceMaterial')?.addEventListener('click',saveSourceMaterialTemplate);id('clearPackagingSourceMaterialEditor')?.addEventListener('click',clearSourceMaterialEditor);id('importPackagingSourceIngredients')?.addEventListener('click',importSourceIngredientText);id('addPackagingSourceInci')?.addEventListener('click',()=>addSourceInciRow({required_on_label:1}));bindSourceInciRows();
