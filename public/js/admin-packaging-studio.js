// Build 248 - Purchased source-material templates, soap-base inheritance, 2026 fragrance-allergen review, botanical palette, reusable templates, and explicit project deletion.
(() => {
  const STORAGE_KEY = 'dd_packaging_studio_local_draft_v5';
  const state = { projects: [], templates: [], products: [], inventory: [], referenceSources: [], formulaLibrary: [], contentLibrary: [], sourceMaterialLibrary: [], librarySchemaReady: true, sourceMaterialSchemaReady: true, detail: null, loading: false, activeTab: 'product' };
  ...
  function currentTemplate() {
    ...
    if (String(selected.package_type || '') === 'soap_ribbon') layout.design_profile = 'soap_reference_v2';
  ...
  function glacialRibbonSvg(data, template) {
    ...
    const family = data.packagingCollection || 'Glacial Purple'; const typeEn = data.packagingIdentityEn || data.packagingProductName || 'PRODUCT IDENTITY REQUIRED'; const typeFr = data.packagingIdentityFr || 'IDENTITÉ FRANÇAISE À RÉVISER';
    const structuredIngredients = Array.isArray(data.structured_ingredients) ? data.structured_ingredients : [];
    const enFromRows = structuredIngredients.filter((row) => Number(row.required_on_label) !== 0).map((row) => row.display_name_en || row.inci_name).filter(Boolean).map((row) => `• ${row}`).join('\n');
    const frFromRows = structuredIngredients.filter((row) => Number(row.required_on_label) !== 0).map((row) => row.display_name_fr || row.inci_name).filter(Boolean).map((row) => `• ${row}`).join('\n');
    ...
    const rearDiameter = Math.min(height - 12, Math.round(num(template.rear_width_mm, 38.1) / 25.4 * 100)); const rearR = rearDiameter / 2; const rearCx = 764;
    ...
    const roseArtwork = roseAsset ? `<image href="${xml(roseAsset)}" x="205" y="${centreY - 74}" width="122" height="148" preserveAspectRatio="xMidYMid meet"/>` : roseVector(266, centreY + 2, .72, rose);
    const frontOuter = artwork.badge_shape === 'scalloped' ? `<path d="${scallopedOvalPath(345,centreY,185,96)}" fill="${background}" stroke="${gold}" stroke-width="5"/>` : `<ellipse cx="345" cy="${centreY}" rx="185" ry="96" fill="${background}" stroke="${gold}" stroke-width="5"/>`; 
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${widthMm}mm" height="${heightMm}mm" viewBox="0 0 1100 ${height}" role="img" aria-label="${xml(family)} ${xml(typeEn)} continuous soap ribbon label">
      ...
      <rect x="2" y="${bandY + 2}" width="1096" height="71" rx="4" fill="none" stroke="${border}" stroke-width="4"/><line x1="2" y1="${bandY + 9}" x2="1098" y2="${bandY + 9}" stroke="${gold}" stroke-width="2"/><line x1="2" y1="${bandY + 66}" x2="1098" y2="${bandY + 66}" stroke="${gold}" stroke-width="2"/><g stroke="${secondary}" stroke-opacity=".25" stroke-width="1.5"><line x1="160" y1="${bandY + 14}" x2="160" y2="${bandY + 61}"/><line x1="538" y1="${bandY + 14}" x2="538" y2="${bandY + 61}"/><line x1="694" y1="${bandY + 14}" x2="694" y2="${bandY + 61}"/><line x1="836" y1="${bandY + 14}" x2="836" y2="${bandY + 61}"/></g>
      <text x="18" y="${bandY + 17}" font-size="7.4" font-weight="700" class="pkg-copy">INGREDIENTS:</text><g clip-path="url(#soap-en-ingredients)">${textBlock(en,18,bandY + 27,5.8,27,8,4.35,'start')}</g>
      <g>${frontOuter}<ellipse cx="345" cy="${centreY}" rx="178" ry="90" fill="none" stroke="${border}" stroke-width="2"/><ellipse cx="345" cy="${centreY}" rx="172" ry="84" fill="none" stroke="${gold}" stroke-width="1.2" stroke-dasharray="2 4"/>
        ${roseArtwork}
        <text x="372" y="${centreY - 54}" text-anchor="middle" font-size="11" class="pkg-script">${xml(topLabel)}</text><text x="372" y="${centreY - 40}" text-anchor="middle" font-size="8" class="pkg-script">- Devil n Dove -</text>
        <line x1="330" y1="${centreY - 31}" x2="414" y2="${centreY - 31}" stroke="${gold}" stroke-width="1"/><text x="372" y="${centreY - 28}" text-anchor="middle" font-size="6" fill="${gold}">${xml(centreMark)}</text>
        <text x="392" y="${centreY - 2}" text-anchor="middle" font-size="${fontSize(family,20,16,10)}" class="pkg-script">${xml(family)}</text><text x="392" y="${centreY + 20}" text-anchor="middle" font-size="13.6" class="pkg-serif">${xml(typeEn)}</text>
        <text x="392" y="${centreY + 43}" text-anchor="middle" font-size="8.8" class="pkg-script">${xml(data.packagingSubtitle || '')}</text><text x="392" y="${centreY + 63}" text-anchor="middle" font-size="7.2" font-weight="700" class="pkg-copy">${xml(bottomLabel)}</text>
        <circle cx="455" cy="${centreY - 12}" r="24" fill="${background}" stroke="${border}" stroke-width="1.8"/><circle cx="455" cy="${centreY - 12}" r="18" fill="none" stroke="${gold}" stroke-width="1"/><text x="455" y="${centreY - 18}" text-anchor="middle" font-size="4.6" font-weight="700" class="pkg-copy">DEVILNDOVE.COM</text><path d="M447 ${centreY - 6}q8-9 16 0q-8 9-16 0z" fill="none" stroke="${border}" stroke-width="1"/><text x="455" y="${centreY + 6}" text-anchor="middle" font-size="4.5" class="pkg-copy">HANDMADE</text>
      </g>
      <text x="548" y="${bandY + 17}" font-size="7.4" font-weight="700" class="pkg-copy">INGRÉDIENTS :</text><g clip-path="url(#soap-fr-ingredients)">${textBlock(fr,548,bandY + 27,5.8,26,8,4.25,'start')}</g>
      ...
      <g clip-path="url(#soap-claims)">${claims.slice(0,4).map((claim,index) => { const y = bandY + 12 + index * 12.5; return `${iconSvg(claim.icon_name, 854, y, secondary)}<text x="866" y="${y - 2.5}" font-size="6" font-weight="700" class="pkg-copy">${xml(claim.claim_en || '')}</text><text x="866" y="${y + 4}" font-size="5.15" class="pkg-copy">${xml(claim.claim_fr || '')}</text>`; }).join('')}</g>
      <line x1="850" y1="${bandY + 56}" x2="1084" y2="${bandY + 56}" stroke="${gold}" stroke-width="1"/><text x="854" y="${bandY + 64}" font-size="5.5" font-weight="700" class="pkg-copy">${xml(data.packagingNetQuantity || 'VERIFIED NET QUANTITY REQUIRED — DRAFT NOT FOR PRINT')}</text>
  ...
  function formulaLibraryOptions(value = '') {
    return `<option value="">Choose a saved soap formula…</option>${state.formulaLibrary.map((row)=>`<option value="${Number(row.packaging_formula_library_id||0)}" ${Number(value)===Number(row.packaging_formula_library_id)?'selected':''}>${esc(row.formula_name)}${Number(row.is_system)===1?' · built-in':' · custom'}</option>`).join('')}`;
  }

  function sourceMaterialOptions(value = '') {
    return `<option value="">Choose a purchased/source material…</option>${state.sourceMaterialLibrary.map((row)=>`<option value="${Number(row.packaging_source_material_template_id||0)}" ${Number(value)===Number(row.packaging_source_material_template_id)?'selected':''}>${esc(row.material_name)} · ${esc(String(row.material_type||'').replaceAll('_',' '))}${row.supplier_name?` · ${esc(row.supplier_name)}`:''}</option>`).join('')}`;
  }

  function sourceMaterialCard(row, attachedIds = new Set()) {
  ...
  function libraryContentCard(row) {
    const type=String(row.content_type||'ingredient'); const addLabel=type==='claim'?'Add claim':'Add ingredient row';
  ...
  function detailMarkup() {
    ...
    return `<section class="card packaging-editor-card"><div class="section-heading-row"><div><p class="eyebrow">${esc(project.project_key || 'New packaging system')}</p><h2 style="margin:0">${esc(project.project_name || project.product_name || 'Labeling and packaging project')}</h2></div><div class="packaging-project-actions"><button class="btn" id="duplicatePackagingProject" type="button">Duplicate</button><button class="btn" id="archivePackagingProject" type="button">Archive</button><button class="btn danger" id="deletePackagingProject" type="button">Delete label</button></div></div>
      ...
      ${tabPanel('library',`<h3>Purchased source materials, finished formulas and reusable label content</h3>
        ...
        <section class="packaging-library-section"><div class="section-heading-row"><div><h4>Purchased/source material templates</h4><p class="small">A soap base is the supplier product we bought. Its ingredient declaration, Master INCI review, allergen statement and supplier benefits remain attached to that source. A finished formula can inherit the base and then add fragrance oil, colourant or other additives separately.</p></div></div>
          <div class="grid cols-2"><label><span class="small">Source material</span><select class="input" id="packagingSourceMaterialTemplateId">${sourceMaterialOptions(attachedBase?.packaging_source_material_template_id||'')}</select></label><label><span class="small">Attached sources</span><input class="input" readonly value="${esc(sourceMaterials.length?sourceMaterials.map((row)=>`${row.material_role}: ${row.material_name}`).join(' · '):'None yet')}"/></label></div>
          <div class="packaging-library-grid packaging-source-grid">${state.sourceMaterialLibrary.map((row)=>sourceMaterialCard(row,attachedSourceIds)).join('')||'<p class="small">No purchased/source material templates are available yet.</p>'}</div>
          <details class="packaging-source-editor"><summary>Add or edit a purchased/source material template</summary><div class="packaging-library-entry-form"><input type="hidden" id="packagingSourceMaterialEditId"/><div class="grid cols-3"><label><span class="small">Material type</span><select class="input" id="packagingSourceMaterialType"><option value="soap_base">Soap base</option><option value="fragrance_oil">Fragrance / essential-oil blend</option><option value="colourant">Colourant</option><option value="additive">Other additive</option></select></label><label><span class="small">Template name</span><input class="input" id="packagingSourceMaterialName" placeholder="Example: Goat’s Milk Melt & Pour 10 lb"/></label><label><span class="small">Supplier product name</span><input class="input" id="packagingSourceSupplierProductName"/></label></div><div class="grid cols-3"><label><span class="small">Supplier</span><input class="input" id="packagingSourceSupplierName"/></label><label><span class="small">Supplier SKU</span><input class="input" id="packagingSourceSupplierSku"/></label><label><span class="small">Supplier/source URL</span><input class="input" id="packagingSourceUrl" type="url"/></label></div><div class="grid cols-2"><label><span class="small">Supplier/product image URL</span><input class="input" id="packagingSourceImageUrl" type="url" placeholder="Optional R2/site image; placeholder is used when blank"/></label><label><span class="small">Supplier INCI/SDS/document URL</span><input class="input" id="packagingSourceDocumentUrl" type="url"/></label></div><div class="grid cols-3"><label><span class="small">Intended use</span><select class="input" id="packagingSourceIntendedUse"><option value="rinse_off">Rinse-off</option><option value="leave_on">Leave-on</option><option value="both">Both</option><option value="not_applicable">Not applicable</option></select></label><label><span class="small">Source verification</span><select class="input" id="packagingSourceVerification"><option value="needs_review">Needs review</option><option value="supplier_verified">Supplier verified</option><option value="owner_verified">Owner verified</option><option value="blocked">Blocked</option></select></label><label><span class="small">Fragrance-allergen review</span><select class="input" id="packagingSourceFragranceReview"><option value="not_applicable">Not applicable</option><option value="needs_supplier_data">Needs supplier data</option><option value="needs_review">Needs review</option><option value="reviewed">Reviewed</option></select></label></div><label><span class="small">Supplier ingredient declaration exactly as received</span><textarea class="input" id="packagingSourceIngredientRaw" rows="3"></textarea></label><label><span class="small">Supplier allergen statement / characteristics</span><textarea class="input" id="packagingSourceAllergenStatement" rows="3"></textarea></label><label><span class="small">Supplier benefits / characteristics — separate entries with a blank line; use “Title: details”</span><textarea class="input" id="packagingSourceBenefits" rows="7"></textarea></label><label><span class="small">Supplier claim suggestions — one per line; never auto-approved</span><textarea class="input" id="packagingSourceClaims" rows="4"></textarea></label><label><span class="small">Fragrance allergens — one per line as INCI | concentration % | required | notes</span><textarea class="input" id="packagingSourceFragranceAllergens" rows="4"></textarea></label><div class="grid cols-2"><label><span class="small">Usage / process notes</span><textarea class="input" id="packagingSourceUsageNotes" rows="3"></textarea></label><label><span class="small">Compliance / review notes</span><textarea class="input" id="packagingSourceComplianceNotes" rows="3"></textarea></label></div><p class="small">Saving uses the current structured ingredient rows as this source material’s Master INCI list. Verify the supplier’s actual INCI documentation before marking it verified.</p><div class="packaging-save-actions"><button class="btn primary" id="savePackagingSourceMaterial" type="button">Save source-material template</button><button class="btn" id="clearPackagingSourceMaterialEditor" type="button">Clear editor</button></div></div></details>
          ${attachedBase?`<div class="packaging-source-detail"><h5>Current base evidence — ${esc(attachedBase.material_name)}</h5><p class="small"><strong>Raw supplier declaration:</strong> ${esc(attachedBase.ingredient_declaration_raw||'Not recorded')}</p><p class="small"><strong>Allergens:</strong> ${esc(attachedBase.allergen_statement||'No supplier statement recorded')}</p>${sourceBenefitMarkup(attachedBase)}</div>`:''}
        </section>
        <section class="packaging-library-section"><div class="section-heading-row"><div><h4>Finished soap formula library</h4><p class="small">A finished formula is what we actually make. It may inherit a purchased soap base, then include any fragrance, colourant and other additions. Saving a formula also records the currently selected source-base template when one is selected.</p></div></div><div class="grid cols-3"><label><span class="small">Saved formula</span><select class="input" id="packagingFormulaLibraryId">${formulaLibraryOptions()}</select></label><label><span class="small">New/custom formula name</span><input class="input" id="packagingFormulaLibraryName" placeholder="Example: Honey Goat Milk Soap"/></label><label><span class="small">Formula notes</span><input class="input" id="packagingFormulaLibraryNotes" placeholder="Finished-formula/source/lot/review notes…"/></label></div><div class="packaging-save-actions"><button class="btn primary" id="applyPackagingFormulaLibrary" type="button">Apply selected finished formula</button><button class="btn" id="savePackagingFormulaLibrary" type="button">Save current ingredients as finished formula</button><button class="btn danger" id="deletePackagingFormulaLibrary" type="button">Delete selected custom formula</button></div></section>
        ...
        <section class="packaging-library-section"><h4>Individual ingredients, fragrance / essential-oil blends and colourants</h4><p class="small">Use these for simple one-click additions. For a purchased product with its own ingredient/allergen/benefit evidence—including a premixed essential-oil blend—prefer a Source Material Template above and record every supplier-listed constituent/INCI rather than creating false separate bottle inventory.</p><div class="packaging-library-grid">${state.contentLibrary.filter((row)=>['ingredient','fragrance_oil','colourant'].includes(row.content_type)).map(libraryContentCard).join('')||'<p class="small">No reusable ingredient/additive entries are available yet.</p>'}</div><div class="packaging-library-entry-form"><div class="grid cols-3"><label><span class="small">Library type</span><select class="input" id="packagingLibraryContentType"><option value="ingredient">Ingredient</option><option value="fragrance_oil">Fragrance / essential-oil blend</option><option value="colourant">Colourant</option><option value="claim">Claim</option></select></label><label><span class="small">Library name</span><input class="input" id="packagingLibraryItemName" placeholder="Supplier/product or claim name"/></label><label><span class="small">INCI (ingredient/additive)</span><input class="input" id="packagingLibraryInci" placeholder="Supplier-verified INCI"/></label></div><div class="grid cols-3"><label><span class="small">English / claim text</span><input class="input" id="packagingLibraryTextEn"/></label><label><span class="small">French / claim text</span><input class="input" id="packagingLibraryTextFr"/></label><label><span class="small">Icon</span><select class="input" id="packagingLibraryIcon">${['leaf','hands','recycle','heart','none'].map((name)=>`<option>${name}</option>`).join('')}</select></label></div><label><span class="small">Supplier / allergen / usage notes</span><input class="input" id="packagingLibraryNotes"/></label><button class="btn" id="savePackagingLibraryContent" type="button">Save library item</button></div></section>`)}
  ...
  function applySelectedTemplate() {
    ...
    const values = {
      ...
      packagingTemplateRearHeight: template.rear_height_mm, packagingTemplateShape: layout.shape || 'rectangle', packagingDesignProfile: String(template.package_type) === 'soap_ribbon' ? 'soap_reference_v2' : (layout.design_profile || 'general_rectangle'),
  ...
  function refreshLibraryState(data) {
    state.projects=data.projects||state.projects; state.templates=data.templates||state.templates; state.formulaLibrary=data.formula_library||[]; state.contentLibrary=data.content_library||[]; state.sourceMaterialLibrary=data.source_material_library||[]; state.librarySchemaReady=data.library_schema_ready!==false; state.sourceMaterialSchemaReady=data.source_material_schema_ready!==false; if(data.detail!==undefined)state.detail=data.detail;
  }
  ...
  function clearSourceMaterialEditor(){['packagingSourceMaterialEditId','packagingSourceMaterialName','packagingSourceSupplierProductName','packagingSourceSupplierName','packagingSourceSupplierSku','packagingSourceUrl','packagingSourceImageUrl','packagingSourceDocumentUrl','packagingSourceIngredientRaw','packagingSourceAllergenStatement','packagingSourceBenefits','packagingSourceClaims','packagingSourceFragranceAllergens','packagingSourceUsageNotes','packagingSourceComplianceNotes'].forEach((field)=>{if(id(field))id(field).value='';});if(id('packagingSourceMaterialType'))id('packagingSourceMaterialType').value='soap_base';if(id('packagingSourceIntendedUse'))id('packagingSourceIntendedUse').value='rinse_off';if(id('packagingSourceVerification'))id('packagingSourceVerification').value='needs_review';if(id('packagingSourceFragranceReview'))id('packagingSourceFragranceReview').value='not_applicable';}
  function loadSourceMaterialEditor(sourceId){const row=sourceMaterialById(sourceId);if(!row)return;if(Number(row.is_system)===1)message('Built-in source templates are protected. You can review/apply them or save your own custom copy.','');if(id('packagingSourceMaterialTemplateId'))id('packagingSourceMaterialTemplateId').value=String(sourceId);if(id('packagingSourceMaterialEditId'))id('packagingSourceMaterialEditId').value=Number(row.is_system)===1?'':String(sourceId);if(id('packagingSourceMaterialType'))id('packagingSourceMaterialType').value=row.material_type||'soap_base';if(id('packagingSourceMaterialName'))id('packagingSourceMaterialName').value=Number(row.is_system)===1?`${row.material_name} copy`:row.material_name||'';if(id('packagingSourceSupplierProductName'))id('packagingSourceSupplierProductName').value=row.supplier_product_name||'';if(id('packagingSourceSupplierName'))id('packagingSourceSupplierName').value=row.supplier_name||'';if(id('packagingSourceSupplierSku'))id('packagingSourceSupplierSku').value=row.supplier_sku||'';if(id('packagingSourceUrl'))id('packagingSourceUrl').value=row.source_url||'';if(id('packagingSourceImageUrl'))id('packagingSourceImageUrl').value=row.source_image_url||'';if(id('packagingSourceDocumentUrl'))id('packagingSourceDocumentUrl').value=row.supplier_document_url||'';if(id('packagingSourceIntendedUse'))id('packagingSourceIntendedUse').value=row.intended_use||'rinse_off';if(id('packagingSourceVerification'))id('packagingSourceVerification').value=Number(row.is_system)===1?'needs_review':(row.verification_status||'needs_review');if(id('packagingSourceFragranceReview'))id('packagingSourceFragranceReview').value=row.fragrance_allergen_review_status||'not_applicable';if(id('packagingSourceIngredientRaw'))id('packagingSourceIngredientRaw').value=row.ingredient_declaration_raw||'';if(id('packagingSourceAllergenStatement'))id('packagingSourceAllergenStatement').value=row.allergen_statement||'';if(id('packagingSourceBenefits'))id('packagingSourceBenefits').value=(row.benefits||[]).map((b)=>`${b.title||'Benefit'}: ${b.body||''}`).join('\n\n');if(id('packagingSourceClaims'))id('packagingSourceClaims').value=(row.supplier_claims||[]).map((c)=>c.claim_en||'').filter(Boolean).join('\n');if(id('packagingSourceFragranceAllergens'))id('packagingSourceFragranceAllergens').value=(row.fragrance_allergens||[]).map((a)=>`${a.inci_name||''} | ${a.concentration_percent??''} | ${Number(a.disclosure_required)===1?'required':''} | ${a.notes||''}`.trim()).join('\n');if(id('packagingSourceUsageNotes'))id('packagingSourceUsageNotes').value=row.usage_notes||'';if(id('packagingSourceComplianceNotes'))id('packagingSourceComplianceNotes').value=row.compliance_notes||'';const details=id('packagingSourceMaterialName')?.closest('details');if(details)details.open=true;}
  function applySourceMaterialRows(row){const rows=Array.isArray(row.master_inci)?row.master_inci:[];if(String(row.material_type)==='soap_base'){if(id('soapIngredientRows'))id('soapIngredientRows').innerHTML=rows.map(ingredientRow).join('');}else{for(const item of rows)addIngredient(item);}if(id('packagingInci'))id('packagingInci').value=ingredientRowsFromDom().map((item)=>item.inci_name).filter(Boolean).join(', ');for(const claim of (row.supplier_claims||[])){if(!claim.claim_en&&!claim.claim_fr)continue;addClaim({claim_en:claim.claim_en||'',claim_fr:claim.claim_fr||'',icon_name:claim.icon_name||'leaf',is_approved:0,compliance_note:claim.compliance_note||`Inherited from supplier/source template “${row.material_name}”; review before approval.`});}bindDynamicRows();renderPreview();}
  async function applySourceMaterialTemplate(sourceIdValue=0){const sourceId=Number(sourceIdValue||id('packagingSourceMaterialTemplateId')?.value||0);const row=sourceMaterialById(sourceId);if(!row){message('Choose a purchased/source material template first.','error');return;}const projectId=Number(id('packagingProjectId')?.value||0);try{if(projectId){message(`Attaching ${row.material_name} and loading its source evidence…`);const data=await api({action:'apply_source_material_template',packaging_project_id:projectId,packaging_source_material_template_id:sourceId});refreshLibraryState(data);state.activeTab='library';renderProjects();renderMain();}if(id('packagingSourceMaterialTemplateId'))id('packagingSourceMaterialTemplateId').value=String(sourceId);applySourceMaterialRows(row);message(`${row.material_name} applied. ${String(row.material_type)==='soap_base'?'Its Master INCI rows replaced the base ingredient rows.':'Its ingredient rows were added.'} Supplier benefits and allergen evidence remain source records; supplier claim suggestions were added only as unapproved drafts.`,'success');}catch(error){message(error.message||'Source-material template could not be applied.','error');}}
  async function saveSourceMaterialTemplate(){const materialName=String(id('packagingSourceMaterialName')?.value||'').trim();if(!materialName){message('Enter a source-material template name first.','error');return;}const masterInci=ingredientRowsFromDom();if(!masterInci.length){message('Add the supplier/base ingredient rows in Ingredients before saving its Master INCI template.','error');return;}try{const data=await api({action:'save_source_material_template',packaging_project_id:Number(id('packagingProjectId')?.value||0),packaging_source_material_template_id:Number(id('packagingSourceMaterialEditId')?.value||0)||null,material_type:id('packagingSourceMaterialType')?.value||'soap_base',material_name:materialName,supplier_product_name:id('packagingSourceSupplierProductName')?.value||'',supplier_name:id('packagingSourceSupplierName')?.value||'',supplier_sku:id('packagingSourceSupplierSku')?.value||'',source_url:id('packagingSourceUrl')?.value||'',source_image_url:id('packagingSourceImageUrl')?.value||'',supplier_document_url:id('packagingSourceDocumentUrl')?.value||'',source_reference:'Packaging Studio source-material library',intended_use:id('packagingSourceIntendedUse')?.value||'rinse_off',ingredient_declaration_raw:id('packagingSourceIngredientRaw')?.value||'',master_inci:masterInci.map((row)=>({...row,verification_status:['supplier_verified','owner_verified'].includes(id('packagingSourceVerification')?.value)?id('packagingSourceVerification').value:'needs_review'})),allergen_statement:id('packagingSourceAllergenStatement')?.value||'',fragrance_allergens:parseFragranceAllergenLines(id('packagingSourceFragranceAllergens')?.value||''),fragrance_allergen_review_status:id('packagingSourceFragranceReview')?.value||'not_applicable',benefits:parseBenefitsText(id('packagingSourceBenefits')?.value||''),supplier_claims:parseClaimLines(id('packagingSourceClaims')?.value||''),usage_notes:id('packagingSourceUsageNotes')?.value||'',compliance_notes:id('packagingSourceComplianceNotes')?.value||'',verification_status:id('packagingSourceVerification')?.value||'needs_review'});refreshLibraryState(data);state.activeTab='library';renderProjects();renderMain();message(data.message,'success');}catch(error){message(error.message||'Source-material template could not be saved.','error');}}
  ...
  async function applyFormulaLibrary() {
    ...
    if(!formula){message('Choose a saved soap formula first.','error');return;}
    ...
    try {
      ...
      bindDynamicRows(); renderPreview(); message(`Formula “${formula.formula_name}” applied. ${formula.source_material_template_id?'Its purchased soap-base dependency is attached for source review. ':''}Review the finished ingredients and save the project.`,'success');
  ...
  async function saveFormulaLibrary() {
    ...
    try{message('Saving reusable soap formula…');const data=await api({action:'save_formula_library',packaging_project_id:Number(id('packagingProjectId')?.value||0),packaging_formula_library_id:selected&&Number(selected.is_system)!==1&&!requestedName?selectedId:null,formula_name:formulaName,product_family:id('packagingCollection')?.value||'',product_identity_en:id('packagingIdentityEn')?.value||'',product_identity_fr:id('packagingIdentityFr')?.value||'',default_rose_asset_id:id('packagingRoseAsset')?.value||'',default_rose_colour:id('packagingRoseColour')?.value||'',structured_ingredients:ingredients,source_material_template_id:Number(id('packagingSourceMaterialTemplateId')?.value||0)||null,notes:id('packagingFormulaLibraryNotes')?.value||''});refreshLibraryState(data);state.activeTab='library';renderProjects();renderMain();message(data.message,'success');}
  ...
  function applyGlacialPurpleReference() {
    ...
    const values = { packagingCollection:'Glacial Purple', packagingRoseColour:'#A57BCB', packagingThemeColour:'#FBF5E8', packagingBorderColour:'#32105E', packagingAccentGold:'#B88A2F', packagingSecondaryColour:'#5A2A86', packagingRoseAsset:'rose-lavender-v1', packagingArtworkAsset:'', packagingDesignProfile:'soap_reference_v2' };
