// Build 261 - Inventory-first packaging components/materials, reusable claim library UI, and tightened soap-label print zones.
(() => {
  ...
  function glacialRibbonSvg(data, template) {
    ...
    const zones={en:{x:20,w:146},front:{cx:365,rx:190},fr:{x:570,w:142},rear:{cx:792,r:61},claims:{x:875,w:205}};
    ...
    const claimMarkup=claims.slice(0,4).map((claim,index)=>{const y=bandY+10+index*11.6;const enText=String(claim.claim_en||'');const frText=String(claim.claim_fr||'');const enSize=fontSize(enText,28,4.7,3.5);const frSize=fontSize(frText,32,4.05,3.2);return `${iconSvg(claim.icon_name,zones.claims.x+7,y+1.6,secondary)}<text x="${zones.claims.x+18}" y="${y}" font-size="${enSize}" font-weight="700" class="pkg-copy">${xml(enText)}</text><text x="${zones.claims.x+18}" y="${y+4.9}" font-size="${frSize}" class="pkg-copy">${xml(frText)}</text>`;}).join('');
    const familyLines=wrapPlainLines(family,20,2);const familyStart=centreY+(familyLines.length>1?-7:-1);const familyMarkup=familyLines.map((line,index)=>`<text x="420" y="${familyStart+index*9}" text-anchor="middle" font-size="${fontSize(line,20,10.5,8.2)}" class="pkg-script">${xml(line)}</text>`).join('');
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${widthMm}mm" height="${heightMm}mm" viewBox="0 0 1100 ${height}" role="img" aria-label="${xml(family)} ${xml(typeEn)} continuous soap ribbon label" data-soap-layout="reference-v3">
      <defs>
        ...
        <clipPath id="soap-en-ingredients"><rect x="${zones.en.x}" y="${bandY+18}" width="${zones.en.w-4}" height="${bandHeight-35}"/></clipPath>
        <clipPath id="soap-fr-ingredients"><rect x="${zones.fr.x}" y="${bandY+18}" width="${zones.fr.w-4}" height="${bandHeight-35}"/></clipPath>
        <clipPath id="soap-claims"><rect x="${zones.claims.x}" y="${bandY+5}" width="${zones.claims.w}" height="${bandHeight-28}"/></clipPath>
      </defs>
      <style>
      ...
      <g>
        ...
        ${familyMarkup}
        <text x="420" y="${centreY+20}" text-anchor="middle" font-size="${fontSize(typeEn,25,10.8,7.2)}" class="pkg-serif">${xml(typeEn)}</text>
        <text x="420" y="${centreY+36}" text-anchor="middle" font-size="${fontSize(data.packagingSubtitle||'',30,7.3,5.5)}" class="pkg-script">${xml(data.packagingSubtitle||'')}</text>
        <text x="420" y="${centreY+52}" text-anchor="middle" font-size="6.2" font-weight="700" class="pkg-copy">${xml(bottomLabel)}</text>
      ...
      <line x1="${zones.claims.x+3}" y1="${bandY+53.5}" x2="1085" y2="${bandY+53.5}" stroke="${gold}" stroke-width="1"/>
      <text x="${zones.claims.x+5}" y="${bandY+61.5}" font-size="${fontSize(data.packagingNetQuantity||'',45,4.6,3.6)}" font-weight="700" class="pkg-copy">${xml(data.packagingNetQuantity||'VERIFIED NET QUANTITY REQUIRED — DRAFT NOT FOR PRINT')}</text>
  ...
  function sourceMaterialManagerMarkup({attachedIds=new Set(),sourceMaterials=[],allowAttach=true,standalone=false}={}) {
    ...
    return `<section class="packaging-library-section packaging-material-library-hub"><div class="section-heading-row packaging-library-heading"><div><p class="eyebrow">Material Library</p><h4>${standalone?'Manage purchased materials before creating a label':'Purchased/source material templates'}</h4><p class="small">This is where we enter soap bases, candle waxes, fragrance and essential-oil blends, colours/micas/dyes, carrier oils/butters and other additives. Each purchased product keeps its own supplier ingredient/INCI list, claims, benefits and allergen evidence.</p></div><span class="status-pill">${rows.length} template${rows.length===1?'':'s'}</span></div>
      ...
      <details class="packaging-source-editor" open><summary>Add or edit a purchased/source material template</summary><div class="packaging-source-editor-body"><input type="hidden" id="packagingSourceMaterialEditId"/><input type="hidden" id="packagingSourceInventoryId"/>
        <div class="packaging-inventory-material-import"><div><strong>Start with information already in Devil n Dove Inventory</strong><span class="small">This is the normal first choice. If the item was already imported or entered in Inventory, Packaging Studio reuses its name, supplier, SKU, Amazon/source URL, image, cost/source metadata and any linked source-material template. We only return to Amazon when Inventory does not contain what we need.</span></div><div class="packaging-amazon-material-row"><input class="input" id="packagingSourceInventorySearch" type="search" list="packagingSourceInventoryOptions" placeholder="Type soap base, wax, essential oil, mica, colour…" autocomplete="off"/><button class="btn primary" id="loadPackagingSourceInventory" type="button">Use inventory information</button></div><div class="small" id="packagingSourceInventoryStatus">Choose the actual purchased inventory item. If it already has a linked Material Library template, that template will open instead of creating a duplicate.</div>${inventoryDatalist('packagingSourceInventoryOptions')}</div>
        <details class="packaging-amazon-material-import"><summary>Amazon fallback — only when the item is not already captured in Inventory</summary><div><span class="small">Paste the exact Amazon.ca product page only for missing source information. Nothing is saved until we review the fields below and press <b>Save material template</b>.</span></div><div class="packaging-amazon-material-row"><input class="input" id="packagingSourceAmazonUrl" type="url" placeholder="https://www.amazon.ca/dp/…"/><button class="btn" id="previewPackagingSourceAmazon" type="button">Create draft from Amazon link</button></div><div class="small" id="packagingSourceAmazonStatus">Amazon descriptions can be incomplete. Supplier/manufacturer INCI, SDS and allergen documents remain the authority.</div></details>
