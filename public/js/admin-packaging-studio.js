...
(() => {
  ...
  function svgMarkup() { const data = snapshot(); const template = currentTemplate(); return String(data.packagingType || template.package_type) === 'soap_ribbon' ? glacialRibbonSvg(data, template) : genericPackageSvg(data, template); }
  ...
  function compliance(payload) {
    ...
    const isSoap = String(payload.package_type || template.package_type || '') === 'soap_ribbon';
    const checks = [
    ...
    if (!isSoap) warnings.push('General packaging preview: verify category-specific legal fields, physical dieline, barcode/QR destination and material fit before approval.');
  ...
  async function createProject() {
    ...
    const requestedType = String(prompt('Packaging type: soap_ribbon, product_label, candle_label, jewelry_card, or package_insert', product?.product_category?.toLowerCase().includes('soap') ? 'soap_ribbon' : 'product_label') || 'product_label').trim().toLowerCase();
