// Build 275 - Packaging source inheritance, French draft recovery, claim spacing, brand title type, and expanded rose directions.
(() => {
  ...
  function glacialRibbonSvg(data, template) {
    ...
    const claimMarkup=claims.slice(0,4).map((claim,index)=>{const y=bandY+14+index*10.2;const enText=String(claim.claim_en||'');const frText=String(claim.claim_fr||'');const enSize=fontSize(enText,25,4.3,3.2);const frSize=fontSize(frText,29,3.75,2.9);const textX=zones.claims.x+27;return `${iconSvg(claim.icon_name,zones.claims.x+8,y+1.2,secondary)}<text x="${textX}" y="${y}" font-size="${enSize}" font-weight="700" class="pkg-copy">${xml(enText)}</text><text x="${textX}" y="${y+4.35}" font-size="${frSize}" class="pkg-copy">${xml(frText)}</text>`;}).join('');
