// Build 256 - Amazon-assisted Material Library templates, reference-aligned soap label renderer, and retained packaging controls.
(() => {
  ...
  function glacialRibbonSvg(data, template) {
    ...
    const zones={en:{x:16,w:154},front:{cx:365,rx:190},fr:{x:566,w:150},rear:{cx:792,r:61},claims:{x:872,w:212}};
    const frontRy=Math.min(72,centreY-3);
    const roseArtwork=roseAsset?`<image href="${xml(roseAsset)}" x="206" y="${centreY-62}" width="128" height="124" preserveAspectRatio="xMidYMid meet"/>`:roseVector(268,centreY,.62,rose);
    ...
    const claimMarkup=claims.slice(0,4).map((claim,index)=>{const y=bandY+11+index*13.2;return `${iconSvg(claim.icon_name,zones.claims.x+8,y+1,secondary)}<text x="${zones.claims.x+21}" y="${y}" font-size="5.4" font-weight="700" class="pkg-copy">${xml(claim.claim_en||'')}</text><text x="${zones.claims.x+21}" y="${y+6}" font-size="4.7" class="pkg-copy">${xml(claim.claim_fr||'')}</text>`;}).join('');
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${widthMm}mm" height="${heightMm}mm" viewBox="0 0 1100 ${height}" role="img" aria-label="${xml(family)} ${xml(typeEn)} continuous soap ribbon label" data-soap-layout="reference-v3">
      <defs>
        ...
        <clipPath id="soap-en-ingredients"><rect x="${zones.en.x}" y="${bandY+17}" width="${zones.en.w-4}" height="${bandHeight-23}"/></clipPath>
        <clipPath id="soap-fr-ingredients"><rect x="${zones.fr.x}" y="${bandY+17}" width="${zones.fr.w-4}" height="${bandHeight-23}"/></clipPath>
        <clipPath id="soap-claims"><rect x="${zones.claims.x}" y="${bandY+5}" width="${zones.claims.w}" height="${bandHeight-18}"/></clipPath>
      </defs>
      <style>
      ...
      <text x="${zones.en.x}" y="${bandY+15}" font-size="7" font-weight="700" class="pkg-copy">INGREDIENTS:</text><g clip-path="url(#soap-en-ingredients)">${textBlock(en,zones.en.x,bandY+25,5.7,29,8,4.25,'start')}</g>
      <g>
        ...
        ${roseArtwork}
        <text x="408" y="${centreY-44}" text-anchor="middle" font-size="10.4" class="pkg-script">${xml(topLabel)}</text><text x="408" y="${centreY-31}" text-anchor="middle" font-size="7.6" class="pkg-script">- Devil n Dove -</text>
        <line x1="372" y1="${centreY-23}" x2="444" y2="${centreY-23}" stroke="${gold}" stroke-width="1"/><text x="408" y="${centreY-20}" text-anchor="middle" font-size="6" fill="${gold}">${xml(centreMark)}</text>
        <text x="420" y="${centreY+1}" text-anchor="middle" font-size="${fontSize(family,20,16,10)}" class="pkg-script">${xml(family)}</text>
        <text x="420" y="${centreY+20}" text-anchor="middle" font-size="${fontSize(typeEn,28,12.5,9)}" class="pkg-serif">${xml(typeEn)}</text>
        <text x="420" y="${centreY+38}" text-anchor="middle" font-size="${fontSize(data.packagingSubtitle||'',34,8.2,6.2)}" class="pkg-script">${xml(data.packagingSubtitle||'')}</text>
        <text x="420" y="${centreY+55}" text-anchor="middle" font-size="6.8" font-weight="700" class="pkg-copy">${xml(bottomLabel)}</text>
        <circle cx="504" cy="${centreY-5}" r="25" fill="${background}" stroke="${border}" stroke-width="1.7"/><circle cx="504" cy="${centreY-5}" r="19" fill="none" stroke="${gold}" stroke-width="1"/>
        <text x="504" y="${centreY-12}" text-anchor="middle" font-size="4.3" font-weight="700" class="pkg-copy">DEVILNDOVE.COM</text><path d="M496 ${centreY-2}q8-9 16 0q-8 9-16 0z" fill="none" stroke="${border}" stroke-width="1"/><text x="504" y="${centreY+11}" text-anchor="middle" font-size="4.2" class="pkg-copy">HANDMADE</text>
      </g>
      <text x="${zones.fr.x}" y="${bandY+15}" font-size="7" font-weight="700" class="pkg-copy">INGRÉDIENTS :</text><g clip-path="url(#soap-fr-ingredients)">${textBlock(fr,zones.fr.x,bandY+25,5.7,27,8,4.05,'start')}</g>
      ...
      <line x1="${zones.claims.x+3}" y1="${bandY+bandHeight-15}" x2="1085" y2="${bandY+bandHeight-15}" stroke="${gold}" stroke-width="1"/>
      <text x="${zones.claims.x+5}" y="${bandY+bandHeight-7}" font-size="${fontSize(data.packagingNetQuantity||'',48,5.2,4.1)}" font-weight="700" class="pkg-copy">${xml(data.packagingNetQuantity||'VERIFIED NET QUANTITY REQUIRED — DRAFT NOT FOR PRINT')}</text>
  ...
  function sourceMaterialManagerMarkup({attachedIds=new Set(),sourceMaterials=[],allowAttach=true,standalone=false}={}) {
    ...
    return `<section class="packaging-library-section packaging-material-library-hub"><div class="section-heading-row packaging-library-heading"><div><p class="eyebrow">Material Library</p><h4>${standalone?'Manage purchased materials before creating a label':'Purchased/source material templates'}</h4><p class="small">This is where we enter soap bases, candle waxes, fragrance and essential-oil blends, colours/micas/dyes, carrier oils/butters and other additives. Each purchased product keeps its own supplier ingredient/INCI list, claims, benefits and allergen evidence.</p></div><span class="status-pill">${rows.length} template${rows.length===1?'':'s'}</span></div>
      ...
      <details class="packaging-source-editor" open><summary>Add or edit a purchased/source material template</summary><div class="packaging-source-editor-body"><input type="hidden" id="packagingSourceMaterialEditId"/>
        <div class="packaging-amazon-material-import"><div><strong>Start from an Amazon product link</strong><span class="small">Paste the exact Amazon.ca product page for a soap base, essential/fragrance oil, wax, mica or colour. Preview fills whatever Amazon exposes; nothing is saved until we review the fields below and press <b>Save material template</b>.</span></div><div class="packaging-amazon-material-row"><input class="input" id="packagingSourceAmazonUrl" type="url" placeholder="https://www.amazon.ca/dp/…"/><button class="btn primary" id="previewPackagingSourceAmazon" type="button">Create draft from Amazon link</button></div><div class="small" id="packagingSourceAmazonStatus">Amazon descriptions can be incomplete. Supplier/manufacturer INCI, SDS and allergen documents remain the authority.</div></div>
