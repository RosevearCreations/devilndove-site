// Release 467 Build 183 identity authority, extended by Release 467 Build 189 Inventory evidence closure and Build 200 Supplier & Source Evidence Workbench.
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  const state={queue:'supplier_source',supplierFilter:'all',q:'',loading:false,summaryLoaded:false,items:[],cursor:0,queueKey:''};
  ...
  function rememberItems(items=[]){
    ...
    state.queueKey='ddSupplierSourceCursor:'+state.supplierFilter+':'+state.q.toLowerCase();
  ...
  async function loadIssues(){
    if(state.loading)return;state.loading=true;msg('Loading up to 40 reviewed supplier/source candidates…');
    ...
    try{
      ...
      msg((data.items||[]).length+' issue row(s) loaded. Candidate evidence is not auto-filled and nothing was changed.');
  ...
  function openNextUnresolved(){
    ...
    const button=document.getElementById('inventorySupplierSourceNext');if(button)button.textContent='Open next unresolved ('+(state.cursor+1)+'/'+state.items.length+')';
  ...
  function syncSupplierControls(){const wrap=document.getElementById('inventorySupplierSourceFilterWrap'),next=document.getElementById('inventorySupplierSourceNext'),supplierMode=state.queue==='supplier_source';if(wrap)wrap.hidden=!supplierMode;if(next)next.hidden=!supplierMode;}
  function render(){
    mount.innerHTML='<section class="card inventory-identity-cleanup" aria-labelledby="inventoryIdentityCleanupHeading">'
      +'<div class="section-heading-row"><div><p class="inventory-operations-eyebrow">Release 467 Build 189 evidence closure retained · Build 200</p><h3 id="inventoryIdentityCleanupHeading">Tool & Supply Identity Review — Supplier & Source Workbench</h3><p class="small">Build 200 turns missing supplier/source evidence into a bounded repair queue. Matching Inventory/Catalog facts are candidates only, never auto-filled. Inventory Operations remains the sole mutation authority.</p></div><button class="btn primary" type="button" id="inventoryIdentityCleanupSummaryButton">Load identity health</button></div>'
      ...
      +'<label class="small">Search<input class="input" id="inventoryIdentityCleanupSearch" type="search" placeholder="name, key, category, supplier"/></label><button class="btn" type="button" id="inventoryIdentityCleanupLoad">Load 40 issue rows</button><button class="btn" type="button" id="inventorySupplierSourceNext" disabled>Open next unresolved</button></div>'
    ...
    document.getElementById('inventoryIdentityCleanupQueue')?.addEventListener('change',(event)=>{state.queue=String(event.target.value||'supplier_source');syncSupplierControls();});
