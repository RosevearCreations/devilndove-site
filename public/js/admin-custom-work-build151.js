// Release 467 Build 210 — seller Custom Work command surface with structured Intake 2.0 context.
    ...
    mount.innerHTML = `
      <section class="card custom-work-b151" aria-labelledby="customWork151Heading">
        ...
        <div class="custom-work-b151__grid"><div><h3 style="margin-top:0">Custom Work queue</h3><div class="custom-work-b151__list">${open.length ? open.slice(0,30).map((x)=>{
          ...
          return `<article class="custom-work-b151__item"><div class="custom-work-b151__item-head"><div><strong>${esc(x.name||'Customer')} — ${esc(x.product_interest||x.request_type||'Custom request')}</strong><div class="small">${esc(x.email||'')} ${x.deadline_date?`• requested by ${esc(x.deadline_date)}`:''}</div></div><span class="custom-work-b151__pill">${esc(x.status||'new')}</span></div><div class="custom-work-b151__meta"><span class="custom-work-b151__pill">Qty: ${Number(x.quantity||1)}</span><span class="custom-work-b151__pill">Intent: ${esc(x.project_intent||'unknown')}</span><span class="custom-work-b151__pill">Capability: ${esc(x.requested_capability_key||(Number(x.help_choose_method||0)===1?'help me decide':'not specified'))}</span><span class="custom-work-b151__pill">Supplied item: ${Number(x.supplied_item||0)===1?'yes':'no'}</span><span class="custom-work-b151__pill">Quote: ${esc(q.quote_status||'not started')}</span><span class="custom-work-b151__pill">Quote total: ${money(q.quote_total_cents||0)}</span><span class="custom-work-b151__pill">Deposit/payment: ${esc(p.payment_request_status||'not started')}</span><span class="custom-work-b151__pill">Order: ${esc(o.order_draft_status||'not started')}</span><span class="custom-work-b151__pill">Refs: ${Number(x.reference_upload_count||0)}</span></div>${(x.intended_use||x.organization_name||x.event_context_structured||x.desired_material||x.desired_finish||x.personalization_text||x.tolerance_size_notes)?`<div class="small custom-work-b151__context"><strong>Structured intake:</strong> ${esc(compact([x.intended_use&&`Use: ${x.intended_use}`,x.organization_name&&`Org: ${x.organization_name}`,x.event_context_structured&&`Event: ${x.event_context_structured}`,x.desired_material&&`Material: ${x.desired_material}`,x.desired_finish&&`Finish: ${x.desired_finish}`,x.personalization_text&&`Personalization: ${x.personalization_text}`,x.tolerance_size_notes&&`Size/tolerance: ${x.tolerance_size_notes}`].filter(Boolean).join(' • '),620))}</div>`:(ctx?`<div class="small custom-work-b151__context"><strong>Gift / pickup / event context:</strong> ${esc(compact(ctx,420))}</div>`:`<div class="small custom-work-b151__context">${esc(compact(x.message,260))}</div>`)}</article>`;
