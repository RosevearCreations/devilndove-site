...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function exportPack(format){
    ...
    a.href=url;a.download=`devilndove-release467-build266-${format==='markdown'?'closure-evidence.md':format+'.json'}`;
    document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);
  }
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 266 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 266</p><h2>CAIP Multipart Recovery Integrity Review</h2><p class="small">Current action only. The immutable Build 265 Development/Production GREEN predecessor is recovered by the shared release evidence. Build 266 verifies multipart interruption/resume, completed-part ETag continuity, exact-size completion and immutable raw-source recovery without contacting Production media.</p><p class="small"><strong>Integrity outcome:</strong> completed parts with ETags remain authoritative, incomplete plans fail closed before R2 completion, final R2 size must match exactly, and uncertain finalized binaries are preserved. Live private-bucket and interruption/resume acceptance remains external evidence.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 266 does not convert missing private-bucket or live interruption/recovery proof into acceptance.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 266 is a read-only multipart recovery integrity review. It changes no schema, D1/R2 business data or media, provider action, Product publication, Inventory movement, Finance posting, workflow deletion or branch protection.</p></section>`;
