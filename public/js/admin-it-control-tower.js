...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 273 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 273</p><h2>Content Studio Standalone-Project Bridge</h2><p class="small">Current action only. The immutable Build 272 Development/Production GREEN predecessor is recovered by the shared release evidence. Build 273 reuses one existing Creative Process identity and exactly one existing CAIP workspace for one Content Studio package.</p><p class="small"><strong>Closure outcome:</strong> missing, ambiguous or conflicting project identity fails closed; a missing Content Studio package may be created without fabricating another Creative Process or CAIP project.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 273 does not convert project identity conflicts, private-media, provider, publication or downstream review gaps into acceptance.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 273 is a read-only release-control projection. The source release adds an idempotent existing-identity Content Studio bridge but creates no Creative Process/CAIP project, performs no R2 deletion, public promotion, provider action, Inventory movement, Finance posting, workflow deletion or branch protection.</p></section>`;
