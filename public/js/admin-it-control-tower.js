// Release 467 Build 215 — current I.T. authority renderer over canonical Build 213 GREEN restart boundary.
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function exportPack(format){
    ...
    a.href=url;a.download=`devilndove-release467-build212-${format==='markdown'?'closure-evidence.md':format+'.json'}`;
  ...
  async function load(){
    ...
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 215</p><h2>Digital Proof &amp; Customer Approval</h2><p class="small">Current read-only I.T. authority. Build 212 is the exact verified Development and Production baseline; Build 213 adds private versioned customer proof/approval while Custom Work, Packaging, CAIP/media, Inventory and publication remain their existing authorities.</p><p class="small"><strong>Verified Development:</strong> <code>${esc(v.dev_sha)}</code> / <code>${esc(v.tree_sha)}</code></p><p class="small"><strong>Production GREEN authority:</strong> Build ${esc(p.build)} • Pages ${esc(p.production_pages_deploy_run)} • Live Resources ${esc(p.production_live_resource_integrity_run)} • Product Browser ${esc(p.products_browser_proof_run)} • Remote D1 ${esc(p.remote_d1_queries)}</p><p class="small"><strong>Active Build 213 acceptance:</strong> current pointer, I.T., Deployment Preflight, Reliability, migration 0015 and Build 213 proof must agree on exact Build 212 closure; final Build 213 SHA is resolved only after exact-head proof.</p><p class="small"><strong>Evidence ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">Stripe Development, PayPal sandbox and Social/OAuth remain HOLD_EXTERNAL; CAIP private media remains EVIDENCE_DEPENDENT. Cloudflare Access remains externally governed.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Canonical migrations are 0001–0015. Build 213 adds private proof versions and append-only approval/change evidence over existing Custom Work; request-time DDL, publication authority, provider messaging, payment, Inventory mutation and private CAIP exposure remain closed.</p>${(d.truth_notes||[]).map((x)=>`<p class="small">• ${esc(x)}</p>`).join('')}</section>`;
