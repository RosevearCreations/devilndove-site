...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 256 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 256</p><h2>Refinement Outcomes Renewal II</h2><p class="small">Current action only. The immutable Build 255 Development/Production proof is rendered once by the shared evidence summary above; this panel closes the Builds 249–255 refinement cycle and shows the measured successor decision without creating a parallel dashboard.</p><p class="small"><strong>Renewal outcome:</strong> Builds 249–255 account for 870 workflow runs on accepted dev/main heads: 863 success, 7 historical failures, zero skipped, and zero rerun attempts. Build 250 remains under its 25,000-row provider ceiling at 2,178 rows but retains a 13-SELECT read hotspot; Build 255 retains high proof fan-out. The queue therefore continues with Build 257.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 256 does not alter provider HOLD/EVIDENCE states.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 256 is a read-only Refinement Outcomes Renewal II. It creates only an evidence-driven successor roadmap; business authorities and Production data remain unchanged. No schema, D1/R2 business-data mutation, provider action, Product publication, Inventory movement or Finance posting is added.</p></section>`;
