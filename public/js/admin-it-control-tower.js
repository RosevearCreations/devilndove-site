// Release 467 Build 231 — current I.T. authority renderer over exact Build 230 GREEN restart boundary.
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 231 I.T. authority diagnostics over exact Build 230 GREEN…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 231</p><h2>Workshop Journal & Capability Case-Study Activation</h2><p class="small">Current read-only I.T. authority. Build 230 is the exact verified Development and Production baseline. Build 231 is a GET-only Workshop Journal and capability case-study activation measurement over existing reviewed Content Release authority.</p><p class="small"><strong>Verified Development:</strong> <code>${esc(v.dev_sha)}</code> / <code>${esc(v.tree_sha)}</code></p><p class="small"><strong>Production GREEN authority:</strong> Build ${esc(p.build)} • Pages ${esc(p.production_pages_deploy_run)} • Live Resources ${esc(p.production_live_resource_integrity_run)} • Product Browser ${esc(p.products_browser_proof_run)} • Remote D1 ${esc(p.remote_d1_queries)}</p><p class="small"><strong>Active Build 231 acceptance:</strong> the measurement is read-only and accepts HOLD_NO_PUBLISHABLE_EVIDENCE when no reviewed publishable evidence exists. Build 232 remains planned; Production promotion remains closed until the exact final Build 231 dev head proves GREEN.</p><p class="small"><strong>Evidence ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">Stripe Development, PayPal sandbox and Social/OAuth remain HOLD_EXTERNAL; CAIP private media remains EVIDENCE_DEPENDENT. Cloudflare Access remains externally governed.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Canonical migrations remain 0001–0022. Build 231 adds no schema migration or publication engine; automatic publication, private/raw CAIP, Inventory, Finance/Accounting, manufacturing-data mutation and provider actions remain closed. Build 232 remains blocked until Build 231 Production GREEN.</p>${(d.truth_notes||[]).map((x)=>`<p class="small">• ${esc(x)}</p>`).join('')}</section>`;
