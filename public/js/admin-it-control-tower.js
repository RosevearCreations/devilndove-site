...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 265 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 265</p><h2>CAIP Private-Media Prerequisite Inventory</h2><p class="small">Current action only. The immutable Build 264 Development/Production GREEN predecessor is recovered by the shared release evidence. Build 265 inventories the existing Build 241 CAIP private-media schema, binding, multipart/recovery and operator prerequisites without contacting Production media.</p><p class="small"><strong>Inventory outcome:</strong> Build 241 records six private-media tables, requires <code>CAIP_PRIVATE_MEDIA_BUCKET</code>, preserves immutable completed raw originals, and retains resumable multipart part/ETag authority. Live private-bucket and interruption/resume acceptance remains external evidence.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 265 does not convert missing private-bucket or recovery proof into acceptance.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 265 is prerequisite inventory only. It changes no schema, D1/R2 business data or media, provider action, Product publication, Inventory movement, Finance posting, workflow deletion or branch protection.</p></section>`;
