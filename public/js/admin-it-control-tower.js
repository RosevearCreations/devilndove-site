...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 257 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 257</p><h2>Workflow Trigger Inventory &amp; Ownership Map</h2><p class="small">Current action only. The immutable Build 256 Development/Production proof is rendered once by the shared evidence summary above. Build 257 inventories the GitHub Actions trigger surface without disabling or deleting any workflow.</p><p class="small"><strong>Inventory outcome:</strong> the pre-build baseline contains 146 workflow YAML files; 140 appear across the primary trigger-family union, including 86 pull-request, 90 push, 89 manual-dispatch and 5 workflow-run chains. Every retained workflow is assigned a current proof owner. Historical broad-trigger coverage moves to Build 258 for bounded scope review.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 257 does not alter provider HOLD/EVIDENCE states.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 257 is a read-only workflow-trigger inventory and ownership map. It disables or deletes no workflow and changes no schema, D1/R2 business data, provider action, Product publication, Inventory movement or Finance posting.</p></section>`;
