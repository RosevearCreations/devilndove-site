// Release 467 Build 217 — current I.T. authority renderer over canonical Build 216 GREEN restart boundary.
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function exportPack(format){
    ...
    a.href=url;a.download=`devilndove-release467-build216-${format==='markdown'?'closure-evidence.md':format+'.json'}`;
    document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);
  }
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 217 I.T. authority diagnostics over exact Build 216 GREEN…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 217</p><h2>Production Cost Evidence v2</h2><p class="small">Current read-only I.T. authority. Build 216 is the exact verified Development and Production baseline; Build 217 extends existing Creative Project source evidence with setup, machine, hands-on labour, consumables, finishing, prototype waste/rework and actual quantity evidence while Inventory and Finance/Accounting ownership remains unchanged.</p><p class="small"><strong>Verified Development:</strong> <code>${esc(v.dev_sha)}</code> / <code>${esc(v.tree_sha)}</code></p><p class="small"><strong>Production GREEN authority:</strong> Build ${esc(p.build)} • Pages ${esc(p.production_pages_deploy_run)} • Live Resources ${esc(p.production_live_resource_integrity_run)} • Product Browser ${esc(p.products_browser_proof_run)} • Remote D1 ${esc(p.remote_d1_queries)}</p><p class="small"><strong>Active Build 217 acceptance:</strong> current pointer, I.T., Deployment Preflight, Reliability, migration 0017 and Build 217 proof must agree on exact Build 216 closure; final Build 217 SHA is resolved only after exact-head proof.</p><p class="small"><strong>Evidence ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">Stripe Development, PayPal sandbox and Social/OAuth remain HOLD_EXTERNAL; CAIP private media remains EVIDENCE_DEPENDENT. Cloudflare Access remains externally governed.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Canonical migrations are 0001–0017. Build 217 adds nullable Production Cost Evidence v2 source facts; unknown cost remains unknown and request-time DDL, automatic Inventory mutation, Finance/Accounting posting, provider/payment and publication actions remain closed.</p>${(d.truth_notes||[]).map((x)=>`<p class="small">• ${esc(x)}</p>`).join('')}</section>`;
