// Release 467 Build 252 — CSP Style Injection-Surface Hardening.
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 252 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 252</p><h2>Release, Diagnostics & Evidence Streamlining</h2><p class="small">Current action only. The immutable Build 251 Development/Production proof is rendered once by the shared evidence summary above; this panel does not restate or reinterpret it.</p><p class="small"><strong>Current candidate:</strong> re-prove phone/tablet/desktop, keyboard/focus, dense-workspace and reduced-motion behavior while retaining the Build 251 CSP boundary.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 252 does not alter provider HOLD/EVIDENCE states.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 252 re-proves accessibility presentation contracts only; business authorities and Production data remain unchanged. No schema, D1/R2 business-data mutation, provider action, Product publication, Inventory movement or Finance posting is added.</p></section>`;
