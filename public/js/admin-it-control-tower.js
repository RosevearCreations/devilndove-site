...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 258 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 258</p><h2>Historical Workflow Trigger Scope Tightening</h2><p class="small">Current action only. The immutable Build 257 Development/Production proof is rendered once by the shared evidence summary above. Build 258 narrows only evidence-backed historical workflow triggers while retaining all historical workflow files and gate scripts.</p><p class="small"><strong>Inventory outcome:</strong> exactly 16 Build 242-257 workflows are manual-only provenance. The candidate contains 148 workflow files with 72 pull-request, 122 push, 123 manual-dispatch and 5 workflow-run triggers, a net reduction of 15 automatic pull-request and 15 automatic push workflows.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 258 does not alter provider HOLD/EVIDENCE states.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 258 changes only historical workflow trigger scope. It deletes no workflow, retains the gate scripts, and changes no schema, D1/R2 business data, provider action, Product publication, Inventory movement or Finance posting.</p></section>`;
