...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 275 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 275</p><h2>Creative Process Planned-vs-Actual Inventory Lifecycle</h2><p class="small">Current action only. The immutable Build 274 Development/Production GREEN predecessor is recovered by the shared release evidence. Build 275 reviews current CAIP production-acceptance evidence without converting historical or static proof into acceptance.</p><p class="small"><strong>Closure outcome:</strong> the current CAIP private-media lane remains EVIDENCE_DEPENDENT; fresh authenticated range evidence and deployed runtime proof remain explicit.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 274 does not convert planned estimates, reviewed-but-unposted actuals, Inventory corrections, provider, publication or Finance gaps into acceptance.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 274 is a read-only release-control projection. The source release clarifies the Creative Process planned-vs-actual lifecycle and routes posted void/correction reversals through Inventory authority, but performs no automatic Inventory movement, Finance posting, R2 deletion, public promotion, provider action, workflow deletion or branch protection.</p></section>`;
