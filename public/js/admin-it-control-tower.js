...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 268 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 268</p><h2>CAIP Private-Media Recovery Hardening Closure</h2><p class="small">Current action only. The immutable Build 267 Development/Production GREEN predecessor is recovered by the shared release evidence. Build 268 closes the static Builds 265–267 recovery prerequisites and preserves the fail-closed pre-transfer boundary for Build 269 without contacting Production media.</p><p class="small"><strong>Closure outcome:</strong> Build 269 must verify Build 241 tables, duplicate-safe columns, the private R2 binding, bounded content fingerprinting and server-side classification before binary transfer; uncertain binaries stay preserved.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 268 does not convert missing private-bucket or live interruption/recovery proof into acceptance.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 268 is a read-only recovery-hardening closure. It changes no schema, D1/R2 business data or media, provider action, Product publication, Inventory movement, Finance posting, workflow deletion or branch protection.</p></section>`;
