...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 262 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 262</p><h2>Operations Today-Tasks Read Fan-Out Review</h2><p class="small">Current action only. The immutable Build 261 Development/Production GREEN proof is rendered once by the shared evidence summary above. Build 262 batches the six latest Today Task action point lookups into one read-only D1 statement.</p><p class="small"><strong>Read-path target:</strong> 13 → 8 top-level Today Tasks statements while retaining the existing Today Tasks ≤15,000, Seller Daily ≤10,000 and aggregate ≤25,000 provider row ceilings. Production D1 measurement remains closed.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 262 does not alter provider HOLD/EVIDENCE states.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 262 changes only the Today Tasks read composition. It keeps the same task/action semantics and provider ceilings, with no schema, D1/R2 business-data mutation, Production D1 measurement, provider action, Product publication, Inventory movement or Finance posting.</p></section>`;
