// Release 467 Build 221 — current I.T. authority renderer over canonical Build 220 GREEN restart boundary.
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function exportPack(format){
    ...
    a.href=url;a.download=`devilndove-release467-build220-${format==='markdown'?'closure-evidence.md':format+'.json'}`;
    document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);
  }
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 221 I.T. authority diagnostics over exact Build 220 GREEN…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 221</p><h2>Workshop Knowledge Library Foundation</h2><p class="small">Current read-only I.T. authority. Build 220 is the exact verified Development and Production baseline; Build 221 adds reviewed source-backed material/tool/process/setting/outcome knowledge without mutating Inventory, Creative Project, media, order or Finance/Accounting authorities.</p><p class="small"><strong>Verified Development:</strong> <code>${esc(v.dev_sha)}</code> / <code>${esc(v.tree_sha)}</code></p><p class="small"><strong>Production GREEN authority:</strong> Build ${esc(p.build)} • Pages ${esc(p.production_pages_deploy_run)} • Live Resources ${esc(p.production_live_resource_integrity_run)} • Product Browser ${esc(p.products_browser_proof_run)} • Remote D1 ${esc(p.remote_d1_queries)}</p><p class="small"><strong>Active Build 221 acceptance:</strong> current pointer, I.T., Deployment Preflight, Reliability and Build 221 proof must agree on exact Build 220 closure; canonical migrations advance through 0001–0020 and the final Build 221 SHA is resolved only after exact-head proof.</p><p class="small"><strong>Evidence ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">Stripe Development, PayPal sandbox and Social/OAuth remain HOLD_EXTERNAL; CAIP private media remains EVIDENCE_DEPENDENT. Cloudflare Access remains externally governed.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Canonical migrations advance through 0001–0020. Build 221 knowledge is source-backed and reference-only; Inventory movements, source media copy, Finance/Accounting posting, provider/payment and publication actions remain closed.</p>${(d.truth_notes||[]).map((x)=>`<p class="small">• ${esc(x)}</p>`).join('')}</section>`;
