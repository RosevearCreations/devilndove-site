...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 264 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 264</p><h2>Release Efficiency &amp; Read-Budget Outcome Verification</h2><p class="small">Current action only. The immutable Build 263 Development/Production GREEN proof is rendered once by the shared evidence summary above. Build 264 closes the measured outcome review of Builds 257–263 and authorizes the evidence-backed Builds 265–275 CAIP recovery/continuity roadmap.</p><p class="small"><strong>Measured outcome:</strong> 584 closure-scoped workflow runs across 12 accepted heads = 48.67 runs/head, 21.69% below the Build 256 baseline. Build 262 closed at 68 runs versus Build 255 at 134. The Today Tasks target remains exactly 8 statements under the existing 15k/10k/25k provider ceilings.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 264 does not alter provider HOLD/EVIDENCE states.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 264 is governance/evidence renewal only. It changes no schema, D1/R2 business data, Production D1 measurement, provider action, Product publication, Inventory movement, Finance posting, workflow deletion or branch protection.</p></section>`;
