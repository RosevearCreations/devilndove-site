...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 272 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 272</p><h2>Upload Prerequisite &amp; Operator Readiness</h2><p class="small">Current action only. The immutable Build 271 Development/Production GREEN predecessor is recovered by the shared release evidence. Build 272 blocks local file selection and binary transfer until the required CAIP tables, duplicate-safe columns and private R2 binding are proven.</p><p class="small"><strong>Closure outcome:</strong> missing prerequisites remain operator/configuration states with transfer_started=false rather than false upload failures.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 272 does not convert missing schema, binding, private-media, provider, publication or downstream Content Studio gaps into acceptance.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 272 is a read-only release-control projection. The source release adds fail-closed readiness gates before local selection and binary transfer, but performs no automatic schema repair, binding mutation, R2 deletion, public promotion, provider action, Inventory movement, Finance posting, workflow deletion or branch protection.</p></section>`;
