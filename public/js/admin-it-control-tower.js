...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 260 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 260</p><h2>Pull-Request Matrix Fan-Out Reduction</h2><p class="small">Current action only. The immutable Build 259 Development/Production proof is rendered once by the shared evidence summary above. Build 260 removes only the redundant pull-request trigger from 38 historical proof workflows whose same contracts remain covered by the canonical System Gate.</p><p class="small"><strong>Inventory outcome:</strong> the Build 259 exact PR head ran 72 workflows. Build 260 retains push/manual evidence while targeting 150 workflow files with 36 pull-request, 124 push, 132 manual-dispatch and 5 workflow-run triggers, with 35 expected actual PR runs.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 260 does not alter provider HOLD/EVIDENCE states.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 260 changes only pull-request trigger scope for evidence-backed historical proof workflows. It retains push/manual evidence and changes no schema, D1/R2 business data, provider action, Product publication, Inventory movement or Finance posting.</p></section>`;
