...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 261 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 261</p><h2>Production Proof Dependency Orchestration</h2><p class="small">Current action only. The immutable Build 260 Development/Production GREEN proof is rendered once by the shared evidence summary above. Build 261 removes only redundant Production <code>main</code>-push subscriptions from 39 historical proof workflows while retaining Development push/manual evidence.</p><p class="small"><strong>Orchestration outcome:</strong> 151 workflow files with 37 pull-request, 125 push, 133 manual-dispatch and 5 workflow-run triggers. Production Pages, Live Resource Integrity, Product Browser and Product Route remain separately visible and exact-SHA bound.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 261 does not alter provider HOLD/EVIDENCE states.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 261 changes only historical Production push orchestration. It retains Development push/manual evidence and all canonical Production proofs, and changes no schema, D1/R2 business data, provider action, Product publication, Inventory movement or Finance posting.</p></section>`;
