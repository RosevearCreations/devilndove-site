...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function load(){
    mount.innerHTML='<section class="card" style="margin-top:18px"><p class="small">Running Release 467 Build 259 diagnostics; immutable verified evidence is shown once above…</p></section>';
    try{
      ...
      mount.innerHTML=`<section class="card" style="margin-top:18px"><p class="eyebrow">Release 467 Build 259</p><h2>Reusable Exact-SHA Proof Composition</h2><p class="small">Current action only. The immutable Build 258 Development/Production proof is rendered once by the shared evidence summary above. Build 259 reuses one read-only exact-SHA proof composition for the current Build 258 and Build 259 proof workflows while retaining every named Development and Production proof.</p><p class="small"><strong>Inventory outcome:</strong> the reusable action requires 5 named Development proofs and 5 named Production proofs at exact requested SHAs. Build 259 adds only its active proof workflow, producing an expected 149 workflow files with 73 pull-request, 123 push, 124 manual-dispatch and 5 workflow-run triggers.</p><p class="small"><strong>Production GREEN authority:</strong> immutable verified evidence is shown once in the shared summary above.</p><p class="small"><strong>Evidence pack ID:</strong> <code>${esc(pack.evidence_id)}</code></p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="itRefresh">Refresh diagnostics</button><button class="btn secondary" id="itVerify">Verify immutable closure artifacts</button><button class="btn secondary" id="itExportMd">Export Markdown</button><button class="btn secondary" id="itExportJson">Export JSON</button><a class="btn secondary" href="/admin/reliability/">Reliability</a><a class="btn secondary" href="/admin/deployment-preflight/">Deployment Preflight</a></div><div id="itVerifyResult" class="small" aria-live="polite" style="margin-top:10px">Cross-artifact verification has not been run in this browser session.</div></section><section class="card" style="margin-top:16px"><h2>External acceptance policy</h2><p class="small">External acceptance remains separately governed; Build 259 does not alter provider HOLD/EVIDENCE states.</p></section><section class="card" style="margin-top:16px"><h2>Safety boundary</h2><p class="small">Build 259 changes only CI proof composition. It remains read-only and changes no schema, D1/R2 business data, provider action, Product publication, Inventory movement or Finance posting.</p></section>`;
