    ...
    mount.innerHTML = `
      <section class="product-media-context-card" id="product-media-workflow" aria-label="Current product media reference">
        <div class="product-media-context-head">
          ...
          ${selected ? `<div class="product-media-context-actions"><a class="btn" href="/admin/product-editor/?product_id=${num(selected.product_id)}&tab=basics">Open product editor</a><a class="btn secondary" href="/admin/release-preflight/?product_id=${num(selected.product_id)}">Release preflight</a><a class="btn secondary" href="/admin/creative-assets/?product_id=${num(selected.product_id)}">Open CAIP</a><a class="btn secondary" href="${esc(publicPreviewUrl(selected))}" target="_blank" rel="noopener">Preview storefront</a>${selected.featured_image_needs_sync ? `<button class="btn secondary" type="button" data-sync-featured-image="${num(selected.product_id)}">Sync resolved featured image</button>` : ''}</div>` : ''}
        </div>
