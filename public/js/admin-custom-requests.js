...
// Brief description: Operations admin panel for reviewing custom requests and converting them into quote/job/product/payment planning records with editable line items, revision history, link lifecycle controls, order-stage tracking, consent review, and accepted-quote follow-through.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderDraftSummary(row, data) {
    ...
    rowsForRequest(data.payment_gates, id).slice(0, 1).forEach((gate) => pills.push(`<span class="status-note small">Payment gate: ${esc(gate.gate_status || 'pending')}</span>`));
    rowsForRequest(data.payment_links, id).forEach((link) => pills.push(`<span class="status-note small">Payment link: ${esc(link.link_status || 'active')} / ${esc(link.external_share_status || 'gate pending')}</span>`));
    rowsForRequest(data.checkout_records, id).forEach((checkout) => pills.push(`<span class="status-note small">Checkout: ${esc(checkout.provider || '')} / ${esc(checkout.checkout_status || '')}</span>`));
    rowsForRequest(data.marketplace_export_packs, id).forEach((pack) => pills.push(`<span class="status-note small">Marketplace pack: ${esc(pack.pack_status || pack.export_status || 'draft')}</span>`));
  ...
  function renderApprovedPaymentLinks(data) {
    ...
    return `<div class="admin-table-wrap"><table><thead><tr><th>Payment link</th><th>Customer</th><th>Amount</th><th>Status</th><th>Private link</th><th>Gate/order</th></tr></thead><tbody>${rows.slice(0, 25).map((row) => {
      const url = row.link_url_path || row.link_path || `/custom-request/pay/?token=${encodeURIComponent(row.link_token || '')}`;
      return `<tr><td><strong>${esc(row.payment_link_key || '')}</strong><br><span class="small">Request #${esc(row.custom_request_id || '')}</span></td><td>${esc(row.customer_name || '')}<br><span class="small">${esc(row.customer_email || '')}</span></td><td>${money(row.amount_cents)}<br><span class="small">Tax ${money(row.tax_cents)}</span></td><td>${esc(row.link_status || '')}<br><span class="small">Share ${esc(row.external_share_status || 'gate pending')} • gate ${esc(row.gate_status || 'pending')}</span><br><span class="small">Provider ${esc(row.preferred_provider || row.provider || 'manual')}</span></td><td><a href="${esc(url)}" target="_blank" rel="noopener">Open payment review</a><br><button class="btn small" type="button" data-copy-payment-link="${esc(url)}">Copy link</button></td><td class="small">Order ID ${esc(row.order_id || '—')}<br>Payment ID ${esc(row.payment_id || '—')}<br>${esc(row.gate_notes || '')}</td></tr>`;
    }).join('')}</tbody></table></div>`;
  }

  function renderPaymentGates(data) {
    const rows = Array.isArray(data.payment_gates) ? data.payment_gates : [];
    if (!rows.length) return '<p class="small">No payment share gates have been run yet.</p>';
    return `<div class="admin-table-wrap"><table><thead><tr><th>Request</th><th>Status</th><th>Order</th><th>Notes</th><th>When</th></tr></thead><tbody>${rows.slice(0, 30).map((row) => `<tr><td>#${esc(row.custom_request_id || '')}</td><td>${esc(row.gate_status || '')}</td><td>Order ${esc(row.order_id || '—')}<br><span class="small">Draft ${esc(row.order_draft_id || '—')}</span></td><td class="small">${esc(row.gate_notes || '')}</td><td class="small">${esc(row.checked_at || '')}</td></tr>`).join('')}</tbody></table></div>`;
  }

  function renderCheckoutRecords(data) {
    const rows = Array.isArray(data.checkout_records) ? data.checkout_records : [];
    if (!rows.length) return '<p class="small">No Stripe/PayPal/Square/manual checkout records have been prepared yet.</p>';
    return `<div class="admin-table-wrap"><table><thead><tr><th>Request</th><th>Provider</th><th>Status</th><th>Order/payment</th><th>Redirect</th></tr></thead><tbody>${rows.slice(0, 30).map((row) => `<tr><td>#${esc(row.custom_request_id || '')}</td><td>${esc(row.provider || '')}<br><span class="small">${esc(row.mode || '')}</span></td><td>${esc(row.checkout_status || '')}<br><span class="small">${esc(row.updated_at || '')}</span></td><td class="small">Order ${esc(row.order_id || '—')}<br>Payment ${esc(row.payment_id || '—')}<br>${esc(row.provider_order_id || row.provider_payment_id || '')}</td><td>${row.redirect_url ? `<a href="${esc(row.redirect_url)}" target="_blank" rel="noopener">Open</a>` : '<span class="small">No redirect</span>'}</td></tr>`).join('')}</tbody></table></div>`;
  }

  function renderOrderStatusLinks(data) {
    const rows = Array.isArray(data.order_status_links) ? data.order_status_links : [];
    if (!rows.length) return '<p class="small">No customer order-status links yet. Convert an order draft to a real order first.</p>';
    return `<div class="admin-table-wrap"><table><thead><tr><th>Request</th><th>Customer</th><th>Status link</th><th>Status</th></tr></thead><tbody>${rows.slice(0, 30).map((row) => { const url = `/custom-request/order/?token=${encodeURIComponent(row.order_status_token || '')}`; return `<tr><td>#${esc(row.custom_request_id || '')}<br><span class="small">Order ${esc(row.order_id || '—')}</span></td><td>${esc(row.customer_name || '')}<br><span class="small">${esc(row.customer_email || '')}</span></td><td><a href="${esc(url)}" target="_blank" rel="noopener">Open order status</a><br><button class="btn small" type="button" data-copy-order-status-link="${esc(url)}">Copy link</button></td><td>${esc(row.link_status || 'active')}<br><span class="small">${esc(row.updated_at || '')}</span></td></tr>`; }).join('')}</tbody></table></div>`;
  }
  ...
  function renderMarketplaceExportPacks(data) {
    ...
    return `<div class="custom-request-actions" style="margin-bottom:10px"><a class="btn small" href="/api/admin/custom-requests?format=marketplace_csv&channel=all">Download all CSV</a><a class="btn small" href="/api/admin/custom-requests?format=marketplace_csv&channel=etsy">Etsy CSV</a><a class="btn small" href="/api/admin/custom-requests?format=marketplace_csv&channel=facebook">Facebook CSV</a><a class="btn small" href="/api/admin/custom-requests?format=marketplace_csv&channel=pinterest">Pinterest CSV</a></div>` + rows.slice(0, 12).map((pack) => `<article class="card custom-request-template-card"><div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;flex-wrap:wrap"><div><strong>${esc(pack.pack_key || '')}</strong><div class="small">Request #${esc(pack.custom_request_id || '')} • ${esc(pack.pack_status || 'draft')} • CSV ${esc(pack.csv_status || 'draft')}</div></div><button class="btn small" type="button" data-copy-marketplace-pack="${esc(pack.custom_request_marketplace_export_pack_id || '')}">Copy all copy</button></div><div class="admin-form-grid" style="margin-top:8px"><label class="small">Etsy<textarea class="input" rows="5" readonly data-marketplace-pack-body="${esc(pack.custom_request_marketplace_export_pack_id || '')}">${esc(pack.etsy_title || '')}

${esc(pack.etsy_description || '')}

Tags: ${esc(pack.tags_json || '')}</textarea></label><label class="small">Facebook<textarea class="input" rows="5" readonly>${esc(pack.facebook_title || '')}
...
${esc(pack.pinterest_description || '')}</textarea></label><label class="small">Manual listing<textarea class="input" rows="5" readonly>${esc(pack.manual_listing_copy || '')}</textarea></label></div></article>`).join('');
  }
  ...
  function renderFulfillmentPrompts(data) {
    ...
    return rows.slice(0, 15).map((prompt) => {
      const consentPath = prompt.prompt_token ? `/custom-request/consent/?token=${encodeURIComponent(prompt.prompt_token)}` : '';
      return `<article class="card custom-request-template-card"><div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;flex-wrap:wrap"><div><strong>${esc(prompt.prompt_key || '')}</strong><div class="small">Request #${esc(prompt.custom_request_id || '')} • ${esc(prompt.prompt_status || 'draft')} • public response ${esc(prompt.public_response_status || 'not_sent')}</div></div><div class="custom-request-actions"><button class="btn small" type="button" data-copy-fulfillment-prompt="${esc(prompt.custom_request_fulfillment_prompt_id || '')}">Copy prompt</button>${consentPath ? `<button class="btn small" type="button" data-copy-consent-link="${esc(consentPath)}">Copy consent link</button>` : ''}</div></div><textarea class="input" rows="8" readonly data-fulfillment-prompt-body="${esc(prompt.custom_request_fulfillment_prompt_id || '')}">${esc(prompt.body_text || '')}

${esc(prompt.consent_question_text || '')}${consentPath ? `

Consent/review response link: ${esc(consentPath)}` : ''}</textarea></article>`;
    }).join('');
  }
  ...
  function renderOrderStages(data) {
    const rows = Array.isArray(data.order_stage_events) ? data.order_stage_events : [];
    if (!rows.length) return '<p class="small">No custom work stage updates yet.</p>';
    return `<div class="admin-table-wrap"><table><thead><tr><th>Request</th><th>Stage</th><th>Notes</th><th>When</th></tr></thead><tbody>${rows.slice(0, 40).map((row) => `<tr><td>#${esc(row.custom_request_id || '')}<br><span class="small">Order ${esc(row.order_id || '—')}</span></td><td>${esc(row.stage_label || row.stage_key || '')}</td><td class="small">${esc(row.stage_notes || '')}</td><td class="small">${esc(row.created_at || '')}</td></tr>`).join('')}</tbody></table></div>`;
  }

  function renderPublicProofCandidates(data) {
    const rows = Array.isArray(data.public_proof_candidates) ? data.public_proof_candidates : [];
    if (!rows.length) return '<p class="small">No consent responses have been converted into public proof candidates yet.</p>';
    return `<div class="admin-table-wrap"><table><thead><tr><th>Request</th><th>Candidate</th><th>Status</th><th>Review notes</th></tr></thead><tbody>${rows.slice(0, 30).map((row) => `<tr><td>#${esc(row.custom_request_id || '')}<br><span class="small">${esc(row.public_use_scope || '')}</span></td><td><strong>${esc(row.title || '')}</strong><br><span class="small">${esc(row.body_text || row.source_review_text || '')}</span></td><td>${esc(row.candidate_status || '')}<br><span class="small">Trust block ${esc(row.trust_block_item_id || '—')}</span></td><td class="small">${esc(row.review_notes || '')}</td></tr>`).join('')}</tbody></table></div>`;
  }

  function renderPaymentProviderTests(data) {
    const rows = Array.isArray(data.payment_provider_tests) ? data.payment_provider_tests : [];
    if (!rows.length) return '<p class="small">No Stripe/PayPal/Square readiness checks have been recorded yet.</p>';
    return `<div class="admin-table-wrap"><table><thead><tr><th>Provider</th><th>Status</th><th>Mode</th><th>Notes</th><th>Checked</th></tr></thead><tbody>${rows.slice(0, 20).map((row) => `<tr><td>${esc(row.provider || '')}</td><td>${esc(row.test_status || '')}</td><td>${esc(row.mode || '')}</td><td class="small">${esc(row.result_notes || '')}</td><td class="small">${esc(row.checked_at || '')}</td></tr>`).join('')}</tbody></table></div>`;
  }
    ...
    result.innerHTML = `
      ...
      <div class="admin-table-wrap" style="margin-top:12px"><table><thead><tr><th>Status</th><th>Request</th><th>Contact</th><th>Details</th><th>Review & convert</th></tr></thead><tbody>
        ${requests.map((row) => `<tr>
          ...
          <td><select data-custom-request-status="${esc(row.custom_request_id)}"><option value="new" ${row.status === 'new' ? 'selected' : ''}>New</option><option value="reviewing" ${row.status === 'reviewing' ? 'selected' : ''}>Reviewing</option><option value="quote_needed" ${row.status === 'quote_needed' ? 'selected' : ''}>Quote needed</option><option value="quoted" ${row.status === 'quoted' ? 'selected' : ''}>Quoted</option><option value="accepted" ${row.status === 'accepted' ? 'selected' : ''}>Accepted</option><option value="declined" ${row.status === 'declined' ? 'selected' : ''}>Declined</option><option value="archived" ${row.status === 'archived' ? 'selected' : ''}>Archived</option></select><textarea data-custom-request-notes="${esc(row.custom_request_id)}" rows="3" placeholder="Admin notes">${esc(row.admin_notes || '')}</textarea><div class="custom-request-actions"><button class="btn small" data-custom-request-save="${esc(row.custom_request_id)}">Save review</button><button class="btn small" data-custom-request-action="create_quote_draft" data-custom-request-id="${esc(row.custom_request_id)}">Quote draft</button><button class="btn small" data-custom-request-action="create_reply_template" data-custom-request-id="${esc(row.custom_request_id)}">Reply template</button><button class="btn small" data-custom-request-action="create_deposit_candidate" data-custom-request-id="${esc(row.custom_request_id)}">Deposit candidate</button><button class="btn small" data-custom-request-action="create_job_draft" data-custom-request-id="${esc(row.custom_request_id)}">Job draft</button><button class="btn small" data-custom-request-action="create_invoice_candidate" data-custom-request-id="${esc(row.custom_request_id)}">Invoice candidate</button><button class="btn small" data-custom-request-action="create_quote_preview_link" data-custom-request-id="${esc(row.custom_request_id)}">Quote preview link</button><button class="btn small" data-custom-request-action="create_quote_revision_link" data-custom-request-id="${esc(row.custom_request_id)}">Revision link</button><button class="btn small" data-custom-request-action="create_accepted_payment_order_drafts" data-custom-request-id="${esc(row.custom_request_id)}">Accepted follow-through</button><button class="btn small" data-custom-request-action="run_payment_share_gate" data-custom-request-id="${esc(row.custom_request_id)}">Run payment gate</button><button class="btn small" data-custom-request-action="approve_payment_link" data-custom-request-id="${esc(row.custom_request_id)}">Approve payment link</button><button class="btn small" data-custom-request-action="convert_order_draft_to_order" data-custom-request-id="${esc(row.custom_request_id)}">Convert order</button><button class="btn small" data-custom-request-action="create_marketplace_export_pack" data-custom-request-id="${esc(row.custom_request_id)}">Marketplace pack</button><button class="btn small" data-custom-request-action="create_post_fulfillment_prompts" data-custom-request-id="${esc(row.custom_request_id)}">Fulfillment prompt</button><button class="btn small" data-custom-request-action="create_product_draft" data-custom-request-id="${esc(row.custom_request_id)}">Product plan</button><button class="btn small" data-custom-request-action="advance_order_stage" data-custom-request-id="${esc(row.custom_request_id)}">Advance stage</button><button class="btn small" data-custom-request-stage-photo="${esc(row.custom_request_id)}">Add stage photo URL</button><button class="btn small" data-custom-request-action="resend_custom_links" data-custom-request-id="${esc(row.custom_request_id)}">Resend links</button><button class="btn small" data-custom-request-action="expire_custom_links" data-custom-request-id="${esc(row.custom_request_id)}">Expire links</button><button class="btn small" data-custom-request-action="void_custom_links" data-custom-request-id="${esc(row.custom_request_id)}">Void links</button><button class="btn small" data-custom-request-action="create_consent_proof_candidate" data-custom-request-id="${esc(row.custom_request_id)}">Consent candidate</button><button class="btn small" data-custom-request-action="approve_consent_public_proof" data-custom-request-id="${esc(row.custom_request_id)}">Approve proof</button><button class="btn small" data-custom-request-action="test_payment_provider_handoffs" data-custom-request-id="${esc(row.custom_request_id)}">Test payment setup</button></div></td>
      ...
      <details style="margin-top:12px" open><summary>Payment share gates</summary>${renderPaymentGates(data)}</details>
      <details style="margin-top:12px" open><summary>Approved payment review links</summary>${renderApprovedPaymentLinks(data)}</details>
      <details style="margin-top:12px" open><summary>Provider checkout records</summary>${renderCheckoutRecords(data)}</details>
      <details style="margin-top:12px" open><summary>Customer order-status links</summary>${renderOrderStatusLinks(data)}</details>
      ...
      <details style="margin-top:12px" open><summary>Custom work stage events</summary>${renderOrderStages(data)}</details>
      <details style="margin-top:12px" open><summary>Consent-to-public-proof candidates</summary>${renderPublicProofCandidates(data)}</details>
      <details style="margin-top:12px" open><summary>Payment provider readiness checks</summary>${renderPaymentProviderTests(data)}</details>
      <details style="margin-top:12px" open><summary>Marketplace channel presets</summary>${renderMarketplacePresets(data)}</details>
  ...
  mount.innerHTML = `<div class="card" style="margin-top:18px"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap"><div><h2 style="margin-top:0">Custom Requests</h2><p class="small" style="margin:0">Review engraving, personalized gift, and workshop-made commission requests before turning them into quotes, replies, deposits, jobs, invoices, approved payment review links, payment share gates, provider checkout records, marketplace CSV exports, link controls, order-stage updates, consent proof candidates, or product draft plans.</p></div><button class="btn" type="button" id="customRequestsLoadButton">Refresh requests</button></div><div id="customRequestsAdminMessage" class="small" style="display:none;margin-top:10px"></div><div id="customRequestsAdminRows"></div></div>`;
  ...
  mount.addEventListener('click', async (event) => {
    ...
    const stagePhotoButton = event.target.closest('[data-custom-request-stage-photo]');
    if (stagePhotoButton) addStagePhoto(stagePhotoButton.getAttribute('data-custom-request-stage-photo'));
    ...
    const orderStatusCopyButton = event.target.closest('[data-copy-order-status-link]');
    if (orderStatusCopyButton) {
      const path = orderStatusCopyButton.getAttribute('data-copy-order-status-link') || '';
      const link = `${window.location.origin}${path.startsWith('/') ? path : `/${path}`}`;
      try { await navigator.clipboard.writeText(link); setMsg('Order-status link copied.'); } catch { setMsg(link, false); }
    }
    const consentCopyButton = event.target.closest('[data-copy-consent-link]');
    if (consentCopyButton) {
      const path = consentCopyButton.getAttribute('data-copy-consent-link') || '';
      const link = `${window.location.origin}${path.startsWith('/') ? path : `/${path}`}`;
      try { await navigator.clipboard.writeText(link); setMsg('Consent response link copied.'); } catch { setMsg(link, false); }
    }
