...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function bulkApplyRoleOrder() {
    const response = await window.DDAuth.apiFetch('/api/admin/marketplace-export-preview', { method: 'POST', body: JSON.stringify({ channel: currentChannel, action: 'bulk_apply_role_order' }) });
    const data = await response.json().catch(() => null);
    if (!response.ok || !data?.ok) throw new Error(data?.error || 'Bulk apply failed.');
