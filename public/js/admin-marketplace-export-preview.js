...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function load(channel = currentChannel) {
    ...
    try {
      ...
      const historyButtons = (data.history || []).slice(0,5).map((item) => `<button class="btn small" data-replay-history="${esc(item.marketplace_export_history_id)}" type="button">Replay ${esc(item.created_at || 'history')}</button><button class="btn small" data-diff-history="${esc(item.marketplace_export_history_id)}" type="button">Diff</button><button class="btn small secondary" data-rollback-channel-history="${esc(item.marketplace_export_history_id)}" type="button">Rollback channel</button>`).join('');
      mount.innerHTML = `<div style="display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap"><div><h2 style="margin-top:0">${esc(rule.label || channel)} preview</h2><p class="small">Ready ${esc(data.summary?.ready || 0)} / ${esc(data.summary?.total || 0)} • Blocked ${esc(data.summary?.blocked || 0)} • Selected ${esc(data.summary?.selected_products || 0)} product(s)</p></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" data-channel="etsy">Etsy</button><button class="btn" data-channel="facebook">Facebook</button><button class="btn" data-channel="pinterest">Pinterest</button><button class="btn" data-channel="manual">Manual</button><a class="btn" href="/admin/marketplace-mapping/">Mapping editor</a><button class="btn" id="bulkApplyMarketplaceImages" type="button">Bulk apply role order</button><button class="btn secondary" id="validateMarketplaceRows" type="button">Validate real export rows</button><button class="btn secondary" id="clearMarketplaceChannel" type="button">Clear channel selections</button><a class="btn primary" href="/api/admin/marketplace-export-preview?channel=${encodeURIComponent(channel)}&format=csv">Download CSV</a></div></div><div class="status-note info small">Select exactly the public-ready images you want each marketplace export to use, then save selection before downloading CSV.</div><div class="marketplace-history small"><strong>Recent ${esc(channel)} exports:</strong> ${historyButtons || 'No export history yet.'}</div><div class="marketplace-preview-grid">${rows.map(rowCard).join('')}</div>`;
      ...
      mount.querySelectorAll('[data-diff-history]').forEach((button) => button.addEventListener('click', () => showHistoryDiff(button.getAttribute('data-diff-history'))));
      mount.querySelectorAll('[data-rollback-channel-history]').forEach((button) => button.addEventListener('click', () => rollbackChannel(button.getAttribute('data-rollback-channel-history'))));
      document.getElementById('bulkApplyMarketplaceImages')?.addEventListener('click', bulkApplyRoleOrder);
      document.getElementById('validateMarketplaceRows')?.addEventListener('click', validateRows);
      document.getElementById('clearMarketplaceChannel')?.addEventListener('click', () => rollbackChannel(0));
  ...
  async function validateRows() { const data = await read(await window.DDAuth.apiFetch('/api/admin/marketplace-export-preview', { method: 'POST', body: JSON.stringify({ channel: currentChannel, action: 'validate_export_rows' }) }), 'Marketplace row validation failed.'); alert(`${data.summary?.passed || 0} passed, ${data.summary?.blocked || 0} blocked real export row(s).`); await load(currentChannel); }
  ...
  async function showHistoryDiff(historyId) { const data = await read(await window.DDAuth.apiFetch(`/api/admin/marketplace-export-preview?channel=${encodeURIComponent(currentChannel)}&diff_history_id=${encodeURIComponent(historyId || '')}`), 'History diff failed.'); const changed = (data.diff || []).filter((row) => row.changed); alert(changed.length ? changed.map((row) => `Product ${row.product_id}: +${row.added_urls.length} / -${row.removed_urls.length}`).join('\n') : 'No differences from this snapshot.'); }
  async function rollbackChannel(historyId) { await read(await window.DDAuth.apiFetch('/api/admin/marketplace-export-preview', { method: 'POST', body: JSON.stringify({ channel: currentChannel, action: 'rollback_channel_export', marketplace_export_history_id: Number(historyId || 0) }) }), 'Channel rollback failed.'); await load(currentChannel); }
