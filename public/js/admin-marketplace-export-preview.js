...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function load(channel = currentChannel) {
    ...
    try {
      ...
      mount.innerHTML = `<div style="display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap"><div><h2 style="margin-top:0">${esc(rule.label || channel)} preview</h2><p class="small">Ready ${esc(data.summary?.ready || 0)} / ${esc(data.summary?.total || 0)} • Blocked ${esc(data.summary?.blocked || 0)} • Selected ${esc(data.summary?.selected_products || 0)} product(s)</p></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" data-channel="etsy">Etsy</button><button class="btn" data-channel="facebook">Facebook</button><button class="btn" data-channel="pinterest">Pinterest</button><button class="btn" data-channel="manual">Manual</button><a class="btn" href="/admin/marketplace-mapping/">Mapping editor</a><button class="btn" id="bulkApplyMarketplaceImages" type="button">Bulk apply role order</button><button class="btn secondary" id="clearMarketplaceChannel" type="button">Clear channel selections</button><a class="btn primary" href="/api/admin/marketplace-export-preview?channel=${encodeURIComponent(channel)}&format=csv">Download CSV</a></div></div><div class="status-note info small">Select exactly the public-ready images you want each marketplace export to use, then save selection before downloading CSV.</div><div class="marketplace-history small"><strong>Recent ${esc(channel)} exports:</strong> ${historyButtons || 'No export history yet.'}</div><div class="marketplace-preview-grid">${rows.map(rowCard).join('')}</div>`;
