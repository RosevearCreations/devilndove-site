...
document.addEventListener('DOMContentLoaded', () => {
    ...
    mountEl.innerHTML = `
      <div class="card" style="margin-top:18px">
        ...
        <p class="small" style="margin-top:0">
          Visually link the tools and supplies used to create a finished product.
          This stores a reusable making-story in D1 so each finished piece can explain what materials and tools shaped it.
        </p>
  ...
  function renderResources() {
    ...
    el.innerHTML = state.resources.map((item) => {
      ...
      return `
        <button
          type="button"
          class="resource-tile ${linked ? 'is-linked' : ''}"
          data-add-resource="1"
          data-kind="${escapeHtml(item.item_kind)}"
          data-key="${escapeHtml(item.source_key)}"
        >
          <div class="resource-tile-media">
            ${item.image_url
              ? `<img src="${escapeHtml(item.image_url)}" alt="${escapeHtml(item.name)}" loading="lazy" />`
              : `<div class="resource-tile-placeholder">${escapeHtml(item.item_kind)}</div>`}
          </div>
          <div class="resource-tile-body">
            ...
            <div class="small">
              On hand ${Number(item.on_hand_quantity || 0)}
              ${Number(item.is_on_reorder_list || 0) === 1 ? ' • reorder list' : ''}
              ${Number(item.do_not_reuse || 0) === 1 ? ' • do not reuse' : ''}
            </div>
          </div>
        </button>
      `;
    ...
    el.innerHTML = state.links.map((link, idx) => `
      <div class="resource-linked-card">
        <div class="resource-linked-summary">
          ...
          <div class="small">${escapeHtml(link.resource_kind)}</div>
          <textarea
            class="input"
            data-link-note="${idx}"
            rows="2"
            placeholder="How was this item used for the story of this product?"
          >${escapeHtml(link.usage_notes || '')}</textarea>
        </div>
        <div class="resource-linked-actions">
          <input type="number" min="1" step="1" value="${Number(link.quantity_used || 1)}" data-link-qty="${idx}" />
  ...
  async function loadData() {
    try {
      ...
      state.links = links.map((x) => ({
        ...x,
        name: (state.resources.find((r) => r.item_kind === x.resource_kind && r.source_key === x.source_key) || {}).name || x.source_key
      }));
  ...
  async function saveLinks() {
    ...
    document.querySelectorAll('[data-link-qty]').forEach((input) => {
      const idx = Number(input.getAttribute('data-link-qty') || -1);
      if (idx >= 0 && state.links[idx]) {
        state.links[idx].quantity_used = Math.max(1, Number(input.value || 1) || 1);
      }
    });

    document.querySelectorAll('[data-link-note]').forEach((input) => {
      const idx = Number(input.getAttribute('data-link-note') || -1);
      if (idx >= 0 && state.links[idx]) {
        state.links[idx].usage_notes = String(input.value || '').trim();
      }
    });
