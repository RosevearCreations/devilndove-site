...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderLinks() {
    ...
    el.innerHTML = state.links.map((link, idx)=>`<div class="resource-linked-card"><div class="resource-linked-summary"><strong>${escapeHtml(link.name || link.source_key)}</strong><div class="small">${escapeHtml(link.resource_kind)}</div><div class="grid cols-2" style="gap:8px;margin:8px 0"><label class="small">Qty used<input class="input" type="number" min="1" step="1" value="${Number(link.quantity_used || 1)}" data-link-qty="${idx}" /></label><label class="small">Inventory use<select class="input" data-link-mode="${idx}"><option value="per_unit" ${String(link.consumption_mode || 'per_unit') === 'per_unit' ? 'selected' : ''}>Per product</option><option value="end_of_lot" ${String(link.consumption_mode || 'per_unit') === 'end_of_lot' ? 'selected' : ''}>End of lot</option><option value="story_only" ${String(link.consumption_mode || 'per_unit') === 'story_only' ? 'selected' : ''}>Story only</option></select></label></div><label class="small" style="display:${String(link.consumption_mode || 'per_unit') === 'end_of_lot' ? 'block' : 'none'}">Products per lot/container<input class="input" type="number" min="1" step="1" value="${Math.max(1, Number(link.lot_size_units || 1) || 1)}" data-link-lot="${idx}" /></label><textarea class="input" data-link-note="${idx}" rows="2" placeholder="How was this item used for the story of this product?">${escapeHtml(link.usage_notes || '')}</textarea></div><div class="resource-linked-actions"><button class="btn" type="button" data-remove-link="${idx}">Remove</button></div></div>`).join('');
  }
  async function loadData() {
    try {
      ...
      state.links = links.map((x) => ({ ...x, consumption_mode: x.consumption_mode || 'per_unit', lot_size_units: Math.max(1, Number(x.lot_size_units || 1) || 1), name: (state.resources.find((r) => r.item_kind === x.resource_kind && r.source_key === x.source_key) || {}).name || x.source_key }));
  ...
  function onClick(event) {
    ...
    if (add) {
      ...
      if (!state.links.find((x) => x.resource_kind === kind && x.source_key === key)) state.links.push({ resource_kind: kind, source_key: key, quantity_used: 1, usage_notes: '', consumption_mode: 'per_unit', lot_size_units: 1, sort_order: state.links.length, name: item.name });
  ...
  async function saveLinks() {
    ...
    document.querySelectorAll('[data-link-mode]').forEach((input) => { const idx = Number(input.getAttribute('data-link-mode') || -1); if (idx >= 0 && state.links[idx]) state.links[idx].consumption_mode = String(input.value || 'per_unit').trim() || 'per_unit'; });
    document.querySelectorAll('[data-link-lot]').forEach((input) => { const idx = Number(input.getAttribute('data-link-lot') || -1); if (idx >= 0 && state.links[idx]) state.links[idx].lot_size_units = Math.max(1, Number(input.value || 1) || 1); });
    ...
    try { setMessage('Saving product links...'); const response = await window.DDAuth.apiFetch('/api/admin/product-resources', { method:'POST', body: JSON.stringify({ product_id: state.selectedProductId, links: state.links.map((x,i)=>({ resource_kind:x.resource_kind, source_key:x.source_key, quantity_used:x.quantity_used, usage_notes:x.usage_notes || '', consumption_mode:x.consumption_mode || 'per_unit', lot_size_units:Math.max(1, Number(x.lot_size_units || 1) || 1), sort_order:i })) }) }); const data = await response.json(); if (!response.ok || !data?.ok) throw new Error(data?.error || 'Failed to save product links.'); setMessage(`Saved ${Number(data.saved_links || 0)} linked items.`); await loadData(); } catch (err) { setMessage(err.message || 'Failed to save product links.', true); }
  }
