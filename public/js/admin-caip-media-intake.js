// Build 267 — CAIP private-media reconciliation, idempotent registration retry, and safe duplicate cleanup.
(() => {
  ...
  function fileRow(file) {
    ...
    return `<article class="caip-upload-file" data-file-row="${num(file.caip_media_upload_file_id)}">
      ...
      <div class="caip-media-actions">
        ...
        ${uploadedFile&&!file.creative_asset_id?`<button class="btn primary" data-retry-registration="${num(file.caip_media_upload_file_id)}" ${disabled}>Retry CAIP registration</button>`:''}${uploadedFile?`<button class="btn" data-save-governance="${num(file.caip_media_upload_file_id)}" ${disabled}>Review privacy / rights</button><button class="btn secondary" data-promotion-file="${num(file.caip_media_upload_file_id)}" ${disabled}>Request public promotion</button>`:''}
      </div>
  ...
  function duplicateAuditHtml(audit){
    const groups=audit?.groups||[];
    if(!groups.length)return `<div class="content-empty-state">No duplicate CAIP intake fingerprints are currently active for this project.</div>`;
    return groups.map((group)=>{const rows=group.rows||[];const canonical=num(group.canonical_file_id);const duplicates=(group.duplicate_file_ids||[]).map(num).filter(Boolean);const safe=(group.reclaimable_file_ids||[]).map(num).filter(Boolean);return `<article class="caip-duplicate-group"><div class="section-title-row"><div><strong>${esc(rows[0]?.original_filename||'Duplicate media')}</strong><p class="small">${fmtBytes(group.file_size_bytes)} • ${rows.length} intake rows • canonical upload #${canonical}</p></div><span class="status-pill">${duplicates.length} duplicate${duplicates.length===1?'':'s'}</span></div><div class="caip-duplicate-rows">${rows.map((row)=>`<div><span>#${num(row.caip_media_upload_file_id)} • ${esc(human(row.upload_status))}${row.creative_asset_id?` • CAIP asset #${num(row.creative_asset_id)}`:''}</span>${num(row.caip_media_upload_file_id)===canonical?'<strong>Keep</strong>':'<span>Redundant candidate</span>'}</div>`).join('')}</div><div class="caip-media-actions"><button class="btn" data-archive-duplicates="${canonical}" data-duplicate-ids="${esc(duplicates.join(','))}" type="button">Archive redundant rows</button>${safe.length?`<button class="btn danger" data-delete-safe-duplicates="${canonical}" data-duplicate-ids="${esc(safe.join(','))}" type="button">Archive + delete ${safe.length} safe R2 duplicate${safe.length===1?'':'s'}</button>`:''}</div><p class="small">Archiving hides redundant recovery rows but keeps audit history. Physical R2 deletion is offered only when both copies also have the same verified content checksum and the redundant row has no linked CAIP asset or downstream references.</p></article>`;}).join('');
  }

  function render() {
    const data=state.data||{}; const projects=data.projects||[]; const project=projectId()||num(data.selected_project_id); const files=(data.files||[]).filter((f)=>num(f.creative_project_id)===project); const sessions=(data.sessions||[]).filter((s)=>num(s.creative_project_id)===project); const jobs=(data.processing_jobs||[]).filter((j)=>num(j.creative_project_id)===project); const promotions=(data.promotion_requests||[]).filter((p)=>num(p.creative_project_id)===project); const binding=data.binding||{}; const duplicateAudit=data.duplicate_audit||{groups:[],duplicate_rows:0,reclaimable_rows:0};
    mount.innerHTML = `<section class="card caip-media-intake-card">
      <div class="section-title-row"><div><p class="eyebrow">Build 267 • Devil n Dove CAIP private media</p><h2>Private large media intake</h2><p class="small">Raw project photos, videos and narration live in private R2. D1 stores metadata and resumable state. Completed originals become internal CAIP assets; public copies require a separate rights/consent review.</p></div><span class="status-pill ${binding.private_bucket_available?'status-approved':'status-missing'}">${binding.private_bucket_available?'Private R2 ready':'Private R2 binding needed'}</span></div>
  ...
  archive</pre><p class="small">The current build implements private <code>raw/</code> multipart intake plus D1 processing plans. Proxy/extracted/derived/export providers remain explicit future adapters and cannot show false completion.</p></aside>
    ...
    <section class="card caip-media-cleanup-card"><div class="section-title-row"><div><h2>CAIP media audit & duplicate cleanup</h2><p class="small">Compares project intake rows by the stored intake fingerprint and file size to find probable duplicates. Nothing is deleted automatically. Keep the best canonical upload, archive duplicate D1 recovery rows, and only delete a redundant private R2 object when it has no linked CAIP asset or downstream references.</p></div><span class="status-pill">${num(duplicateAudit.duplicate_rows)} duplicate rows</span></div><div class="caip-media-actions"><button class="btn" id="caipAuditDuplicates" type="button">Refresh duplicate audit</button></div><div class="caip-duplicate-audit">${duplicateAuditHtml(duplicateAudit)}</div></section>
  ...
  async function uploadFile(serverFile,file){
    ...
    if(serverFile.upload_status==='uploaded'){ return; }
    state.paused.delete(fileId); const initiated=await initiate(fileId); if(initiated.file?.upload_status==='uploaded'){return;} if(initiated.direct_upload){const response=await DDAuth.apiFetch(`/api/admin/caip-media-upload-direct?file_id=${encodeURIComponent(fileId)}`,{method:'PUT',headers:{'Content-Type':file.type||'application/octet-stream'},body:file});const result=await response.json().catch(()=>null);if(!response.ok||!result?.ok)throw new Error(`${result?.error||'Direct private-R2 upload failed.'}${result?.error_code?` [${result.error_code}]`:''}`);if(result.registration_pending)setMsg(`${file.name} is safely stored in private R2. CAIP metadata registration is pending and can be retried without re-uploading the file.`,true);await refresh();return;} const parts=(state.data?.parts||[]).filter((p)=>num(p.caip_media_upload_file_id)===fileId).sort((a,b)=>num(a.part_number)-num(b.part_number)); const queue=parts.filter((p)=>p.part_status!=='uploaded');
  ...
  async function retryRegistration(fileId){try{const data=await api({action:'retry_registration',caip_media_upload_file_id:fileId});state.data=data;render();const pending=Boolean(data?.result?.registration_pending);const refreshWarning=data?.refresh_warning?` Screen refresh warning: ${data.refresh_warning}`:'';setMsg((pending?(data?.result?.registration_warning||'The binary is safe in R2, but CAIP registration still needs attention.'):'CAIP registration completed from the existing private R2 file. No binary was re-uploaded.')+refreshWarning,pending||Boolean(data?.refresh_warning));}catch(error){setMsg(error.message,true);}}
  async function auditDuplicates(){try{const data=await api({action:'audit_duplicates'});state.data=data;render();setMsg('Duplicate audit refreshed. No files were changed.',false);}catch(error){setMsg(error.message,true);}}
  async function cleanupDuplicates(canonical,ids,deleteR2=false){const list=String(ids||'').split(',').map(num).filter(Boolean);if(!canonical||!list.length)return;const warning=deleteR2?'This will archive the redundant CAIP rows and permanently delete only the safe redundant private R2 copies shown for this group. The canonical file is kept. Continue?':'This will archive the redundant recovery rows but retain every private R2 object. Continue?';if(!confirm(warning))return;try{const data=await api({action:'cleanup_duplicate_group',canonical_file_id:num(canonical),duplicate_file_ids:list,delete_private_r2_copy:Boolean(deleteR2)});state.data=data;render();setMsg(deleteR2?`Duplicate cleanup completed. ${num(data?.result?.archived_count)} row(s) archived; ${num(data?.result?.r2_deleted_count)} redundant private R2 object(s) deleted.`:`${num(data?.result?.archived_count)} duplicate row(s) archived. Private R2 objects were retained.`,false);}catch(error){setMsg(error.message,true);}}
  ...
  function bind(){
    ...
    document.getElementById('caipAuditDuplicates')?.addEventListener('click',auditDuplicates);document.querySelectorAll('[data-archive-duplicates]').forEach((b)=>b.onclick=()=>cleanupDuplicates(b.dataset.archiveDuplicates,b.dataset.duplicateIds,false));document.querySelectorAll('[data-delete-safe-duplicates]').forEach((b)=>b.onclick=()=>cleanupDuplicates(b.dataset.deleteSafeDuplicates,b.dataset.duplicateIds,true));
    document.querySelectorAll('[data-resume-file]').forEach((b)=>b.onclick=()=>pickForExisting(b.dataset.resumeFile));document.querySelectorAll('[data-retry-registration]').forEach((b)=>b.onclick=()=>retryRegistration(b.dataset.retryRegistration));document.querySelectorAll('[data-pause-file]').forEach((b)=>b.onclick=()=>{state.paused.add(num(b.dataset.pauseFile));setMsg('Upload will pause after currently active parts finish. Completed parts remain in R2.',false);});document.querySelectorAll('[data-abort-file]').forEach((b)=>b.onclick=()=>abortFile(b.dataset.abortFile));document.querySelectorAll('[data-save-governance]').forEach((b)=>b.onclick=()=>saveGovernance(b.dataset.saveGovernance));document.querySelectorAll('[data-promotion-file]').forEach((b)=>b.onclick=()=>requestPromotion(b.dataset.promotionFile));
  }
