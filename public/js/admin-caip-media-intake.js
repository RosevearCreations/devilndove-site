// Build 266 — CAIP private-media upload diagnostics, productless project safety and broader media intake.
(() => {
  ...
  async function retryRegistration(fileId){try{state.data=await api({action:'retry_registration',caip_media_upload_file_id:fileId});render();const pending=Boolean(state.data?.result?.registration_pending);setMsg(pending?(state.data?.result?.registration_warning||'The binary is safe in R2, but CAIP registration still needs attention.'):'CAIP registration completed from the existing private R2 file. No binary was re-uploaded.',pending);}catch(error){setMsg(error.message,true);}}
