document.addEventListener('DOMContentLoaded', () => {
  ...
  async function onClick(event) {
    const actionMap = [
      ...
      ['[data-gift-card-action]', async (el) => { const action = el.getAttribute('data-gift-card-action') || ''; const gift_card_id = Number(el.getAttribute('data-gift-card-id') || 0); const note = window.prompt(`Note for ${action}:`, '') || ''; const amount_cents = action === 'refund' ? Math.round(Number(window.prompt('Refund/reduce amount in cents (blank = remaining balance):', '') || 0)) : 0; const response = await window.DDAuth.apiFetch('/api/admin/gift-card-actions', { method: 'POST', body: JSON.stringify({ action, gift_card_id, note, amount_cents }) }); const data = await response.json().catch(() => null); if (!response.ok || !data?.ok) throw new Error(data?.error || 'Gift card action failed.'); setMessage(data.message || 'Gift card action saved.'); await load(); }],
