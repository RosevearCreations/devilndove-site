document.addEventListener('DOMContentLoaded', () => {
  ...
  let rendered = false;
  const filterState = { search: '', openOnly: false, giftCardSearch: '', giftCardStatus: 'all', notificationStatus: 'all' };
    ...
    mountEl.innerHTML = `
      <div class="card" style="margin-top:18px">
        ...
        <div class="card" style="margin-top:12px">
          ...
          <div class="grid cols-3" style="gap:10px;margin-bottom:10px"><input id="giftCardBoardSearch" type="search" placeholder="Search purchaser, recipient, email, code" /><select id="giftCardBoardStatus"><option value="all">All gift cards</option><option value="active">Active only</option><option value="inactive">Inactive only</option><option value="expired">Expired only</option><option value="pending_activation">Pending activation</option></select><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" type="button" id="activateSelectedGiftCardsButton">Activate selected</button><button class="btn" type="button" id="deactivateSelectedGiftCardsButton">Deactivate selected</button></div></div>
  ...
  function renderLists(data) {
    ...
    renderList('customerEngagementGiftCards', filterGiftCards(data.gift_cards || []).map((row) => `<div class="card" style="margin-bottom:10px"><label style="display:flex;gap:10px;align-items:flex-start"><input type="checkbox" name="gift-card-select" value="${esc(row.gift_card_id)}"/><span><strong>${esc(row.code || 'Gift card')}</strong><div class="small">${esc(money(row.remaining_amount_cents || 0, row.currency || 'CAD'))} remaining • ${esc(row.status || 'active')}</div><div class="small">Purchaser: ${esc(row.purchaser_name || row.purchaser_email || '—')}</div><div class="small">Recipient: ${esc(row.recipient_name || row.recipient_email || '—')}</div><div class="small">Recipient sends ${esc(String(row.recipient_send_count || 0))}${row.recipient_last_sent_at ? ` • last ${esc(fmtDate(row.recipient_last_sent_at))}` : ''}</div><div class="small">Purchaser confirmations ${esc(String(row.purchaser_send_count || 0))}${row.purchaser_last_sent_at ? ` • last ${esc(fmtDate(row.purchaser_last_sent_at))}` : ''}</div>${row.recipient_note ? `<div class="small">Message: ${esc(row.recipient_note)}</div>` : ''}</span></label><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px"><button class="btn" type="button" data-copy-gift-card-code="${esc(row.code)}">Copy code</button><button class="btn" type="button" data-resend-gift-card="${esc(row.gift_card_id)}">Resend</button><button class="btn" type="button" data-gift-card-status="${esc(row.gift_card_id)}" data-status-value="active">Activate</button><button class="btn" type="button" data-gift-card-status="${esc(row.gift_card_id)}" data-status-value="inactive">Deactivate</button></div></div>`).join(''), 'No gift cards issued yet.');
