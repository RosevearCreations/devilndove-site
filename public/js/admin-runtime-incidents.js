...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function ownerFor(row){
    const hay=`${row?.incident_scope||''} ${row?.incident_code||''} ${row?.endpoint_path||''} ${row?.message||''}`.toLowerCase();
    if(/media|upload|image|photo/.test(hay))return {area:'Media / images',href:'/admin/media-content-studio/'};
    if(/product|catalog|storefront|shop/.test(hay))return {area:'Products / Storefront',href:'/admin/products/'};
    if(/inventory|supply|tool|stock|reorder/.test(hay))return {area:'Inventory / Supplies / Tools',href:'/admin/inventory-operations/'};
    if(/order|fulfill|cart|checkout/.test(hay))return {area:'Orders / Fulfilment',href:'/admin/orders/'};
    if(/payment|stripe|paypal|provider|webhook/.test(hay))return {area:'Payments / Integrations',href:'/admin/it-integrations/'};
    if(/account|finance|ledger|reconcil/.test(hay))return {area:'Finance / Accounting',href:'/admin/accounting/'};
    if(/auth|login|session|user|member|password/.test(hay))return {area:'Users / Access',href:'/admin/users/'};
    return {area:'I.T. / Operations',href:'/admin/it/'};
  }
  function breachLabel(key,count){
    const labels={
      open_critical:'Open incidents already marked critical',
      payment_provider_failures_24h:'Payment/provider failures in the past 24 hours',
      notification_failures_24h:'Notification failures in the past 24 hours',
      upload_failures_24h:'Upload/media failures in the past 24 hours',
      stale_open_72h:'Incidents still open after 72 hours',
      stale_open_24h:'Incidents still open after 24 hours'
    };
  ...
  function render(data) {
    ...
    results.innerHTML = `
      <section aria-labelledby="todayNeedsAttentionHeading"><h3 id="todayNeedsAttentionHeading">Today Needs Attention</h3>
        ...
        <div class="card" style="margin-top:10px" data-context-help="today_attention"><strong>Attention thresholds</strong><ul class="small">${breaches.map((b)=>`<li>${pill(b.level)} ${esc(breachLabel(b.key,b.count))}</li>`).join('') || '<li>No operational threshold is currently breached.</li>'}</ul></div>
        <div class="runtime-attention-grid" style="margin-top:10px">${attentionCards(incidents) || '<div class="card small">No matching open incident records in this view.</div>'}</div>
      </section>
      <details open style="margin-top:12px" data-context-help="recurring_incidents"><summary><strong>Grouped recurring incidents</strong></summary><div class="admin-table-wrap" style="margin-top:10px"><table><thead><tr><th>Count</th><th>Severity</th><th>What</th><th>Where</th><th>Owner</th><th>Last seen</th></tr></thead><tbody>${groups.map((row)=>{const owner=ownerFor(row);return `<tr><td><strong>${esc(Number(row.incident_count||0))}</strong></td><td>${pill(row.severity)}</td><td><strong>${esc(row.incident_scope)}</strong><br><span class="small">${esc(row.incident_code)}</span></td><td><code>${esc(row.endpoint_path||'unknown endpoint')}</code></td><td><a class="btn small" href="${esc(owner.href)}">${esc(owner.area)}</a></td><td>${esc(row.last_seen_at)}</td></tr>`;}).join('') || '<tr><td colspan="6">No matching grouped incidents.</td></tr>'}</tbody></table></div></details>
      <details open style="margin-top:12px" data-context-help="runtime_incident"><summary><strong>Recent incident records and safe recovery</strong></summary>
    ...
    document.dispatchEvent(new CustomEvent('dd:admin-context-help-refresh'));
  }
  async function load() {
  ...
  mount.innerHTML = `<div class="card" style="margin-top:18px" aria-labelledby="runtimeIncidentsHeading"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap"><div><h2 id="runtimeIncidentsHeading" data-context-help="today_attention" style="margin-top:0">Today Needs Attention</h2><p class="small">Operational thresholds, recurring runtime incidents, audited safe recovery, and archive-before-delete retention control.</p></div><button class="btn primary" id="refreshRuntimeIncidentsButton" type="button">Refresh attention</button></div><div class="grid cols-4" style="gap:12px;margin-top:12px"><label class="small">Days <input class="input" id="runtimeIncidentDays" type="number" min="1" max="90" value="7"></label><label class="small">Limit <input class="input" id="runtimeIncidentLimit" type="number" min="1" max="100" value="50"></label><label class="small" data-context-help="incident_severity">Severity <select class="input" id="runtimeIncidentSeverity"><option value="">All</option><option>critical</option><option>error</option><option>warning</option><option>info</option></select></label><label class="small" data-context-help="incident_review_status">Review status <select class="input" id="runtimeIncidentReviewStatus"><option value="open">Open</option><option value="reviewing">Reviewing</option><option value="resolved">Resolved</option><option value="ignored">Ignored</option><option value="all">All</option></select></label></div><p id="runtimeIncidentsMessage" class="small" hidden aria-live="polite"></p><div id="runtimeIncidentsResults"></div></div>`;
