...
// and Build 83 add bounded Packaging intelligence/composition/production/release-workflow
// layers without replacing it.
(() => {
  const BUILD = 301;
  ...
  function snapshot() {
    const gate=safeStatus(globalThis.DDPackagingStartupGate), contracts=safeStatus(globalThis.DDPackagingContracts), client=safeStatus(globalThis.DDPackagingClient), stabilizer=safeStatus(globalThis.DDPackagingSaveStabilizer), material=safeStatus(globalThis.DDPackagingMaterialIntelligence), composition=safeStatus(globalThis.DDPackagingLabelComposition), production=safeStatus(globalThis.DDPackagingLabelProduction), releaseWorkflow=safeStatus(globalThis.DDPackagingReleaseWorkflow);
    ...
    return Object.freeze({
      ...
      releaseWorkflowBuild:Number(releaseWorkflow?.build||0)||null,releaseWorkflowActive:Boolean(releaseWorkflow?.state==='active'),releaseWorkflowOverallStatus:releaseWorkflow?.overallStatus||null,releaseWorkflowFailures:Number(releaseWorkflow?.failureCount||0),
  ...
  function loadReleaseWorkflow(){if(typeof document==='undefined'||!String(globalThis.location?.pathname||'').includes('/admin/packaging-studio'))return;addScript('script[data-dd-packaging-release-workflow]','/public/js/admin-packaging-release-workflow-v83.js?v=46783','ddPackagingReleaseWorkflow',RELEASE_WORKFLOW_BUILD);}
