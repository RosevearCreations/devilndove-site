# Devil n Dove Project Status and Roadmap — Build 273

## Build 273 completed — standalone project bridge, searchable inventory and output clarity

1. Creative Process Inventory selectors are type-searchable on both direct project usage and reviewed timeline-material posting. Inventory authority/conversion/reversal behavior is unchanged.
2. Content Studio now lists existing Creative Process projects separately from approved Products and existing Content Studio packages. Gray Hair / other standalone projects are selected from the existing project list; creating/refreshing a package never creates a second Creative Process project.
3. `?creative_project_id=` now resolves the Creative Process project into its Content Studio package when one exists, rather than being ignored as the older endpoint did.
4. A standalone Content Studio package can be created in draft state before timeline evidence is selected. It archives existing CAIP assets as private/reference-only media rows, then becomes review-ready as evidence/story work is added.
5. Existing CAIP identity is preserved: Content Studio attaches to the `creative_work_project` CAIP row instead of creating a competing CAIP project identity.
6. Automatic Output Blueprint now explains that it is a destination checklist—not a render engine—and shows live CAIP assets/probes/evidence/story/derivative counts plus Content Studio deliverables/channels. Each output indicates its current connected feed.
7. Standalone Content Studio copy is project-journal/evidence-first; product/availability/local-shopping assumptions are removed from the content-only path. Google Business Profile photo packs are not generated by default for unrelated standalone/social projects.
8. Content Studio and Creative Process received responsive overflow/search/layout hardening and existing visual placeholders remain the fallback for private CAIP media that intentionally has no public URL.
9. Markdown authority is reconfirmed: `AI_HANDOFF.md` + `PROJECT_STATUS_AND_ROADMAP.md` are the only mutable current authorities. Build files are release evidence and CAIP documents are specialist references, not competing roadmaps.
10. Build 273 is code/documentation-only. **No new D1 migration is required; Build 269 remains the focused CAIP schema boundary.**

### Ordered next work after Build 273

1. Production-test Gray Hair: open the existing Creative Process project from Content Studio, create/refresh its package, and confirm the existing CAIP project/asset references appear without a duplicate Creative Project or duplicate R2 upload.
2. Finish rights/privacy review on wanted CAIP media and choose the project events/source moments that actually support the story.
3. Implement first-class **video review + timecode evidence markers** so an operator can watch a proxy/source, mark in/out times, label technique/problem/result/lesson and attach those moments to `creative_story_evidence`.
4. Add the first real memory-bounded proxy/frame/audio/transcript provider adapter. Provider output must be verified before any job becomes complete.
5. Connect transcript/timecode evidence to story-segment drafting and require human approval before Content Studio copy refresh.
6. Create derivatives only for approved story needs; do not generate a derivative plan for every raw source by default.
7. Add reviewed content-calendar/scheduling handoff and post-release analytics loop after deliverable approval; publication remains outside CAIP and explicitly human-controlled.
8. Continue production CSS/mobile/SEO verification, using public-page evidence rather than adding thin pages or unrelated local keywords.
