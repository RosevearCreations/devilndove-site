...
1. Added an operator-facing six-step Creative Process lifecycle covering concept/estimates, actual work, material review/posting, CAIP media/evidence, Content Studio story/output work and project completion/archive.
2. Timeline material/ingredient fields are explicitly planning estimates and do not change Inventory. The direct-usage shortcut is now labeled as actual usage, requires a confirmation checkbox and remains appropriate for content-only/research work that physically consumes supplies.
3. Added audited timeline editing/removal. Unposted entries can be edited. Removing a posted usage restores stock through a compensating reversal first, then marks the event voided instead of deleting ledger history.
4. Added **Correct actual usage**: the original posting is reversed and preserved, the old event is voided, and a new corrected usage event/post becomes the active truth.
5. Active totals, material-review queues and selected-evidence feeds ignore voided entries; a collapsed Voided / corrected history remains available for audit.
6. Renamed and clarified Admin Dashboard responsibilities: **Website Media & Content Studio** owns static/public-site placements; **Creative Project Workflow** owns project/process → CAIP → Content Studio. The overlapping release cards are consolidated behind **Release & Go-Live Center**, with individual stages retained under Advanced release tools.
7. Build 274 adds `database_build274_creative_process_lifecycle_corrections.sql` after Build 269 and synchronizes the authoritative aggregate full/store schemas plus `fullSchemaRequirements.js`.
8. The two mutable Markdown authorities remain `AI_HANDOFF.md` and `PROJECT_STATUS_AND_ROADMAP.md`; specialist/build documents defer to them.
9. Admin Dashboard is explicitly `noindex,nofollow`. Public SEO direction remains evidence-rich, people-first, descriptive/concise titles and headings, and contextually relevant images/alt text rather than thin keyword pages.

### Ordered next work after Build 274

1. Production-test Project 7: concept-stage planned material should not change stock; then use one small actual material posting, correct it, and verify the original reversal + corrected post + voided history all reconcile.
2. Repair the accidental duplicate Goat's Milk entry using **Remove / undo entire entry** on the mistaken posting; confirm stock returns once and the entry moves to Voided / corrected history.
3. Production-test the content-only hair-dye project through the same lifecycle: inventory use is legitimate, Product is optional, CAIP media/evidence becomes the story source and Content Studio becomes the final package authority.
4. Implement first-class **video review + timecode/range evidence markers** so an operator can watch source/proxy footage and attach exact moments to technique/problem/result/lesson evidence.
5. Add the first real memory-bounded proxy/frame/audio/transcript provider adapter and verify provider output before marking jobs complete.
6. Connect transcript/timecode evidence to story-segment drafting and require human approval before Content Studio copy refresh.
7. Generate derivatives only for approved story needs; then add reviewed scheduling/calendar and post-release analytics as a downstream Content Studio/release concern, not a CAIP raw-intake concern.
8. Continue production CSS/mobile/SEO verification and gradually replace intentional visual placeholders with approved real workshop/project evidence through the correct media authority.
