...
export async function onRequestPost(context) {
  ...
  const requested_product_number =
  ...
  const inventory_quantity =
    body.inventory_quantity == null || body.inventory_quantity === "" ? 0 : Number(body.inventory_quantity);
  ...
  const existingProductNumber = await db
    .prepare(`SELECT product_id FROM products WHERE product_number = ? LIMIT 1`)
    .bind(product_number)
    .first();
  if (existingProductNumber) return json({ ok: false, error: "That product number already exists." }, 409);

  const existingSlug = await db
    .prepare(`SELECT product_id FROM products WHERE slug = ? LIMIT 1`)
    .bind(slug)
    .first();
  ...
  if (sku) {
    const existingSku = await db
      .prepare(`SELECT product_id FROM products WHERE sku = ? LIMIT 1`)
      .bind(sku)
      .first();
  ...
  const placeholders = columns.map((column) =>
    column === "created_at" || column === "updated_at" ? "CURRENT_TIMESTAMP" : "?"
  );
  ...
  for (let i = 0; i < image_urls.length; i += 1) {
    await db
      ...
      `)
      .bind(newProductId, image_urls[i], name, i)
      .run();
  }
  ...
  const createdImagesResult = await db
    .prepare(`
