...
import {
  ...
  safeUploadFileForClient, updateUploadFileGovernance
...
export async function onRequestPost(context){
  ...
  }catch(error){await captureRuntimeIncident(context.env,context.request,{incident_scope:'caip_media_intake',incident_code:'caip_media_intake_post_failed',severity:'warning',message:error?.message||'CAIP media intake action failed.',related_user_id:state.adminUser.user_id,details:{action,creative_project_id:projectId||null,upload_file_id:fileId||null,error:String(error?.message||error)}});return json({ok:false,error:error?.message||'CAIP media intake action failed.'},400);}
}
