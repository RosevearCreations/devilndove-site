...
import {
  ...
  createUploadSession, initiateUploadFile, listCaipMediaIntake, requestPublicPromotion,
...
export async function onRequestPost(context){
  ...
  }catch(error){const errorCode=intakeErrorCode(error,action);await captureRuntimeIncident(context.env,context.request,{incident_scope:'caip_media_intake',incident_code:'caip_media_intake_post_failed',severity:'warning',message:error?.message||'CAIP media intake action failed.',related_user_id:state.adminUser.user_id,details:{action,error_code:errorCode,creative_project_id:projectId||null,upload_file_id:fileId||null,error:String(error?.message||error)}});return json({ok:false,error:error?.message||'CAIP media intake action failed.',error_code:errorCode,stage:action||'unknown',creative_project_id:projectId||null,upload_file_id:fileId||null,binding:{private_bucket_binding:'CAIP_PRIVATE_MEDIA_BUCKET',private_bucket_available:privateBucketAvailable(context.env)}},400);}
}
