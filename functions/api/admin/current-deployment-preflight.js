// Release 467 Build 252 — current read-only Deployment Preflight over Build 251 Production source.
...
const REQUIRED_DEVELOPMENT_PROOFS=Object.freeze(['System Gate','Current Application Quality Proof','I.T. Admin Runtime Proof','Repository Branch Hygiene','Release 467 Build 252 Cross-Device Accessibility Acceptance Refresh']);
const VERIFIED_DEVELOPMENT=Object.freeze({
  ...
  branch_hygiene_run:35999263534,proof_state:'EXACT_BRANCH_HEAD_FOUR_PROOF_GREEN',exact_preview_deployment:true
});
...
export async function onRequestGet(context){
  ...
  const checks=[
    ...
    {status:'pass',code:'build250_verified_baseline',label:'Build 250 verified restart baseline',detail:'Build 250 Startup & Provider Read-Budget Verification is exact-tree Development and Production GREEN.'},
    {status:'pass',code:'build251_source_promotion',label:'Build 251 Production source promotion',detail:'Build 251 is on main f8e15d07e97e9a4e2953a65d09b494c73a36192a with the identical Development tree 2d6d06e321693779cee55ed2dd892bff3b364a36.'},
    {status:'review',code:'build252_accessibility_acceptance',label:'Build 252 Cross-Device Accessibility Acceptance Refresh',detail:'Build 252 re-proves phone/tablet/desktop, keyboard/focus, dense-workspace and reduced-motion contracts without business-data mutation.'}
  ];
  const blocker_count=checks.filter((x)=>x.status==='fail').length,warning_count=checks.filter((x)=>x.status==='review').length;
  const data={ok:true,release:RELEASE,build:BUILD,title:TITLE,state:'CURRENT_READ_ONLY',generated_at:new Date().toISOString(),summary:{status:blocker_count?'blocked':warning_count?'review':'ready',blocker_count,warning_count,pass_count:checks.filter((x)=>x.status==='pass').length,check_count:checks.length},checks,recent_runs:Array.isArray(historical?.recent_runs)?historical.recent_runs:[],post_deploy_confirmations:Array.isArray(historical?.post_deploy_confirmations)?historical.post_deploy_confirmations:[],canonical_migration_truth:truth,release_authority:{current_release:RELEASE,current_build:BUILD,required_development_proofs:REQUIRED_DEVELOPMENT_PROOFS,verified_development_checkpoint:VERIFIED_DEVELOPMENT,production:PRODUCTION,production_proof_transport:PRODUCTION_PROOF_TRANSPORT,current_candidate:{release:467,build:252,title:TITLE,authority:'release467-build252-cross-device-accessibility-acceptance-refresh.json'},rollback_readiness:'release-neutral-read-only',historical_feature_authority:'release467-build37-deployment-preflight-canonical-migration.json'},truth_notes:['Build 250 remains the last restart checkpoint with all four Development and Production proof run IDs enumerated in this source.','Build 251 source is promoted to Production main on the exact Development tree; unavailable push-only run IDs are not invented by Build 252.','Canonical migration authority remains through data-only 0023.','Build 252 is a source-and-CI cross-device accessibility acceptance refresh only; no schema change, Product publication, Inventory movement, Finance posting, R2 mutation, provider action or Production business-data copy.'],safety:{mutation_capability:'none',request_time_schema_mutation:false,d1_business_data_mutation:false,r2_mutation:false,binding_mutation:false,server_persistence:false,automatic_business_action:false,production_mutation:false}};
