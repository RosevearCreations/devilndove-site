// Release 467 Build 279 — current read-only Deployment Preflight over exact Build 278 Production source.
...
const REQUIRED_DEVELOPMENT_PROOFS=Object.freeze(['System Gate','Current Application Quality Proof','I.T. Admin Runtime Proof','Repository Branch Hygiene','Release 467 Build 279 Multipart Interruption Resume Acceptance Drill']);
const VERIFIED_DEVELOPMENT=Object.freeze({
  ...
  branch_hygiene_run:36247953198,dedicated_gate_run:36247952942,proof_state:'EXACT_BRANCH_HEAD_FIVE_PROOF_GREEN',exact_preview_deployment:true
});
const PRODUCTION=Object.freeze({
  ...
  products_browser_proof_run:36248156279,products_route_proof_run:36248156316,build_specific_proof_run:36248115177
});
...
export async function onRequestGet(context){
  ...
  const checks=[
    ...
    {status:'pass',code:'build278_verified_baseline',label:'Build 278 verified restart baseline',detail:'Build 278 Authenticated Private Review & Range-Streaming Acceptance Refresh is exact-tree Development and Production GREEN with all required named proofs.'},
    {status:'pass',code:'build278_source_promotion',label:'Build 278 Production source promotion',detail:'Build 278 is on main 5d418eb1160caa7af855a247e1ff3510e4c1c9b8 with the identical Development tree b7ad133e79cd01a30d2056f77ffd069996560cf6.'},
    {status:'review',code:'build279_multipart_resume_drill',label:'Build 279 Multipart Interruption & Resume Acceptance Drill',detail:'Build 279 performs the final Development-only multipart interruption/reselection/resume acceptance exercise; a GREEN dedicated workflow closes current-release CAIP acceptance at 3/3.'}
  ];
  const blocker_count=checks.filter((x)=>x.status==='fail').length,warning_count=checks.filter((x)=>x.status==='review').length;
  const data={ok:true,release:RELEASE,build:BUILD,title:TITLE,state:'CURRENT_READ_ONLY',generated_at:new Date().toISOString(),summary:{status:blocker_count?'blocked':warning_count?'review':'ready',blocker_count,warning_count,pass_count:checks.filter((x)=>x.status==='pass').length,check_count:checks.length},checks,recent_runs:Array.isArray(historical?.recent_runs)?historical.recent_runs:[],post_deploy_confirmations:Array.isArray(historical?.post_deploy_confirmations)?historical.post_deploy_confirmations:[],canonical_migration_truth:truth,release_authority:{current_release:RELEASE,current_build:BUILD,required_development_proofs:REQUIRED_DEVELOPMENT_PROOFS,verified_development_checkpoint:VERIFIED_DEVELOPMENT,production:PRODUCTION,production_proof_transport:PRODUCTION_PROOF_TRANSPORT,current_candidate:{release:467,build:279,title:TITLE,authority:'release467-build279-multipart-interruption-resume-acceptance-drill.json'},rollback_readiness:'release-neutral-read-only',historical_feature_authority:'release467-build37-deployment-preflight-canonical-migration.json'},truth_notes:['Build 278 is the exact fully verified Development and Production restart boundary.','Build 278 Development and Production share tree b7ad133e79cd01a30d2056f77ffd069996560cf6.','Canonical migration authority remains through data-only 0023.','Build 279 runs the Development-only multipart interruption/reselection/resume drill; Production remains mutation-free.'],safety:{mutation_capability:'none',request_time_schema_mutation:false,d1_business_data_mutation:false,r2_mutation:false,binding_mutation:false,server_persistence:false,automatic_business_action:false,production_mutation:false}};
