// Release 467 Build 251 — current read-only Deployment Preflight over exact Build 250 GREEN predecessor.
...
const REQUIRED_DEVELOPMENT_PROOFS=Object.freeze(['System Gate','Current Application Quality Proof','I.T. Admin Runtime Proof','Repository Branch Hygiene','Release 467 Build 251 CSP Style Injection Surface Hardening']);
...
export async function onRequestGet(context){
  ...
  const checks=[
    ...
    {status:'review',code:'build251_csp_style_candidate',label:'Build 251 CSP Style Injection-Surface Hardening',detail:'Build 251 narrows style-element CSP enforcement with response nonces while retaining explicit legacy style-attribute compatibility over the exact Build 250 GREEN boundary.'}
  ];
  const blocker_count=checks.filter((x)=>x.status==='fail').length,warning_count=checks.filter((x)=>x.status==='review').length;
  const data={ok:true,release:RELEASE,build:BUILD,title:TITLE,state:'CURRENT_READ_ONLY',generated_at:new Date().toISOString(),summary:{status:blocker_count?'blocked':warning_count?'review':'ready',blocker_count,warning_count,pass_count:checks.filter((x)=>x.status==='pass').length,check_count:checks.length},checks,recent_runs:Array.isArray(historical?.recent_runs)?historical.recent_runs:[],post_deploy_confirmations:Array.isArray(historical?.post_deploy_confirmations)?historical.post_deploy_confirmations:[],canonical_migration_truth:truth,release_authority:{current_release:RELEASE,current_build:BUILD,required_development_proofs:REQUIRED_DEVELOPMENT_PROOFS,verified_development_checkpoint:VERIFIED_DEVELOPMENT,production:PRODUCTION,production_proof_transport:PRODUCTION_PROOF_TRANSPORT,current_candidate:{release:467,build:251,title:TITLE,authority:'release467-build251-csp-style-injection-surface-hardening.json'},rollback_readiness:'release-neutral-read-only',historical_feature_authority:'release467-build37-deployment-preflight-canonical-migration.json'},truth_notes:['Build 250 is the exact fully verified Development and Production restart boundary.','Build 250 is the exact predecessor closure for Build 251.','Canonical migration authority remains through data-only 0023.','Build 251 changes browser CSP/style presentation plumbing only; no schema change, Product publication, Inventory movement, Finance posting, R2 mutation, provider action or Production business-data copy.'],safety:{mutation_capability:'none',request_time_schema_mutation:false,d1_business_data_mutation:false,r2_mutation:false,binding_mutation:false,server_persistence:false,automatic_business_action:false,production_mutation:false}};
