// Release 467 Build 253 — current read-only Deployment Preflight over Build 252 Production source.
...
const RELEASE=467;
const BUILD=253;
const TITLE='Session & Abuse-Control Runtime Evidence';
...
const REQUIRED_DEVELOPMENT_PROOFS=Object.freeze(['System Gate','Current Application Quality Proof','I.T. Admin Runtime Proof','Repository Branch Hygiene','Release 467 Build 253 Session Abuse-Control Runtime Evidence']);
const VERIFIED_DEVELOPMENT=Object.freeze({
  ...
  branch_hygiene_run:36042886574,proof_state:'EXACT_BRANCH_HEAD_FOUR_PROOF_GREEN',exact_preview_deployment:true
});
const PRODUCTION=Object.freeze({
  ...
  products_browser_proof_run:36043274912,products_route_proof_run:36043274915
});
...
export async function onRequestGet(context){
  ...
  const checks=[
    ...
    {status:'pass',code:'build252_verified_baseline',label:'Build 252 verified restart baseline',detail:'Build 252 Cross-Device Accessibility Acceptance Refresh is exact-tree Development and Production GREEN.'},
    {status:'pass',code:'build252_source_promotion',label:'Build 252 Production source promotion',detail:'Build 252 is on main 3c14d72ed481035d82f3cffa2d16f733f603f1d9 with the identical Development tree bc93b89d4b649d035e4dc5cb0f9deeb9d01399e2.'},
    {status:'review',code:'build253_session_abuse_runtime_evidence',label:'Build 253 Session & Abuse-Control Runtime Evidence',detail:'Build 253 exercises cookie-first sessions, origin guards, throttling, revoke-other-sessions and step-up paths with synthetic bounded runtime evidence only.'}
  ];
  const blocker_count=checks.filter((x)=>x.status==='fail').length,warning_count=checks.filter((x)=>x.status==='review').length;
  const data={ok:true,release:RELEASE,build:BUILD,title:TITLE,state:'CURRENT_READ_ONLY',generated_at:new Date().toISOString(),summary:{status:blocker_count?'blocked':warning_count?'review':'ready',blocker_count,warning_count,pass_count:checks.filter((x)=>x.status==='pass').length,check_count:checks.length},checks,recent_runs:Array.isArray(historical?.recent_runs)?historical.recent_runs:[],post_deploy_confirmations:Array.isArray(historical?.post_deploy_confirmations)?historical.post_deploy_confirmations:[],canonical_migration_truth:truth,release_authority:{current_release:RELEASE,current_build:BUILD,required_development_proofs:REQUIRED_DEVELOPMENT_PROOFS,verified_development_checkpoint:VERIFIED_DEVELOPMENT,production:PRODUCTION,production_proof_transport:PRODUCTION_PROOF_TRANSPORT,current_candidate:{release:467,build:253,title:TITLE,authority:'release467-build253-session-abuse-control-runtime-evidence.json'},rollback_readiness:'release-neutral-read-only',historical_feature_authority:'release467-build37-deployment-preflight-canonical-migration.json'},truth_notes:['Build 252 is the exact fully verified Development and Production restart boundary.','Build 252 Development and Production share tree bc93b89d4b649d035e4dc5cb0f9deeb9d01399e2.','Canonical migration authority remains through data-only 0023.','Build 253 is bounded synthetic session/abuse-control runtime evidence only; no schema, business-data, provider, Product, Inventory, Finance or R2 mutation.'],safety:{mutation_capability:'none',request_time_schema_mutation:false,d1_business_data_mutation:false,r2_mutation:false,binding_mutation:false,server_persistence:false,automatic_business_action:false,production_mutation:false}};
