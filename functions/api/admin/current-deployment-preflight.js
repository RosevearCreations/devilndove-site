// Release 467 Build 48 — current Deployment Preflight canonical migration projection.
// Build 49 restart-integrity convergence exposes the later fully verified closure
// separately from Build 48 implementation acceptance.
...
const LEGACY_MIGRATION_CHECK_PREFIXES = Object.freeze(['schema_ledger_','build171_','build173_','build174_','build175_','build176_']);
...
function currentCheck(status, code, label, detail, action = '', evidence = {}) { return { status, code, label, detail, action, evidence }; }
async function tableExists(db, tableName) { try { return Boolean(await db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=? LIMIT 1").bind(tableName).first()); } catch { return false; } }
...
async function canonicalMigrationTruth(db) {
  ...
  const migrations = CANONICAL_MIGRATIONS.map((file, index) => ({ version: index + 1, file, native_applied: nativeNames.has(file), proof_recorded: proofNames.has(file), native_row: nativeRows.find((row) => normalizeText(row.name) === file) || null, proof_row: proofRows.find((row) => normalizeText(row.migration_name) === file) || null }));
  return { stream: 'devilndove-canonical-forward', manifest_path: 'migrations/canonical/manifest.json', applicator: 'scripts/d1_migrate.py', native_ledger: 'd1_migrations', proof_table: 'app_schema_migration_proofs', expected_count: CANONICAL_MIGRATIONS.length, native_ledger_exists: nativeLedgerExists, proof_table_exists: proofTableExists, migrations, native_applied_count: migrations.filter((row) => row.native_applied).length, proof_recorded_count: migrations.filter((row) => row.proof_recorded).length, foreign_key_violations: fkRows.length, foreign_key_rows: fkRows.slice(0, 20) };
}
function filterHistoricalChecks(checks = []) {
  return checks.filter((check) => { const code = normalizeText(check?.code); if (LEGACY_MIGRATION_CHECK_CODES.has(code)) return false; return !LEGACY_MIGRATION_CHECK_PREFIXES.some((prefix) => code.startsWith(prefix)); }).map((check) => {
    if (check?.code === 'deployment_preflight_runs_missing') return { ...check, action: 'Historical snapshot storage is unavailable. Do not create schema during a request; repair only through a new canonical migration when required.' };
    if (check?.code === 'post_deploy_confirmations_missing') return { ...check, action: 'Historical confirmation storage is unavailable. Do not repair schema from this page; use the canonical migration process when a real forward change is approved.' };
    if (check?.code === 'core_tables_present' && check?.status === 'fail') return { ...check, action: 'Fail closed. Add or apply the next canonical migration through scripts/d1_migrate.py; never replay historical build-numbered SQL.' };
    return check;
  });
}
function canonicalChecks(truth, expectedSchema) {
  ...
  return [
    ...
    currentCheck('pass', 'restart_integrity_checkpoint', 'Restart integrity checkpoint', `Last fully verified Development is Build ${VERIFIED_DEVELOPMENT.build} at ${VERIFIED_DEVELOPMENT.dev_sha}.`, 'Before starting a later build, verify any newer dev branch head has all four required proofs and exact Preview acceptance.', VERIFIED_DEVELOPMENT),
  ];
}
function currentMigrationPlan() {
  return {
    canonical_authority: ['Treat migrations/canonical/manifest.json as the only forward D1 schema authority.','Historical build-numbered migration files are provenance only and must not be replayed from Deployment Preflight.','Never perform schema repair during an HTTP request; use only the approved canonical migration path.'],
    development: ['Add a new monotonically numbered canonical migration only when a real forward schema change is required.','Run scripts/d1_migrate.py --target development --apply.','Verify d1_migrations, app_schema_migration_proofs, checksums, source SHA and PRAGMA foreign_key_check.','Require exact merged-Development System Gate, Current Application Quality, I.T. Admin Runtime and Branch Hygiene proofs.'],
    production: ['Do not infer Production readiness from Development alone.','Promote only when explicitly authorized, using the exact fully-green Development tree.','Apply the same immutable canonical migration stream to Production before dependent code when a schema change exists.','Rollback readiness remains release-neutral and read-only; it does not reverse schema automatically.'],
  };
}
function markdownReport(data) {
  const lines = [`# Release ${RELEASE} Build ${BUILD} Deployment Preflight`,'',`Generated: ${data.generated_at}`,'','## Current authority','',`- State: ${data.state}`,`- Status: ${data.summary?.status || 'unknown'}`,`- Last fully verified Development: ${data.release_authority?.verified_development_checkpoint?.dev_sha || 'unknown'}`,`- Canonical migrations: ${data.canonical_migration_truth?.native_applied_count || 0}/${data.canonical_migration_truth?.expected_count || 0}`,`- Canonical proof rows: ${data.canonical_migration_truth?.proof_recorded_count || 0}/${data.canonical_migration_truth?.expected_count || 0}`,`- Foreign-key violations: ${data.canonical_migration_truth?.foreign_key_violations || 0}`,'','## Checklist',''];
  ...
  for (const [section, items] of Object.entries(data.migration_plan || {})) { lines.push(`### ${section.replace(/_/g, ' ')}`, ''); for (const item of items || []) lines.push(`- ${item}`); lines.push(''); }
  return `${lines.join('\n')}\n`;
}
...
export async function onRequestGet(context) {
  ...
  const data = {
    ...base, release: RELEASE, build: BUILD, title: TITLE, build_label: `Release ${RELEASE} Build ${BUILD}`, authority: AUTHORITY, state: 'CURRENT_READ_ONLY', generated_at, checks, summary: statusSummary(checks), ledger: null, expected_schema: expectedSchema, migration_plan: currentMigrationPlan(), canonical_migration_truth: truth,
    release_authority: {
    ...
    truth_notes: [
      ...
      'The last fully verified Development checkpoint is Build 48 dev 3980661045c68f55fa64e66e6414055ee0d359f6 / tree 66e5e0a088d6eb9f4f85f70791de8925ac40adb0 with System 33925647553, Quality 33925647532, I.T. 33925647561 and Hygiene 33925647559 successful.',
      'Build 48 implementation acceptance remains a distinct earlier checkpoint; a closure candidate never self-claims proof results that run only after the commit exists.',
      ...
      'Build 48 adds no canonical migration and no request-time DDL; the stream remains exactly 0001-0004.',
      'ACCEPTED_FOR_CONTROLLED_PRODUCTION is metadata-only and authorizes no renderer, provider execution, publication, social handoff, R2 mutation, main mutation or application Production promotion.',
      'Build 49 Current Authority Convergence & Restart Integrity adds no schema and opens no external execution path.',
      'Production remains Build 32 until a deliberate exact-tree promotion is requested and independently proven.'
    ],
    safety: { mutation_capability: 'none', request_time_schema_mutation: false, d1_business_data_mutation: false, r2_mutation: false, provider_execution: false, provider_publication: false, cloudflare_access_mutation: false, main_mutation: false, production_mutation: false, rollback_execution: false, secret_values_emitted: false },
  };
