// Release 467 Build 153 — current Deployment Preflight canonical migration projection.
...
const RELEASE=467;
const BUILD=153;
const TITLE='Layout Observer Performance Hotfix';
...
const VERIFIED_DEVELOPMENT=Object.freeze({release:467,build:153,title:TITLE,dev_sha:'b8323b4e13ae08a8126da761106367de75f7cd40',tree_sha:'a3d0225c579953d5572dc99313661c2981c42510',system_gate_run:34863777573,current_application_quality_run:34863777529,it_admin_runtime_proof_run:34863777559,branch_hygiene_run:34863777543,proof_state:'EXACT_BRANCH_HEAD_FOUR_PROOF_GREEN',exact_preview_deployment:true});
const PRODUCTION=Object.freeze({release:467,build:153,state:'PRODUCTION_GREEN',main_sha:'ba8b3c2406335391334b2a74a89e5819236c770b',tree_sha:'a3d0225c579953d5572dc99313661c2981c42510',production_pages_deploy_run:34864015766,production_live_resource_integrity_run:34864113781,post_closure_incident:'Build 154 addresses /admin/products/ HTTP 503 / Cloudflare Error 1102 observed 2026-09-14T15:48:10Z.'});
...
export async function onRequestGet(context){
  ...
  const checks=[
    ...
    {status:'pass',code:'restart_integrity_checkpoint',label:'Restart integrity checkpoint',detail:`Build 153 is fully verified at ${VERIFIED_DEVELOPMENT.dev_sha}; Build 154 is the active Products Worker resource candidate.`}
  ];
  const blocker_count=checks.filter((x)=>x.status==='fail').length;
  const data={ok:true,release:RELEASE,build:BUILD,title:TITLE,state:'CURRENT_READ_ONLY',generated_at:new Date().toISOString(),summary:{status:blocker_count?'blocked':'ready',blocker_count,warning_count:1,pass_count:checks.length-blocker_count,check_count:checks.length},checks,recent_runs:Array.isArray(historical?.recent_runs)?historical.recent_runs:[],post_deploy_confirmations:Array.isArray(historical?.post_deploy_confirmations)?historical.post_deploy_confirmations:[],canonical_migration_truth:truth,release_authority:{current_release:RELEASE,current_build:BUILD,required_development_proofs:REQUIRED_DEVELOPMENT_PROOFS,verified_development_checkpoint:VERIFIED_DEVELOPMENT,production:PRODUCTION,production_proof_transport:PRODUCTION_PROOF_TRANSPORT,rollback_readiness:'release-neutral-read-only',historical_feature_authority:'release467-build37-deployment-preflight-canonical-migration.json'},truth_notes:[`Build 153 is the current fully verified Development baseline at ${VERIFIED_DEVELOPMENT.dev_sha}; Production is ${PRODUCTION.main_sha}; both share ${VERIFIED_DEVELOPMENT.tree_sha}.`,`Development proofs: System ${VERIFIED_DEVELOPMENT.system_gate_run}, Quality ${VERIFIED_DEVELOPMENT.current_application_quality_run}, I.T. ${VERIFIED_DEVELOPMENT.it_admin_runtime_proof_run}, Hygiene ${VERIFIED_DEVELOPMENT.branch_hygiene_run}.`,`Production proofs: Pages ${PRODUCTION.production_pages_deploy_run}, Live Resources ${PRODUCTION.production_live_resource_integrity_run}.`,'Build 154 Products Worker Resource Hotfix is active because /admin/products/ later returned HTTP 503 / Cloudflare Error 1102.','Build 154 must add direct live Products-route proof before the incident is closed.','Canonical migrations remain exactly 0001-0004.'],safety:{mutation_capability:'none',request_time_schema_mutation:false,d1_business_data_mutation:false,r2_mutation:false,binding_mutation:false,server_persistence:false,automatic_business_action:false,production_mutation:false}};
