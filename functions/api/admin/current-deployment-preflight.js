// Release 467 Build 262 — current read-only Deployment Preflight over exact Build 261 Production source.
...
const RELEASE=467;
const BUILD=262;
const TITLE='Operations Today-Tasks Read Fan-Out Review';
...
const REQUIRED_DEVELOPMENT_PROOFS=Object.freeze(['System Gate','Current Application Quality Proof','I.T. Admin Runtime Proof','Repository Branch Hygiene','Release 467 Build 262 Operations Today-Tasks Read Fan-Out Review']);
const VERIFIED_DEVELOPMENT=Object.freeze({
  release:467,build:261,title:'Production Proof Dependency Orchestration',state:'DEVELOPMENT_GREEN',
  dev_sha:'c4e57f8d4c47a709021fca65037b25f4e74d63e2',tree_sha:'a442d160e1619e4b362de2bbda509ab25c310290',
  system_gate_run:36087874819,current_application_quality_run:36087874854,it_admin_runtime_proof_run:36087874772,
  branch_hygiene_run:36087874784,proof_state:'EXACT_BRANCH_HEAD_FOUR_PROOF_GREEN',exact_preview_deployment:true
});
const PRODUCTION=Object.freeze({
  release:467,build:261,title:'Production Proof Dependency Orchestration',state:'PRODUCTION_GREEN',
  main_sha:'f90944f80ec193610d3b87312487799ec425d983',tree_sha:'a442d160e1619e4b362de2bbda509ab25c310290',
  production_pages_deploy_run:36088094461,production_live_resource_integrity_run:36088159346,
  products_browser_proof_run:36088159392,products_route_proof_run:36088159367
});
...
export async function onRequestGet(context){
  ...
  const checks=[
    ...
    {status:'pass',code:'build261_verified_baseline',label:'Build 261 verified restart baseline',detail:'Build 261 Production Proof Dependency Orchestration is exact-tree Development and Production GREEN.'},
    {status:'pass',code:'build261_source_promotion',label:'Build 261 Production source promotion',detail:'Build 261 is on main f90944f80ec193610d3b87312487799ec425d983 with the identical Development tree a442d160e1619e4b362de2bbda509ab25c310290.'},
    {status:'review',code:'build262_today_tasks_read_fanout',label:'Build 262 Operations Today-Tasks Read Fan-Out Review',detail:'Build 262 batches six latest-action point lookups into one D1 statement, targeting 13 to 8 top-level Today Tasks reads while retaining the Build 250 provider ceilings.'}
  ];
  const blocker_count=checks.filter((x)=>x.status==='fail').length,warning_count=checks.filter((x)=>x.status==='review').length;
  const data={ok:true,release:RELEASE,build:BUILD,title:TITLE,state:'CURRENT_READ_ONLY',generated_at:new Date().toISOString(),summary:{status:blocker_count?'blocked':warning_count?'review':'ready',blocker_count,warning_count,pass_count:checks.filter((x)=>x.status==='pass').length,check_count:checks.length},checks,recent_runs:Array.isArray(historical?.recent_runs)?historical.recent_runs:[],post_deploy_confirmations:Array.isArray(historical?.post_deploy_confirmations)?historical.post_deploy_confirmations:[],canonical_migration_truth:truth,release_authority:{current_release:RELEASE,current_build:BUILD,required_development_proofs:REQUIRED_DEVELOPMENT_PROOFS,verified_development_checkpoint:VERIFIED_DEVELOPMENT,production:PRODUCTION,production_proof_transport:PRODUCTION_PROOF_TRANSPORT,current_candidate:{release:467,build:262,title:TITLE,authority:'release467-build262-operations-today-tasks-read-fanout-review.json'},rollback_readiness:'release-neutral-read-only',historical_feature_authority:'release467-build37-deployment-preflight-canonical-migration.json'},truth_notes:['Build 261 is the exact fully verified Development and Production restart boundary.','Build 261 Development and Production share tree a442d160e1619e4b362de2bbda509ab25c310290.','Canonical migration authority remains through data-only 0023.','Build 262 reviews the measured Today Tasks read hotspot and targets 13 to 8 top-level provider statements without raising the 15k/25k provider ceilings; Build 263 is next.'],safety:{mutation_capability:'none',request_time_schema_mutation:false,d1_business_data_mutation:false,r2_mutation:false,binding_mutation:false,server_persistence:false,automatic_business_action:false,production_mutation:false}};
