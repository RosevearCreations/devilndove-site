// Release 467 Build 157 — current Deployment Preflight over exact Build 156 baseline.
...
const RELEASE=467;
const BUILD=157;
const TITLE='Product Admin + Admin Data Delivery';
...
const REQUIRED_DEVELOPMENT_PROOFS=Object.freeze(['System Gate','Current Application Quality Proof','I.T. Admin Runtime Proof','Repository Branch Hygiene','Release 467 Build 157 Admin Data Delivery Proof','Release 467 Build 156 Inventory Process Assignment Proof']);
const VERIFIED_DEVELOPMENT=Object.freeze({release:467,build:156,title:'Tool & Supply Process Assignment',dev_sha:'1b375af8dadb1bfc32086708469cc2008ba9fc9f',tree_sha:'96928f3c3d09dcf1999387d9a63a823f264bebf8',system_gate_run:35019952492,current_application_quality_run:35019952480,it_admin_runtime_proof_run:35019952545,branch_hygiene_run:35019952489,proof_state:'EXACT_BRANCH_HEAD_FOUR_PROOF_GREEN',exact_preview_deployment:true});
const PRODUCTION=Object.freeze({release:467,build:156,state:'PRODUCTION_GREEN',main_sha:'1b375af8dadb1bfc32086708469cc2008ba9fc9f',tree_sha:'96928f3c3d09dcf1999387d9a63a823f264bebf8',production_pages_deploy_run:35020167619,production_live_resource_integrity_run:35020259348,products_production_browser_proof_run:35020259412,products_route_production_proof_run:35020259408,client_ui_incident_state:'BUILD157_QUALITY_TIMEOUT_AND_ADMIN_MEDIA_RETRY_REPAIR',client_ui_usability_proven:true});
...
export async function onRequestGet(context){
  ...
  const checks=[
    ...
    {status:'pass',code:'products_build156_baseline',label:'Build 156 Production baseline',detail:'Exact Build 156 Product population/browser proof, live resources and no-1102 route proof are GREEN.'},
    {status:'review',code:'build157_product_quality_media_follow_up',label:'Build 157 Product Admin follow-up',detail:'A live administrator session later exposed an 8000 ms Product Quality startup timeout plus repeated missing-media 404 retries. Build 157 addresses those as a bounded Development candidate without changing Product write authority.'},
    {status:'review',code:'restart_integrity_checkpoint',label:'Restart integrity checkpoint',detail:`Build 156 is fully verified at ${VERIFIED_DEVELOPMENT.dev_sha}; Build 157 is the active Product Admin + Admin Data Delivery candidate.`}
  ];
  const blocker_count=checks.filter((x)=>x.status==='fail').length;
  const warning_count=checks.filter((x)=>x.status==='review').length;
  const data={ok:true,release:RELEASE,build:BUILD,title:TITLE,state:'CURRENT_READ_ONLY',generated_at:new Date().toISOString(),summary:{status:blocker_count?'blocked':warning_count?'review':'ready',blocker_count,warning_count,pass_count:checks.filter((x)=>x.status==='pass').length,check_count:checks.length},checks,recent_runs:Array.isArray(historical?.recent_runs)?historical.recent_runs:[],post_deploy_confirmations:Array.isArray(historical?.post_deploy_confirmations)?historical.post_deploy_confirmations:[],canonical_migration_truth:truth,release_authority:{current_release:RELEASE,current_build:BUILD,required_development_proofs:REQUIRED_DEVELOPMENT_PROOFS,verified_development_checkpoint:VERIFIED_DEVELOPMENT,production:PRODUCTION,production_proof_transport:PRODUCTION_PROOF_TRANSPORT,current_candidate:{release:467,build:157,title:TITLE,authority:'release467-build157-admin-data-delivery.json'},rollback_readiness:'release-neutral-read-only',historical_feature_authority:'release467-build37-deployment-preflight-canonical-migration.json'},truth_notes:[`Build 156 is the current fully verified Development baseline at ${VERIFIED_DEVELOPMENT.dev_sha}; Production is ${PRODUCTION.main_sha}; both share ${VERIFIED_DEVELOPMENT.tree_sha}.`,`Development proofs: System ${VERIFIED_DEVELOPMENT.system_gate_run}, Quality ${VERIFIED_DEVELOPMENT.current_application_quality_run}, I.T. ${VERIFIED_DEVELOPMENT.it_admin_runtime_proof_run}, Hygiene ${VERIFIED_DEVELOPMENT.branch_hygiene_run}.`,`Production proofs: Pages ${PRODUCTION.production_pages_deploy_run}, Live Resources ${PRODUCTION.production_live_resource_integrity_run}, Products Browser ${PRODUCTION.products_production_browser_proof_run}, Products Route ${PRODUCTION.products_route_production_proof_run}.`,'Build 157 Product Admin + Admin Data Delivery is the active Development candidate for the observed Product Quality 8000 ms timeout and Admin missing-media retry storm.','Canonical migrations are 0001-0005; request-time DDL remains closed.'],safety:{mutation_capability:'none',request_time_schema_mutation:false,d1_business_data_mutation:false,r2_mutation:false,binding_mutation:false,server_persistence:false,automatic_business_action:false,production_mutation:false}};
