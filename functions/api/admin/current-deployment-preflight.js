// Release 467 Build 223 — current read-only Deployment Preflight over exact Build 222 GREEN predecessor.
...
const REQUIRED_DEVELOPMENT_PROOFS=Object.freeze(['System Gate','Current Application Quality Proof','I.T. Admin Runtime Proof','Repository Branch Hygiene','Release 467 Build 222 Project-to-Knowledge Promotion Recipe History Proof']);
const VERIFIED_DEVELOPMENT=Object.freeze({
  ...
  branch_hygiene_run:35624331702,build_specific_proof_run:35624331629,proof_state:'EXACT_BRANCH_HEAD_FOUR_PROOF_GREEN',exact_preview_deployment:true
});
const PRODUCTION=Object.freeze({
  ...
  products_browser_proof_run:35624977818,products_route_proof_run:35624977830,build_specific_proof_run:35624807852
});
...
export async function onRequestGet(context){
  ...
  const checks=[
    ...
    {status:'pass',code:'build222_verified_baseline',label:'Build 222 verified restart baseline',detail:'Build 222 Project-to-Knowledge Promotion & Recipe History is exact-SHA Production GREEN.'},
    {status:'review',code:'build223_case_studies_search_richness',label:'Build 223 Capability Case Studies, Workshop Journal & Search Richness',detail:'Read-only published-content projection and public discovery changes must pass exact-head Development proof before Production promotion; no migration is added.'}
  ];
  const blocker_count=checks.filter((x)=>x.status==='fail').length,warning_count=checks.filter((x)=>x.status==='review').length;
  const data={ok:true,release:RELEASE,build:BUILD,title:TITLE,state:'CURRENT_READ_ONLY',generated_at:new Date().toISOString(),summary:{status:blocker_count?'blocked':warning_count?'review':'ready',blocker_count,warning_count,pass_count:checks.filter((x)=>x.status==='pass').length,check_count:checks.length},checks,recent_runs:Array.isArray(historical?.recent_runs)?historical.recent_runs:[],post_deploy_confirmations:Array.isArray(historical?.post_deploy_confirmations)?historical.post_deploy_confirmations:[],canonical_migration_truth:truth,release_authority:{current_release:RELEASE,current_build:BUILD,required_development_proofs:REQUIRED_DEVELOPMENT_PROOFS,verified_development_checkpoint:VERIFIED_DEVELOPMENT,production:PRODUCTION,production_proof_transport:PRODUCTION_PROOF_TRANSPORT,current_candidate:{release:467,build:223,title:TITLE,authority:'release467-build223-capability-case-studies-workshop-journal-search-richness.json'},rollback_readiness:'release-neutral-read-only',historical_feature_authority:'release467-build37-deployment-preflight-canonical-migration.json'},truth_notes:['Build 222 is the exact fully verified Development and Production restart boundary.','Build 223 Capability Case Studies, Workshop Journal & Search Richness is the active Development closure candidate.','Canonical migrations remain through 0021; Build 223 adds no schema, publication engine, private CAIP exposure or source mutation.'],safety:{mutation_capability:'none',request_time_schema_mutation:false,d1_business_data_mutation:false,r2_mutation:false,binding_mutation:false,server_persistence:false,automatic_business_action:false,production_mutation:false}};
