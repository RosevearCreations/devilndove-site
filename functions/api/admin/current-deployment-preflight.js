// Release 467 Build 237 — current read-only Deployment Preflight over exact Build 234 GREEN predecessor.
...
const RELEASE=467;
const BUILD=237;
const TITLE='Mobile, Touch, Keyboard & Dense-Workspace Ergonomics';
...
const REQUIRED_DEVELOPMENT_PROOFS=Object.freeze(['System Gate','Current Application Quality Proof','I.T. Admin Runtime Proof','Repository Branch Hygiene','Release 467 Build 237 Save Confidence Unsaved-Work Safe Batch Review']);
const VERIFIED_DEVELOPMENT=Object.freeze({
  ...
  branch_hygiene_run:35857576861,proof_state:'EXACT_BRANCH_HEAD_FOUR_PROOF_GREEN',exact_preview_deployment:true
});
const PRODUCTION=Object.freeze({
  ...
  products_browser_proof_run:35857829164,products_route_proof_run:35857829198
});
...
export async function onRequestGet(context){
  ...
  const checks=[
    ...
    {status:'pass',code:'build236_verified_baseline',label:'Build 236 verified restart baseline',detail:'Build 236 Mobile, Touch, Keyboard & Dense-Workspace Ergonomics is exact-SHA Development and Production GREEN.'},
    {status:'review',code:'build237_ergonomics_candidate',label:'Build 236 Mobile, Touch, Keyboard & Dense-Workspace Ergonomics',detail:'Build 237 is a presentation-only mobile/touch/keyboard ergonomics candidate; Production remains closed until exact Development proof.'}
  ];
  const blocker_count=checks.filter((x)=>x.status==='fail').length,warning_count=checks.filter((x)=>x.status==='review').length;
  const data={ok:true,release:RELEASE,build:BUILD,title:TITLE,state:'CURRENT_READ_ONLY',generated_at:new Date().toISOString(),summary:{status:blocker_count?'blocked':warning_count?'review':'ready',blocker_count,warning_count,pass_count:checks.filter((x)=>x.status==='pass').length,check_count:checks.length},checks,recent_runs:Array.isArray(historical?.recent_runs)?historical.recent_runs:[],post_deploy_confirmations:Array.isArray(historical?.post_deploy_confirmations)?historical.post_deploy_confirmations:[],canonical_migration_truth:truth,release_authority:{current_release:RELEASE,current_build:BUILD,required_development_proofs:REQUIRED_DEVELOPMENT_PROOFS,verified_development_checkpoint:VERIFIED_DEVELOPMENT,production:PRODUCTION,production_proof_transport:PRODUCTION_PROOF_TRANSPORT,current_candidate:{release:467,build:237,title:TITLE,authority:'release467-build237-mobile-touch-keyboard-dense-workspace-ergonomics.json'},rollback_readiness:'release-neutral-read-only',historical_feature_authority:'release467-build37-deployment-preflight-canonical-migration.json'},truth_notes:['Build 236 is the exact fully verified Development and Production restart boundary.','Build 236 standardizes save states and adds unsaved-work protection plus opt-in review-only batch summaries.','Canonical migration authority remains through data-only 0023.','Build 236 adds no schema change, automatic retry, batch mutation, Product publication, Inventory movement, Finance posting, R2 mutation, or provider action.'],safety:{mutation_capability:'none',request_time_schema_mutation:false,d1_business_data_mutation:false,r2_mutation:false,binding_mutation:false,server_persistence:false,automatic_business_action:false,production_mutation:false}};
