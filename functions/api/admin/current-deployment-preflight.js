// Release 467 Build 159 — current Deployment Preflight over exact Build 158 baseline.
...
const RELEASE=467;
const BUILD=159;
const TITLE='Product Returning-Browser Cache Coherence';
...
const REQUIRED_DEVELOPMENT_PROOFS=Object.freeze(['System Gate','Current Application Quality Proof','I.T. Admin Runtime Proof','Repository Branch Hygiene','Release 467 Build 159 Product Returning-Browser Cache Coherence Proof','Release 467 Build 158 Product Editor Startup Proof','Release 467 Build 157 Admin Data Delivery Proof','Release 467 Build 156 Inventory Process Assignment Proof']);
const VERIFIED_DEVELOPMENT=Object.freeze({release:467,build:158,title:'Product Editor Startup Resilience',dev_sha:'764ff267dab58c0c1068919f9a494c94984d054e',tree_sha:'bdaa5302bcaf26b451e9ef6954f777e26a2ee20b',system_gate_run:35055159738,current_application_quality_run:35055159744,it_admin_runtime_proof_run:35055159758,branch_hygiene_run:35055159722,proof_state:'EXACT_BRANCH_HEAD_FOUR_PROOF_GREEN',exact_preview_deployment:true});
const PRODUCTION=Object.freeze({release:467,build:158,state:'PRODUCTION_GREEN',main_sha:'764ff267dab58c0c1068919f9a494c94984d054e',tree_sha:'bdaa5302bcaf26b451e9ef6954f777e26a2ee20b',production_pages_deploy_run:35055350586,production_live_resource_integrity_run:35055416344,products_production_browser_proof_run:35055416341,products_route_production_proof_run:35055416367,client_ui_incident_state:'BUILD159_RETURNING_BROWSER_CACHE_COHERENCE',client_ui_usability_proven:true});
...
export async function onRequestGet(context){
  ...
  const checks=[
    ...
    {status:'pass',code:'products_build158_baseline',label:'Build 158 Production baseline',detail:'Exact Build 158 Product editor, Product browser, live-resource and route proofs are GREEN.'},
    {status:'review',code:'build159_returning_browser_cache_follow_up',label:'Build 159 returning-browser cache follow-up',detail:'Product Admin cache generations must converge for returning browsers without changing Product write authority.'},
    {status:'review',code:'restart_integrity_checkpoint',label:'Restart integrity checkpoint',detail:`Build 158 is fully verified at ${VERIFIED_DEVELOPMENT.dev_sha}; Build 159 is the active Product Returning-Browser Cache Coherence candidate.`}
  ];
  const blocker_count=checks.filter((x)=>x.status==='fail').length;
  const warning_count=checks.filter((x)=>x.status==='review').length;
  const data={ok:true,release:RELEASE,build:BUILD,title:TITLE,state:'CURRENT_READ_ONLY',generated_at:new Date().toISOString(),summary:{status:blocker_count?'blocked':warning_count?'review':'ready',blocker_count,warning_count,pass_count:checks.filter((x)=>x.status==='pass').length,check_count:checks.length},checks,recent_runs:Array.isArray(historical?.recent_runs)?historical.recent_runs:[],post_deploy_confirmations:Array.isArray(historical?.post_deploy_confirmations)?historical.post_deploy_confirmations:[],canonical_migration_truth:truth,release_authority:{current_release:RELEASE,current_build:BUILD,required_development_proofs:REQUIRED_DEVELOPMENT_PROOFS,verified_development_checkpoint:VERIFIED_DEVELOPMENT,production:PRODUCTION,production_proof_transport:PRODUCTION_PROOF_TRANSPORT,current_candidate:{release:467,build:159,title:TITLE,authority:'release467-build159-products-returning-browser-cache-coherence.json'},rollback_readiness:'release-neutral-read-only',historical_feature_authority:'release467-build37-deployment-preflight-canonical-migration.json'},truth_notes:[`Build 158 is the current fully verified Development baseline at ${VERIFIED_DEVELOPMENT.dev_sha}; Production is ${PRODUCTION.main_sha}; both share ${VERIFIED_DEVELOPMENT.tree_sha}.`,`Development proofs: System ${VERIFIED_DEVELOPMENT.system_gate_run}, Quality ${VERIFIED_DEVELOPMENT.current_application_quality_run}, I.T. ${VERIFIED_DEVELOPMENT.it_admin_runtime_proof_run}, Hygiene ${VERIFIED_DEVELOPMENT.branch_hygiene_run}.`,`Production proofs: Pages ${PRODUCTION.production_pages_deploy_run}, Live Resources ${PRODUCTION.production_live_resource_integrity_run}, Products Browser ${PRODUCTION.products_production_browser_proof_run}, Products Route ${PRODUCTION.products_route_production_proof_run}.`,'Build 159 Product Returning-Browser Cache Coherence is the active Development candidate; Production promotion remains closed until exact-head Development and browser acceptance are GREEN.','Canonical migrations are 0001-0005; request-time DDL remains closed.'],safety:{mutation_capability:'none',request_time_schema_mutation:false,d1_business_data_mutation:false,r2_mutation:false,binding_mutation:false,server_persistence:false,automatic_business_action:false,production_mutation:false}};
