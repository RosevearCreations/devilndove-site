// Release 467 Build 249 — current read-only Deployment Preflight over exact Build 248 GREEN predecessor.
...
const REQUIRED_DEVELOPMENT_PROOFS=Object.freeze(['System Gate','Current Application Quality Proof','I.T. Admin Runtime Proof','Repository Branch Hygiene','Release 467 Build 249 Refinement Runtime Measurement Outcome Baseline']);
const VERIFIED_DEVELOPMENT=Object.freeze({
  ...
  branch_hygiene_run:35942841826,proof_state:'EXACT_BRANCH_HEAD_FOUR_PROOF_GREEN',exact_preview_deployment:true
});
...
export async function onRequestGet(context){
  ...
  const checks=[
    ...
    {status:'pass',code:'build248_verified_baseline',label:'Build 248 verified restart baseline',detail:'Build 248 Refinement Outcomes Review & Roadmap Renewal is exact-tree Development and Production GREEN.'},
    {status:'review',code:'build249_runtime_measurement_candidate',label:'Build 249 Refinement Runtime Measurement & Outcome Baseline',detail:'Build 249 adds browser-local, payload-free measurement over the exact Build 248 GREEN boundary.'}
  ];
  const blocker_count=checks.filter((x)=>x.status==='fail').length,warning_count=checks.filter((x)=>x.status==='review').length;
  const data={ok:true,release:RELEASE,build:BUILD,title:TITLE,state:'CURRENT_READ_ONLY',generated_at:new Date().toISOString(),summary:{status:blocker_count?'blocked':warning_count?'review':'ready',blocker_count,warning_count,pass_count:checks.filter((x)=>x.status==='pass').length,check_count:checks.length},checks,recent_runs:Array.isArray(historical?.recent_runs)?historical.recent_runs:[],post_deploy_confirmations:Array.isArray(historical?.post_deploy_confirmations)?historical.post_deploy_confirmations:[],canonical_migration_truth:truth,release_authority:{current_release:RELEASE,current_build:BUILD,required_development_proofs:REQUIRED_DEVELOPMENT_PROOFS,verified_development_checkpoint:VERIFIED_DEVELOPMENT,production:PRODUCTION,production_proof_transport:PRODUCTION_PROOF_TRANSPORT,current_candidate:{release:467,build:249,title:TITLE,authority:'release467-build249-refinement-runtime-measurement-outcome-baseline.json'},rollback_readiness:'release-neutral-read-only',historical_feature_authority:'release467-build37-deployment-preflight-canonical-migration.json'},truth_notes:['Build 248 is the exact fully verified Development and Production restart boundary.','Build 248 is the exact predecessor closure for Build 249.','Canonical migration authority remains through data-only 0023.','Build 249 adds browser-local measurement only and no schema change, Product publication, Inventory movement, Finance posting, R2 mutation, provider action or remote telemetry.'],safety:{mutation_capability:'none',request_time_schema_mutation:false,d1_business_data_mutation:false,r2_mutation:false,binding_mutation:false,server_persistence:false,automatic_business_action:false,production_mutation:false}};
