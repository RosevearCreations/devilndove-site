...
export async function upsertAnnotation(db,payload){
  ...
  if(!columns.has('product_image_id'))return {saved_fields:[],unsupported_fields:Object.keys(payload.fields||{}),rows_read:measured,warning:'Product image annotations are unavailable; core Product image changes were still saved.'};
  ...
  const stable={
    ...
    alt_text:payload.alt_text==null?null:clean(payload.alt_text,1000),
    image_title:payload.image_title==null?null:clean(payload.image_title,250),
    caption:payload.caption==null?null:clean(payload.caption,2000),
    focal_point_x:payload.focal_point_x==null?null:Number(payload.focal_point_x),
    focal_point_y:payload.focal_point_y==null?null:Number(payload.focal_point_y),
    annotation_notes:payload.annotation_notes==null?null:clean(payload.annotation_notes,3000),
    image_role:payload.image_role==null?null:clean(payload.image_role,80),
    public_use_status:payload.public_use_status==null?null:clean(payload.public_use_status,80),
    role_review_notes:payload.role_review_notes==null?null:clean(payload.role_review_notes,1500),
    width_px:payload.width_px==null?null:Number(payload.width_px),
    height_px:payload.height_px==null?null:Number(payload.height_px),
    image_orientation:payload.image_orientation==null?null:clean(payload.image_orientation,20),
  };
  const candidateNames=Object.keys(stable).filter((name)=>name!=='product_image_id'&&columns.has(name)&&stable[name]!==undefined);
  const unsupported=Object.keys(payload.fields||{}).filter((name)=>!columns.has(name));
