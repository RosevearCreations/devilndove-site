...
function itemReadiness(item,triage,review,evidence,acks){
  ...
  if(review){
    ...
    if(decision==='accepted_with_limitations'&&String(latestAck?.acknowledgement_status||'')!=='acknowledged')blockers.push('Customer limitation acknowledgement is still required.');
  ...
  return {state,blockers,warnings,intake_evidence_count:intake.length,post_work_evidence_count:post.length,latest_acknowledgement_status:latestAck?.acknowledgement_status||null};
}
...
export async function onRequestPost(context){
  ...
  if(action==='create_acknowledgement_link'){
    ...
    const existing=await ctx.db.prepare(`SELECT * FROM custom_request_supplied_item_acknowledgements WHERE custom_request_supplied_item_id=? AND acknowledgement_status='active' ORDER BY custom_request_supplied_item_acknowledgement_id DESC LIMIT 1`)
      .bind(Number(item.custom_request_supplied_item_id)).first().catch(()=>null);
