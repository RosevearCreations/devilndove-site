...
function buildAccountantZip(data) {
  const manifest = {
    ...
    included_files: ['close-summary.csv', 'evidence-index.csv', 'manifest.json'],
...
async function payload(db, periodMonth) {
  ...
  return { ok: true, period_month: periodMonth, payment, hst_review: hst, closure, export_packages: packages, close_readiness: closeReadiness(closure, hst, payment) };
}
