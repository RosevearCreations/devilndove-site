...
// Release 467 Build 31: administrator-created accounts use the current salted password hash format.
...
export async function onRequestPost(context){
 ...
 const existing=await env.DB.prepare(`SELECT user_id,email FROM users WHERE LOWER(email)=LOWER(?) LIMIT 1`).bind(email).first();if(existing)return json({ok:false,error:"An account with this email already exists."},409);
 const password_hash=await formatStoredPasswordHashFromPlaintext(password);
 const insert=await env.DB.prepare(`INSERT INTO users (email,password_hash,display_name,role,is_active,created_at,updated_at) VALUES (?,?,?,?,?,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`).bind(email,password_hash,display_name||null,role,is_active).run();
 const user_id=insert?.meta?.last_row_id;if(!user_id)return json({ok:false,error:"User could not be created."},500);
