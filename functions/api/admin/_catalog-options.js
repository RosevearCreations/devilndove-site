...
export const DEFAULT_CATEGORY_OPTIONS = ['Rings','Necklaces','Bracelets','Earrings','Pendants','CNC Components','3D Printed Items','Laser Engraved Items','Polymer Clay Items','Home Decor','Accessories','Other'];
export const DEFAULT_COLOR_OPTIONS = ['Silver','Gold','Black','White','Red','Blue','Green','Purple','Pink','Orange','Yellow','Brown','Clear','Multicolor'];
export const DEFAULT_SHIPPING_CODE_OPTIONS = ['standard-jewelry','small-parcel','oversize','pickup-only','digital'];
...
export function uniqueSortedOptions(values = []) {
  return Array.from(new Set((Array.isArray(values) ? values : []).map(normalizeArrayValue).filter(Boolean)))
...
async function readProductDistinctValues(db, columnName) {
  try {
    ...
    `).all();
    return (Array.isArray(rows?.results) ? rows.results : []).map((row) => normalizeArrayValue(row?.option_value)).filter(Boolean);
  } catch {
    return [];
  }
}
...
export async function loadCatalogOptionSets(db) {
  const [savedCategories, savedColors, savedShipping, productCategories, productColors, productShipping] = await Promise.all([
  ...
  return {
    category_options: uniqueSortedOptions([...DEFAULT_CATEGORY_OPTIONS, ...savedCategories, ...productCategories]),
    color_options: uniqueSortedOptions([...DEFAULT_COLOR_OPTIONS, ...savedColors, ...productColors]),
    shipping_code_options: uniqueSortedOptions([...DEFAULT_SHIPPING_CODE_OPTIONS, ...savedShipping, ...productShipping]),
  };
}
...
export async function saveCatalogOptionSet(db, key, values, updatedByUserId = null) {
  ...
  `).bind(key, JSON.stringify(normalized), updatedByUserId || null).run();
  return normalized;
}
