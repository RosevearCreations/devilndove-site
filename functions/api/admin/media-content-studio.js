...
const MEDIA_TYPES = new Set(["photo","product","artwork","logo","background","banner","packaging","icon","proof","process","technique","evidence","materials","review","blog","other"]);
const SLOT_TYPES = new Set(["image","background","text"]);
const TARGET_ATTRS = new Set(["src","background-image","textContent"]);
const APPROVED_R2_PREFIXES = ["products/","brand/","creations/","social/","uploads/","public/","packaging/","artwork/","backgrounds/","banners/","galleries/","blog/"];
...
async function mediaList(db, query = {}) {
  ...
  const result = await db.prepare(`
    SELECT ma.media_asset_id,ma.product_id,ma.object_key,ma.public_url,ma.original_filename,ma.mime_type,ma.file_size_bytes,
           ma.width_px,ma.height_px,ma.created_at,ma.updated_at,ma.deleted_at,p.name AS product_name,
           ...
           (SELECT COUNT(*) FROM media_content_assignments a WHERE a.media_asset_id=ma.media_asset_id AND a.active=1) AS assignment_count
    ...
    WHERE ma.deleted_at IS NULL
      ...
      AND (?='' OR LOWER(COALESCE(mm.display_name,ma.original_filename,ma.object_key,'')) LIKE ? OR LOWER(COALESCE(mm.alt_text,'')) LIKE ? OR LOWER(COALESCE(mm.tags_json,'')) LIKE ? OR LOWER(COALESCE(p.name,'')) LIKE ?)
    ...
    LIMIT ?
  `).bind(includeArchived, mediaType, mediaType, q, `%${q}%`, `%${q}%`, `%${q}%`, `%${q}%`, assignment, assignment, assignment, limit).all();
  return rows(result).map((r) => ({
    ...
    tags:safeJson(r.tags_json,[]), media_type:r.media_type||"photo", display_name:r.display_name||r.product_name||r.original_filename||r.object_key,
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    return json({ok:false,error:"Media & Content Studio data could not be loaded. Apply the Build 256 D1 migration first.",detail:String(error?.message||error)},503);
  }
}
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (action === "save_media_metadata") {
      ...
      const exists=await db.prepare(`SELECT media_asset_id FROM media_assets WHERE media_asset_id=? AND deleted_at IS NULL`).bind(mediaId).first(); if(!exists)return json({ok:false,error:"Media item not found."},404);
    ...
    if (action === "assign_media") {
      ...
      const media=await db.prepare(`SELECT ma.media_asset_id,mm.archived_at FROM media_assets ma LEFT JOIN managed_media_metadata mm ON mm.media_asset_id=ma.media_asset_id WHERE ma.media_asset_id=? AND ma.deleted_at IS NULL`).bind(mediaId).first(); if(!media)return json({ok:false,error:"Media item not found."},404); if(media.archived_at)return json({ok:false,error:"Archived media cannot be newly assigned. Restore it first."},409);
