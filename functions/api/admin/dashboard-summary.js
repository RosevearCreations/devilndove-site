...
export async function onRequestGet(context) {
  ...
  const summary = {
    ...
    admin_write_runtime_incidents_count: await safeCount(db, `SELECT COUNT(*) AS count FROM runtime_incidents WHERE incident_scope IN ('admin_order_status_update','admin_record_payment','admin_payment_actions','admin_product_review_actions','admin_product_update','admin_product_seo','admin_create_product') AND created_at >= datetime('now', '-7 days')`),
    ...
    pending_shared_product_seo_actions_count: await safeCount(db, `SELECT COUNT(*) AS count FROM admin_pending_actions WHERE LOWER(COALESCE(action_scope,'')) = 'product_seo' AND queue_status IN ('queued','retrying','failed')`),
    pending_shared_product_create_actions_count: await safeCount(db, `SELECT COUNT(*) AS count FROM admin_pending_actions WHERE LOWER(COALESCE(action_scope,'')) = 'product_create' AND queue_status IN ('queued','retrying','failed')`),
