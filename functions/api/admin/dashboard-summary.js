...
export async function onRequestGet(context) {
  ...
  const summary = {
    ...
    products_missing_featured_image_count: await safeCount(db, `SELECT COUNT(*) AS count FROM products WHERE COALESCE(featured_image_url,'') = '' AND COALESCE(status,'draft') != 'archived'`),
    products_needing_three_images_count: await safeCount(db, `SELECT COUNT(*) AS count FROM (SELECT p.product_id FROM products p LEFT JOIN product_images pi ON pi.product_id = p.product_id WHERE COALESCE(p.status,'draft') != 'archived' GROUP BY p.product_id HAVING COUNT(DISTINCT pi.product_image_id) < 3)`),
    products_missing_image_roles_count: await safeCount(db, `SELECT COUNT(*) AS count FROM (SELECT p.product_id FROM products p LEFT JOIN product_images pi ON pi.product_id = p.product_id LEFT JOIN product_image_annotations pia ON pia.product_image_id = pi.product_image_id WHERE COALESCE(p.status,'draft') != 'archived' GROUP BY p.product_id HAVING COUNT(DISTINCT pi.product_image_id) > 0 AND SUM(CASE WHEN COALESCE(pia.image_role,'') = '' THEN 1 ELSE 0 END) > 0)`),
    products_missing_hero_role_count: await safeCount(db, `SELECT COUNT(*) AS count FROM (SELECT p.product_id FROM products p LEFT JOIN product_images pi ON pi.product_id = p.product_id LEFT JOIN product_image_annotations pia ON pia.product_image_id = pi.product_image_id WHERE COALESCE(p.status,'draft') != 'archived' GROUP BY p.product_id HAVING COUNT(DISTINCT pi.product_image_id) > 0 AND SUM(CASE WHEN LOWER(COALESCE(pia.image_role,'')) = 'hero_front' THEN 1 ELSE 0 END) = 0)`),
    products_missing_alt_text_count: await safeCount(db, `SELECT COUNT(*) AS count FROM (SELECT p.product_id FROM products p LEFT JOIN product_images pi ON pi.product_id = p.product_id WHERE COALESCE(p.status,'draft') != 'archived' GROUP BY p.product_id HAVING COUNT(DISTINCT pi.product_image_id) > 0 AND SUM(CASE WHEN LENGTH(TRIM(COALESCE(pi.alt_text,''))) >= 5 THEN 1 ELSE 0 END) < MIN(3, COUNT(DISTINCT pi.product_image_id)))`),
    products_blocked_public_images_count: await safeCount(db, `SELECT COUNT(DISTINCT p.product_id) AS count FROM products p INNER JOIN product_images pi ON pi.product_id = p.product_id INNER JOIN product_image_annotations pia ON pia.product_image_id = pi.product_image_id WHERE LOWER(COALESCE(pia.public_use_status,'')) IN ('consent_needed','blocked') AND COALESCE(p.status,'draft') != 'archived'`),
    products_missing_seo_count: await safeCount(db, `SELECT COUNT(*) AS count FROM products p LEFT JOIN product_seo ps ON ps.product_id = p.product_id WHERE COALESCE(p.status,'draft') != 'archived' AND (LENGTH(TRIM(COALESCE(ps.meta_title,''))) < 10 OR LENGTH(TRIM(COALESCE(ps.meta_description,''))) < 50)`),
