...
import { ensureAccountingStatementImportsTables } from './_accountingStatementImports.js';
...
export async function onRequestPost(context) {
  ...
  await db.prepare(`UPDATE accounting_reconciliation_exceptions SET exception_status = ?, notes = COALESCE(?, notes), assigned_to_user_id = CASE WHEN ? = 1 THEN ? ELSE assigned_to_user_id END, accountant_review_flag = CASE WHEN ? = 1 THEN 1 ELSE accountant_review_flag END, resolved_by_user_id = CASE WHEN ? = 'resolved' THEN ? ELSE resolved_by_user_id END, resolved_at = CASE WHEN ? = 'resolved' THEN CURRENT_TIMESTAMP ELSE resolved_at END, reopened_by_user_id = CASE WHEN ? = 'reopened' THEN ? ELSE reopened_by_user_id END, reopened_at = CASE WHEN ? = 'reopened' THEN CURRENT_TIMESTAMP ELSE reopened_at END, updated_at = CURRENT_TIMESTAMP WHERE accounting_reconciliation_exception_id = ?`).bind(status,notes||null,assignToSelf?1:0,adminUser.user_id,accountantFlag,status,adminUser.user_id,status,status,adminUser.user_id,status,exceptionId).run();
