// Devil n Dove Build 404 — Gift Card activation, void, refund, and reissue implementation.
// Schema authority: database_gift_card_runtime_parity.sql + database_notification_runtime_parity.sql.
...
const BUILD = 404;
const CONTRACT_ID = 'operations-gift-card-action-write';
...
async function queueGiftCardEmail(db, card, action, adminUser, note = '') {
  const schema = await requireNotificationSchema(db, { requiredTables: ['notification_outbox'] });
  if (!schema.ok) return { queued: false, reason: schema.error, schema: schema.readiness };
  ...
  const result = await db.prepare(`INSERT INTO notification_outbox (
    notification_kind, channel, destination, related_order_id, payload_json, metadata_json,
    status, next_attempt_at, created_at, updated_at
  ) VALUES ('gift_card_delivery', 'email', ?, ?, ?, ?, 'queued', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`)
    .bind(
      destination,
      Number(card?.order_id || 0) || null,
      JSON.stringify(payload),
      JSON.stringify({ source: 'gift_card_actions', action, queued_by_user_id: adminUser?.user_id || null })
    ).run();
...
export async function onRequestPost(context) {
  ...
  const db = getDb(env);
  if (!db) return json({ ok: false, build: BUILD, contract: CONTRACT_ID, error: 'Database binding is missing.' }, 500);
  const adminUser = await getAdminUserFromRequest(request, env);
  if (!adminUser) return json({ ok: false, build: BUILD, contract: CONTRACT_ID, error: 'Unauthorized.' }, 401);

  const schema = await requireGiftCardSchema(db, { requiredTables: ['gift_cards', 'gift_card_admin_events'] });
  if (!schema.ok) return json({
    ok: false,
    build: BUILD,
  ...
  let body = {};
  try { body = await request.json(); } catch { return json({ ok: false, build: BUILD, contract: CONTRACT_ID, error: 'Invalid JSON body.' }, 400); }
  const action = clean(body.action, 40).toLowerCase();
  const id = Number(body.gift_card_id || 0);
  if (!id) return json({ ok: false, build: BUILD, contract: CONTRACT_ID, error: 'gift_card_id is required.' }, 400);
  const card = await loadCard(db, id);
  if (!card) return json({ ok: false, build: BUILD, contract: CONTRACT_ID, error: 'Gift card was not found.' }, 404);
  ...
  if (action === 'activate_paid') {
    ...
    if (!paid) return json({ ok: false, build: BUILD, contract: CONTRACT_ID, error: 'The connected order is not marked paid yet.' }, 409);
    ...
    targetId = Number(insert?.meta?.last_row_id || 0) || id;
    ...
    await recordEvent(db, adminUser, { gift_card_id: targetId }, action, amount, note || `Reissued from card #${id}.`, id);
    message = `Gift card reissued as ${newCode}.`;
  } else {
    return json({ ok: false, build: BUILD, contract: CONTRACT_ID, error: 'Supported actions: activate_paid, void, refund, reissue.' }, 400);
  }
  ...
  const updated = await loadCard(db, targetId).catch(() => null);
  let delivery = { queued: false, reason: 'No delivery needed for this action.' };
  if (['activate_paid', 'reissue'].includes(action)) {
    delivery = await queueGiftCardEmail(db, updated, action === 'reissue' ? 'reissue' : 'activation', adminUser, note)
      .catch((error) => ({ queued: false, reason: error?.message || 'Gift-card email queue failed.' }));
  }
