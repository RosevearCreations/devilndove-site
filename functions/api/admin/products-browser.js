...
// One indexed Product-page read only. No joins, no correlated subqueries, no readiness scans,
// no media-table reads, no R2 access and no background refresh contract.
...
export async function onRequestGet(context){
  ...
  try{
    ...
    const products=rows.slice(0,limit).map((row)=>({
      ...
      product_number:row.product_number==null?null:Number(row.product_number),
