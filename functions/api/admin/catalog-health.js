...
async function summary(db) {
  ...
  // running correlated subqueries for every Product or Tool/Supply row.
  const [productRow, inventoryRow] = await Promise.all([
    db.prepare(`
      WITH active_products AS (
        SELECT product_id,featured_image_url,inventory_tracking,inventory_quantity
        FROM products
      ...
      SELECT COUNT(*) AS active_products,
        SUM(CASE WHEN TRIM(COALESCE(ap.featured_image_url,''))='' THEN 1 ELSE 0 END) AS missing_featured,
        SUM(CASE WHEN COALESCE(im.image_count,0)=0 THEN 1 ELSE 0 END) AS no_gallery,
        SUM(CASE WHEN COALESCE(im.image_count,0)>0 AND COALESCE(im.image_count,0)<3 THEN 1 ELSE 0 END) AS shallow_gallery,
        SUM(CASE WHEN COALESCE(im.alt_attention,0)>0 THEN 1 ELSE 0 END) AS products_with_alt_attention,
        SUM(CASE WHEN COALESCE(rs.missing_inventory_links,0)>0 THEN 1 ELSE 0 END) AS products_with_resource_gaps,
        SUM(CASE WHEN COALESCE(ap.inventory_tracking,0)=1 AND COALESCE(ap.inventory_quantity,0)<=0 THEN 1 ELSE 0 END) AS tracked_zero_stock
      FROM active_products ap
      LEFT JOIN image_stats im ON im.product_id=ap.product_id
      LEFT JOIN resource_stats rs ON rs.product_id=ap.product_id
    `).first(),
    db.prepare(`
      ...
      duplicate_counts AS (
        SELECT item_kind,external_key_norm,COUNT(*) AS duplicate_count
        FROM active_inventory GROUP BY item_kind,external_key_norm
      ),
      catalog_ranked AS (
        SELECT catalog_item_id,
               LOWER(TRIM(COALESCE(item_kind,''))) AS item_kind,
      ...
      SELECT COUNT(*) AS active_inventory,
        SUM(CASE WHEN ai.item_kind='tool' THEN 1 ELSE 0 END) AS active_tools,
        SUM(CASE WHEN ai.item_kind='supply' THEN 1 ELSE 0 END) AS active_supplies,
        SUM(CASE WHEN TRIM(COALESCE(ai.image_url,''))='' THEN 1 ELSE 0 END) AS blank_inventory_images,
        SUM(CASE WHEN TRIM(COALESCE(ai.image_url,''))<>'' AND TRIM(COALESCE(cr.image_url,''))<>'' AND TRIM(ai.image_url)<>TRIM(cr.image_url) THEN 1 ELSE 0 END) AS image_authority_mismatches,
        SUM(CASE WHEN COALESCE(dc.duplicate_count,0)>1 THEN 1 ELSE 0 END) AS rows_in_duplicate_identities,
        SUM(CASE WHEN ai.item_kind='supply' AND COALESCE(ai.on_hand_quantity,0)<=0 THEN 1 ELSE 0 END) AS zero_on_hand_supplies
      FROM active_inventory ai
      LEFT JOIN duplicate_counts dc ON dc.item_kind=ai.item_kind AND dc.external_key_norm=ai.external_key_norm
...
async function productIssues(db, q, limit) {
  ...
  const result = await db.prepare(`
    ...
    WHERE (?='' OR LOWER(COALESCE(name,'')) LIKE ? OR LOWER(COALESCE(sku,'')) LIKE ? OR LOWER(COALESCE(slug,'')) LIKE ?)
      AND (TRIM(COALESCE(featured_image_url,''))='' OR image_count<3 OR alt_attention>0 OR missing_inventory_links>0
...
async function inventoryIssues(db, q, kind, limit) {
  ...
  const result = await db.prepare(`
    ...
    base AS (
      SELECT ai.*,cr.catalog_item_id,cr.image_url AS catalog_image_url,COALESCE(dc.duplicate_count,0) AS duplicate_count
      FROM active_inventory ai
      LEFT JOIN duplicate_counts dc ON dc.item_kind=ai.item_kind AND dc.external_key_norm=ai.external_key_norm
      LEFT JOIN catalog_ranked cr ON cr.rn=1 AND cr.item_kind=ai.item_kind AND cr.source_key_norm=ai.external_key_norm
    )
    ...
    WHERE (?='' OR LOWER(COALESCE(item_name,'')) LIKE ? OR LOWER(COALESCE(external_key,'')) LIKE ? OR LOWER(COALESCE(category,'')) LIKE ?)
      AND (?='' OR item_kind=?)
      AND (TRIM(COALESCE(image_url,''))='' OR duplicate_count>1
