// Build 266 — bounded single-request CAIP upload for normal photos and short clips.
...
export async function onRequestPut(context){
  ...
  try{
    ...
    if(existing && Number(existing.size||0)===expected){
      ...
      return json({ok:true,direct_upload:true,already_present:true,uploaded_bytes:Number(existing.size||expected),etag:existing.etag||null,...completed});
    }
    ...
    return json({ok:true,direct_upload:true,uploaded_bytes:Number(head.size||expected),etag:object?.etag||head.etag||null,...completed});
