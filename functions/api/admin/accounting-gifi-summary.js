...
function asInt(value) { return Math.round(Number(value || 0)); }
...
async function buildFallbackSummary(db, range) {
  ...
  if (hasExpenses) {
    const expenseRows = normalizeResults(await db.prepare(`
      SELECT COALESCE(NULLIF(e.ledger_code,''), 'UNASSIGNED') AS ledger_code,
  ...
  if (hasWriteoffs) {
    const writeoffRows = normalizeResults(await db.prepare(`
      SELECT '6900' AS ledger_code,
    ...
    `).bind(range.start, range.end).all());
    rows.push(...writeoffRows.map((row) => ({ ...row, source_type: 'writeoffs' })).filter((row) => asInt(row.net_cents) !== 0));
  }

  if (hasOrders) {
    const orderRows = normalizeResults(await db.prepare(`
      SELECT '4000' AS ledger_code,
    ...
    `).bind(range.start, range.end).all());
    rows.push(...orderRows.map((row) => ({ ...row, source_type: 'orders' })).filter((row) => asInt(row.net_cents) !== 0));
  }
...
export async function onRequestGet(context) {
  ...
  const descriptionCol = entryCols.has('description') ? 'description' : (entryCols.has('memo') ? 'memo' : "''");
  ...
  if (entryTableExists && lineTableExists && entryId && lineId && lineEntryId) {
    rows = normalizeResults(await db.prepare(`
      SELECT
    ...
    unmapped = rows.filter((row) => !String(row.gifi_code || '').trim()).map((row) => ({
  ...
  for (const row of rows) {
    ...
    current.deductible_cents += Math.round(net * (Math.max(0, Math.min(100, Number(row.tax_deductibility_percent == null ? 100 : row.tax_deductibility_percent))) / 100));
    current.source_count += Number(row.source_count || 0);
    String(row.ledger_code || row.ledger_codes || '').split(',').map((part) => String(part).trim()).filter(Boolean).forEach((code) => current.ledger_codes.add(code));
    grouped.set(key, current);
  }
