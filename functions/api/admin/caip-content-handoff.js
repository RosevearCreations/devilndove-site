// Devil n Dove Release 448 — reviewed CAIP evidence -> Content Studio handoff.
...
async function load(db, projectId) {
  ...
  if (!projectRow) return { readiness, project:null, evidence:[], segments:[], handoff:null, handoff_evidence:[] };
  ...
  return { readiness, project:projectRow, evidence, segments, handoff, handoff_evidence:handoffEvidence };
}
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    await captureRuntimeIncident(context.env,context.request,{incident_scope:'caip_content_handoff',incident_code:'caip_content_handoff_get_failed',severity:'warning',message:error?.message||'CAIP content handoff could not load.',related_user_id:state.adminUser.user_id,details:{release:RELEASE,creative_project_id:projectId,error:String(error?.stack||error)}});
...
export async function onRequestPost(context) {
  ...
  try {
    const data=await load(state.db,projectId);
    if(!data.project)return json({ok:false,error:'CAIP project not found.'},404);
    if(!data.readiness.schema_ready)return json({ok:false,schema_ready:false,missing_tables:data.readiness.missing_tables,error:'Release 448 CAIP handoff authority is not fully installed.'},409);
    ...
    if(action!=='prepare'&&action!=='review')return json({ok:false,error:'Unsupported Release 448 CAIP content-handoff action.'},400);
    ...
    if(action==='review') {
      ...
      const packageJson=JSON.stringify({release:RELEASE,creative_project_id:projectId,creative_project_key:data.project.creative_project_key,content_project_id:contentProjectId,product_id:data.project.product_id||null,approved_marker_count:evidence.length,approved_story_evidence_count:new Set(evidence.map(row=>row.creative_story_evidence_id)).size,approved_segment_count:data.segments.length,review_first:true,source_media_copied:false,provider_execution_active:false,publication_active:false});
    ...
    await auditAdminAction(context.env,context.request,state.adminUser,{action_type:`caip_content_handoff_${action}`,target_type:'creative_project',target_id:projectId,target_key:data.project.creative_project_key||null,details:{release:RELEASE,content_project_id:contentProjectId,approved_evidence_count:data.evidence.length,source_media_copied:false,publication_active:false}});
    ...
    await captureRuntimeIncident(context.env,context.request,{incident_scope:'caip_content_handoff',incident_code:'caip_content_handoff_post_failed',severity:'warning',message:error?.message||'CAIP content handoff could not save.',related_user_id:state.adminUser.user_id,details:{release:RELEASE,action,creative_project_id:projectId,error:String(error?.stack||error)}});
