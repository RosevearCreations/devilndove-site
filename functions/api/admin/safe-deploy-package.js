...
const BUILD_LABEL = 'Build 176 release safety controls';
const changedFiles = [
...
async function buildZip(request) {
  ...
  const index = {
    ...
    post_deploy_order: ['database_build171_ledger_repair.sql if needed only', 'database_build173_deployment_preflight.sql', 'database_build174_deployment_preflight_detail.sql', 'database_build175_release_control.sql', 'database_build176_release_safety_controls.sql']
  };
...
function packagePayload() {
  return {
    build_label: BUILD_LABEL,
    schema: [
      'Fresh D1: run database_upgrade_current_pass.sql, then database_build173_deployment_preflight.sql, then database_build174_deployment_preflight_detail.sql, then database_build175_release_control.sql, then database_build176_release_safety_controls.sql.',
      'Partial D1 with Build 171 tables but missing marker: run database_build171_ledger_repair.sql only, then Build 173, Build 174, Build 175, and Build 176.',
      'Do not rerun ALTER TABLE-heavy SQL blocks on a database where those columns already exist.'
    ],
    sql_copy_blocks: {
      fresh_install: ['database_upgrade_current_pass.sql', 'database_build173_deployment_preflight.sql', 'database_build174_deployment_preflight_detail.sql', 'database_build175_release_control.sql', 'database_build176_release_safety_controls.sql'],
      repair_only: ['database_build171_ledger_repair.sql', 'database_build173_deployment_preflight.sql', 'database_build174_deployment_preflight_detail.sql', 'database_build175_release_control.sql', 'database_build176_release_safety_controls.sql']
    },
    ...
    post_deploy_actions: [
      ...
      'Review Product QA bulk preview cards before applying any low-risk fix.',
      ...
      'Run /admin/post-deploy-smoke-tests/ and confirm dashboard/preflight badges before promotion.'
    ]
  };
}
  ...
  if (url.searchParams.get('format') === 'zip') {
    ...
    return new Response(pack.bytes, {
      ...
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="devilndove-safe-deploy-build176.zip"',
