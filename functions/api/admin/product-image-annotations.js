// File: /functions/api/admin/product-image-annotations.js
// Brief description: Gets and updates richer product image annotation and alt text data.
...
export async function onRequestGet(context) {
  ...
  `).bind(product_id).all());
  return json({ ok: true, annotations: rows.map((row) => ({
    product_image_annotation_id: Number(row.product_image_annotation_id || 0),
    product_id: Number(row.product_id || 0), product_image_id: Number(row.product_image_id || 0) || null,
    image_url: row.image_url || '', alt_text: row.alt_text || '', image_title: row.image_title || '',
    caption: row.caption || '', focal_point_x: row.focal_point_x ?? null, focal_point_y: row.focal_point_y ?? null,
    annotation_notes: row.annotation_notes || '', updated_at: row.updated_at || null
  })) });
}
...
export async function onRequestPost(context) {
  ...
  for (const row of annotations.slice(0, 20)) {
    ...
    `).bind(
      product_id,
      Number(row.product_image_id) || null,
      normalizeText(row.image_url) || null,
      normalizeText(row.alt_text) || null,
  ...
  return json({ ok: true, message: 'Image annotations saved.' });
}
