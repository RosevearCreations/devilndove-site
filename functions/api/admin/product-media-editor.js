// Release 467 Build 172 — bounded Product media workspace with canonical-gallery editing.
...
function json(data,status=200,headers={}){return jsonResponse(data,status,{'Cache-Control':'no-store','X-DD-D1-Read-Contract':'product-media-workspace-v172',...headers});}
...
function addImage(list,seen,row,source){
  const url=clean(row?.image_url||row?.public_url);if(!url)return;const k=key(url);if(!k||seen.has(k))return;seen.add(k);
  const canonical=source==='gallery';const rawId=Number(row?.product_image_id||0);
  list.push({...row,image_url:url,source,canonical_gallery:canonical,product_image_id:canonical?rawId:0,recovered_product_image_id:canonical?0:rawId,editable:canonical&&rawId>0});
}
...
export async function onRequestGet(context){
  ...
  try{
    ...
    // Canonical gallery rows win URL de-duplication. Featured/role/asset recovery is display-only.
    if(clean(product.featured_image_url))addImage(images,seen,{product_id:productId,image_url:product.featured_image_url,alt_text:product.name,sort_order:-1},'featured');
    ...
    return json({ok:true,product,images:bounded,delivery:'product-media-workspace-v172',limits:{gallery_rows:20},lazy:{image_detail:true,scoring:true,annotations:true,roles:true},recovery:{multi_source:true,source_order:['gallery','featured','media role','media asset'],recovered_reference_count:bounded.filter((row)=>!row.canonical_gallery).length,recovered_references_editable:false},warnings,r2_listing:false,d1_rows_read:measured},200,{'X-DD-D1-Rows-Read':String(measured)});
  }catch(error){
