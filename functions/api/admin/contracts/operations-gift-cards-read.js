...
async function safeRows(db, sql, key, issues) {
  try {
    ...
    issues.push({ key, message: String(error?.message || error) });
    return [];
  }
}
function emailProvider(env) {
  return String(env?.GIFT_CARD_EMAIL_PROVIDER || env?.EMAIL_PROVIDER || 'manual').trim().toLowerCase() || 'manual';
}
function abuseRows(attempts, lockouts) {
  return attempts.map((row) => {
    ...
    return {
      ...
      severity_label: severityScore >= 75 ? 'high' : severityScore >= 40 ? 'medium' : 'low',
...
export async function onRequestGet(context) {
  ...
  const templates = present.gift_card_delivery_templates
    ? await safeRows(db, `SELECT * FROM gift_card_delivery_templates ORDER BY template_key ASC`, 'templates', issues)
    : [];
  const queue = present.gift_card_delivery_queue
    ? await safeRows(db, `SELECT * FROM gift_card_delivery_queue ORDER BY datetime(queued_at) DESC LIMIT 200`, 'delivery_queue', issues)
    : [];
  const logs = present.gift_card_provider_send_logs
    ? await safeRows(db, `SELECT * FROM gift_card_provider_send_logs ORDER BY datetime(created_at) DESC LIMIT 120`, 'provider_logs', issues)
    : [];
  const attemptsRaw = present.gift_card_lookup_attempts
    ...
    : [];
  const lockouts = present.gift_card_lookup_lockouts
    ? await safeRows(db, `SELECT * FROM gift_card_lookup_lockouts WHERE lockout_status='active' ORDER BY datetime(locked_at) DESC LIMIT 100`, 'lockouts', issues)
    : [];
  const attempts = abuseRows(attemptsRaw, lockouts);
  const cards = present.gift_cards
    ? await safeRows(db, `SELECT * FROM gift_cards ORDER BY datetime(updated_at) DESC, gift_card_id DESC LIMIT 120`, 'cards', issues)
    : [];
  const redemptions = present.gift_card_redemptions
    ? await safeRows(db, `SELECT * FROM gift_card_redemptions ORDER BY datetime(created_at) DESC LIMIT 120`, 'redemptions', issues)
    : [];
  const adminEvents = present.gift_card_admin_events
    ? await safeRows(db, `SELECT * FROM gift_card_admin_events ORDER BY datetime(created_at) DESC LIMIT 120`, 'admin_events', issues)
    : [];
  ...
  return json({
    ...
    provider: emailProvider(context.env),
    templates,
    queue,
    logs,
    attempts,
