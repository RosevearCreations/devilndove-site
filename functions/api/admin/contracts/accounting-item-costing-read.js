// Devil n Dove Build 325 — Accounting-owned GET-only item-costing read contract.
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    if (error instanceof RangeError || error?.code === 'invalid_accounting_month') {
      return json({ ok: false, build: BUILD, contract: CONTRACT_ID, owner: OWNER, error: error?.message }, 400);
    }
    return json({
      ok: false,
      build: BUILD,
      ...
      owner: OWNER,
      error: 'Accounting item-costing read contract failed.',
      error_code: 'accounting_item_costing_read_failed',
