// Release 467 Build 168 — secondary image recovery only for visible Products missing featured images.
// The Product Browser already owns featured_image_url from its page query, so this endpoint
// never re-reads products. It checks only existing secondary image authorities for supplied ids.
// No R2 listing, catalog-wide scan, readiness/quality work or background refresh.
import { getDb, jsonResponse } from '../_lib/adminAudit.js';

const MAX_IDS=24;
function json(data,status=200,headers={}){return jsonResponse(data,status,{'Cache-Control':'no-store','X-DD-D1-Read-Contract':'product-browser-images-v168',...headers});}
...
export async function onRequestPost(context){
  ...
  if(!ids.length)return json({ok:true,images_by_product:{},product_ids:[],delivery:'product-browser-images-v168',recovered_count:0,requested_missing_featured:0,d1_rows_read:0});
  ...
  try{
    ...
    return json({ok:true,images_by_product:images,product_ids:ids,delivery:'product-browser-images-v168',read_policy:'visible_products_missing_featured_secondary_only',source_order:['gallery','media role','media asset'],requested_missing_featured:ids.length,recovered_count:Object.keys(images).length,product_table_read:false,r2_listing:false,warnings,d1_rows_read:rowsRead},200,{'X-DD-D1-Rows-Read':String(rowsRead)});
  }catch(error){
