// Release 467 Build 164 — explicit one-file Product image add/replace authority.
// One R2 PUT plus bounded selected-Product D1 work. No R2 listing, schema introspection, or request-time DDL.
import { auditAdminAction, getDb, jsonResponse } from '../_lib/adminAudit.js';

function json(data,status=200,headers={}){return jsonResponse(data,status,{'Cache-Control':'no-store','X-DD-D1-Contract':'product-image-file-v164',...headers});}
...
export async function onRequestPost(context){
  ...
  let measured=0;let existing=null;
  try{
    if(action==='add'){
      ...
      const selected=await db.prepare('SELECT product_image_id,product_id,image_url,alt_text,sort_order FROM product_images WHERE product_image_id=? AND product_id=? LIMIT 1').bind(imageId,productId).all();measured+=rowsRead(selected);existing=(selected?.results||[])[0]||null;if(!existing)return json({ok:false,error:'Selected Product image was not found.'},404);
    }

    const ext=extension(file),safe=sanitizeName(file.name||`product-${productId}.${ext}`);const objectKey=`products/${productId}/editor/${Date.now()}-${crypto.randomUUID()}-${safe.replace(/\.[^.]+$/,'')}.${ext}`;const buffer=await file.arrayBuffer();
    await bucket.put(objectKey,buffer,{httpMetadata:{contentType:mime,cacheControl:'public, max-age=31536000, immutable'},customMetadata:{product_id:String(productId),product_image_id:action==='replace'?String(imageId):'',uploaded_by_user_id:String(admin.user_id||''),source:'product-image-file-v164'}});
    const publicUrl=`${publicBase(context.env)}/${objectKey}`;const sortOrder=integer(form.get('sort_order'),0,999);const alt=clean(form.get('alt_text')||file.name||'',1000);const width=integer(form.get('width_px'),0,100000);const height=integer(form.get('height_px'),0,100000);const orient=orientation(form.get('image_orientation'),width,height);
    let targetImageId=imageId;
    if(action==='add'){
      const insert=await db.prepare('INSERT INTO product_images (product_id,image_url,alt_text,sort_order,created_at) VALUES (?,?,?,?,CURRENT_TIMESTAMP)').bind(productId,publicUrl,alt||null,sortOrder).run();targetImageId=Number(insert?.meta?.last_row_id||0);if(!targetImageId)return json({ok:false,error:'Image file was uploaded but the Product image row could not be created.'},500);
      await db.prepare(`INSERT INTO product_image_annotations (product_id,product_image_id,image_url,alt_text,width_px,height_px,image_orientation,public_use_status,updated_at) VALUES (?,?,?,?,?,?,?,'internal_review',CURRENT_TIMESTAMP)`).bind(productId,targetImageId,publicUrl,alt||null,width||null,height||null,orient).run();
    }else{
      ...
      const update=await db.prepare(`UPDATE product_image_annotations SET image_url=?,alt_text=COALESCE(?,alt_text),width_px=?,height_px=?,image_orientation=?,merchandising_score=NULL,first_image_score=NULL,updated_at=CURRENT_TIMESTAMP WHERE product_image_id=?`).bind(publicUrl,alt||null,width||null,height||null,orient,targetImageId).run();
      if(Number(update?.meta?.changes||0)<1)await db.prepare(`INSERT INTO product_image_annotations (product_id,product_image_id,image_url,alt_text,width_px,height_px,image_orientation,public_use_status,updated_at) VALUES (?,?,?,?,?,?,?,'internal_review',CURRENT_TIMESTAMP)`).bind(productId,targetImageId,publicUrl,alt||existing.alt_text||null,width||null,height||null,orient).run();
      await db.prepare('DELETE FROM product_image_quality_reviews WHERE product_id=? AND product_image_id=?').bind(productId,targetImageId).run();
      await db.prepare('UPDATE products SET featured_image_url=?,updated_at=CURRENT_TIMESTAMP WHERE product_id=? AND featured_image_url=?').bind(publicUrl,productId,existing.image_url||'').run();
      await db.prepare('UPDATE product_media_role_assignments SET image_url=?,updated_at=CURRENT_TIMESTAMP WHERE product_id=? AND product_image_id=?').bind(publicUrl,productId,targetImageId).run();
    }
    if(typeof context.waitUntil==='function')context.waitUntil(auditAdminAction(context.env,context.request,admin,{action_type:action==='add'?'add_product_image_v164':'replace_product_image_v164',target_type:'product_image',target_id:targetImageId,target_key:objectKey,details:{product_id:productId,public_url:publicUrl,file_size_bytes:size,width_px:width,height_px:height,image_orientation:orient,r2_source_preserved:action==='replace'}}).catch(()=>null));
    return json({ok:true,action,product_id:productId,product_image_id:targetImageId,image_url:publicUrl,object_key:objectKey,width_px:width,height_px:height,image_orientation:orient,d1_rows_read:measured,r2_listing:false,automatic_scoring:false},200,{'X-DD-D1-Rows-Read':String(measured)});
