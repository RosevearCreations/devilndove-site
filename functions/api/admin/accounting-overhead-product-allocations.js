...
async function ensureTable(db) {
  ...
  `).run();
  await db.prepare(`CREATE INDEX IF NOT EXISTS idx_accounting_overhead_product_allocations_month ON accounting_overhead_product_allocations(period_month, ledger_code, product_id)`).run();
  await db.prepare(`CREATE INDEX IF NOT EXISTS idx_accounting_overhead_product_allocations_product ON accounting_overhead_product_allocations(product_id, period_month DESC)`).run();
}
...
export async function onRequestPost(context) {
  ...
  if (!/^\d{4}-\d{2}$/.test(periodMonth)) return json({ ok: false, error: 'period_month must be YYYY-MM.' }, 400);
  if (!ledgerCode) return json({ ok: false, error: 'ledger_code is required.' }, 400);
  if (!Number.isInteger(productId) || productId <= 0) return json({ ok: false, error: 'product_id is required.' }, 400);
  if (!Number.isInteger(amountCents) || amountCents < 0) return json({ ok: false, error: 'amount_cents must be a whole number of cents.' }, 400);

  const product = await db.prepare(`SELECT product_id, product_number, name, status, review_status FROM products WHERE product_id = ? LIMIT 1`).bind(productId).first();
  if (!product) return json({ ok: false, error: 'Product not found.' }, 404);

  if (mode === 'delete' || amountCents === 0) {
    await db.prepare(`DELETE FROM accounting_overhead_product_allocations WHERE period_month = ? AND ledger_code = ? AND product_id = ?`).bind(periodMonth, ledgerCode, productId).run();
    await auditAdminAction(context.env, context.request, adminUser, {
      ...
      details: { period_month: periodMonth, ledger_code: ledgerCode, product_id: productId }
    });
    return json({ ok: true, deleted: true });
  }

  await db.prepare(`
    INSERT INTO accounting_overhead_product_allocations (period_month, ledger_code, product_id, amount_cents, notes, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    ON CONFLICT(period_month, ledger_code, product_id) DO UPDATE SET
      amount_cents = excluded.amount_cents,
      notes = excluded.notes,
      updated_at = CURRENT_TIMESTAMP
  `).bind(periodMonth, ledgerCode, productId, amountCents, notes || null).run();
  ...
  await auditAdminAction(context.env, context.request, adminUser, {
    ...
    target_id: productId,
    target_key: `${periodMonth}:${ledgerCode}:${productId}`,
    details: { period_month: periodMonth, ledger_code: ledgerCode, product_id: productId, amount_cents: amountCents }
  });
