...
async function seedOutputs(db,id,projectType='maker_project'){
  const contentOnly=['content_only','education','research','archive'].includes(String(projectType||'').toLowerCase());
  const excluded=contentOnly?new Set(['etsy_listing','website_product','material_usage','cost_analysis']):new Set();
...
export async function onRequestPost(context){
  ...
  try{
    ...
    if(action==='create_project'){
      ...
      projectId=num(result.meta?.last_row_id); await seedOutputs(access.db,projectId,projectType);
      ...
      const item=await access.db.prepare(`SELECT sii.*,COALESCE(siup.usage_tracking_mode,CASE WHEN LOWER(TRIM(COALESCE(sii.source_type,'')))='tool' THEN 'reusable' ELSE 'exact' END) usage_tracking_mode,COALESCE(siup.minimum_usage_increment,0.001) minimum_usage_increment FROM site_item_inventory sii LEFT JOIN site_inventory_usage_profiles siup ON siup.site_item_inventory_id=sii.site_item_inventory_id WHERE sii.site_item_inventory_id=?1 AND sii.is_active=1`).bind(inventoryId).first();
      ...
      if(stockQuantity>previous+1e-9) throw new Error(`Only ${previous} ${item.stock_unit_label||'unit'} are on hand. ${usageQuantity} ${item.usage_unit_label||'unit'} would consume ${stockQuantity.toFixed(4)} stock unit(s).`);
      ...
      const post=await access.db.prepare(`SELECT creative_project_inventory_post_id FROM creative_project_inventory_posts WHERE creative_project_material_review_id=?1 LIMIT 1`).bind(review.creative_project_material_review_id).first();
      if(post?.creative_project_inventory_post_id){
        await access.db.prepare(`INSERT INTO creative_project_inventory_usage_details (creative_project_inventory_post_id,usage_quantity_consumed,usage_unit_label,stock_quantity_consumed,stock_unit_label,tracking_mode,is_estimated,created_at,updated_at) VALUES (?1,?2,?3,?4,?5,?6,?7,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP) ON CONFLICT(creative_project_inventory_post_id) DO UPDATE SET usage_quantity_consumed=excluded.usage_quantity_consumed,usage_unit_label=excluded.usage_unit_label,stock_quantity_consumed=excluded.stock_quantity_consumed,stock_unit_label=excluded.stock_unit_label,tracking_mode=excluded.tracking_mode,is_estimated=excluded.is_estimated,updated_at=CURRENT_TIMESTAMP`).bind(post.creative_project_inventory_post_id,usageQuantity,item.usage_unit_label||'unit',stockQuantity,item.stock_unit_label||'unit',trackingMode,trackingMode==='estimated'?1:0).run();
        await access.db.prepare(`INSERT INTO site_inventory_usage_movements (site_item_inventory_id,usage_quantity_delta,usage_unit_label,stock_quantity_delta,stock_unit_label,tracking_mode,is_estimated,note,actor_user_id,created_at) VALUES (?1,?2,?3,?4,?5,?6,?7,?8,?9,CURRENT_TIMESTAMP)`).bind(inventoryId,-usageQuantity,item.usage_unit_label||'unit',-stockQuantity,item.stock_unit_label||'unit',trackingMode,trackingMode==='estimated'?1:0,`Creative Project ${projectId}, event ${eventId}.`,access.adminUser.user_id).run();
      }
      message=['log_only','reusable'].includes(trackingMode)?`Usage recorded: ${usageQuantity} ${item.usage_unit_label||'unit'} of ${item.item_name}. Stock was not auto-reduced (${trackingMode}).`:`Inventory posted: ${usageQuantity} ${item.usage_unit_label||'unit'} used from ${item.item_name}, reducing stock by ${stockQuantity.toFixed(4)} ${item.stock_unit_label||'unit'}.`;
