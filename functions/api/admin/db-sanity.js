...
async function getIndexNames(db, tableName) {
  try {
...
export async function onRequestGet(context) {
  ...
  const indexChecks = [];
  for (const tableName of ['catalog_items', 'site_item_inventory', 'product_resource_links', 'accounting_journal_entries', 'schema_migration_ledger']) {
    if (!existing.has(tableName)) continue;
    indexChecks.push({ table_name: tableName, indexes: await getIndexNames(db, tableName) });
  }

  const catalog_counts = existing.has('catalog_items') ? await safeAll(db, `
    SELECT item_kind, COUNT(*) AS total,
           SUM(CASE WHEN COALESCE(amazon_url,'') <> '' THEN 1 ELSE 0 END) AS with_amazon_url
    FROM catalog_items
    WHERE item_kind IN ('tool','supply','creation')
    GROUP BY item_kind
    ORDER BY item_kind ASC
  `) : [];

  const inventory_counts = existing.has('site_item_inventory') ? await safeAll(db, `
  ...
  const journal_balance = existing.has('accounting_journal_entries') ? await safeFirst(db, `
    SELECT COUNT(*) AS unbalanced_entry_count,
           COALESCE(SUM(ABS(imbalance_cents)),0) AS total_imbalance_cents
    FROM accounting_journal_entries
    WHERE COALESCE(imbalance_cents,0) != 0
  `) : { unbalanced_entry_count: 0, total_imbalance_cents: 0 };

  const migration_ledger_summary = existing.has('schema_migration_ledger') ? await safeFirst(db, `
    SELECT COUNT(*) AS total,
           SUM(CASE WHEN status='applied' THEN 1 ELSE 0 END) AS applied_count,
           SUM(CASE WHEN status='failed' THEN 1 ELSE 0 END) AS failed_count,
           SUM(CASE WHEN status='pending_review' THEN 1 ELSE 0 END) AS pending_review_count
    FROM schema_migration_ledger
  `) : { total: 0, applied_count: 0, failed_count: 0, pending_review_count: 0 };

  const criticalChecks = [];
  const catByKind = Object.fromEntries(catalog_counts.map((row) => [row.item_kind, row]));
  const invByType = Object.fromEntries(inventory_counts.map((row) => [row.source_type, row]));
  const toolMismatch = Number(catByKind.tool?.total || 0) !== Number(invByType.tool?.total || 0);
  const supplyMismatch = Number(catByKind.supply?.total || 0) !== Number(invByType.supply?.total || 0);
  if (toolMismatch || supplyMismatch) {
    criticalChecks.push({
      check: 'catalog_inventory_sync',
      status: 'attention_required',
      detail: `Catalog/inventory count mismatch. Tools ${Number(invByType.tool?.total || 0)}/${Number(catByKind.tool?.total || 0)}, Supplies ${Number(invByType.supply?.total || 0)}/${Number(catByKind.supply?.total || 0)}.`,
      action: 'Open /admin/catalog/ and run Sync all tools + supplies.'
    });
  }
  if (Number(journal_balance.unbalanced_entry_count || 0) > 0) {
    criticalChecks.push({
      check: 'journal_balance',
      status: 'attention_required',
      detail: `${Number(journal_balance.unbalanced_entry_count || 0)} unbalanced journal entry/entries.`,
      action: 'Regenerate or correct journal lines before locking a month.'
    });
  }
  if (Number(migration_ledger_summary.failed_count || 0) > 0) {
    criticalChecks.push({
      check: 'migration_ledger',
      status: 'attention_required',
      detail: `${Number(migration_ledger_summary.failed_count || 0)} migration ledger row(s) are failed.`,
      action: 'Open Operations > D1 Migration Ledger and resolve failed rows.'
    });
  }
  ...
  return jsonResponse({
    ...
    summary: {
      ...
      critical_check_count: criticalChecks.length,
      status: missingTables.length || staleTables.length || criticalChecks.length ? 'attention_required' : 'ok',
    },
