...
function buildOfferStage(product, offer) {
  ...
  return stage('offers', 'Quantity specials & limited sets', [
    check('offer_schema', 'Offer controls available', Boolean(offer?.schema_ready), offer?.schema_ready ? 'Build 220 offer and set tables are available.' : 'Apply the Build 220 database migration before using quantity specials or limited sets.', { required: false, href: `/admin/product-editor/?product_id=${product.product_id}&tab=basics` }),
    check('tier_integrity', 'Quantity-price progression', tierIntegrity, tiers.length ? (tierIntegrity ? `${tiers.length} quantity price break${tiers.length===1?'':'s'} decreases or holds the per-item price correctly.` : 'One or more quantity breaks increases the per-item price or has an invalid quantity.') : 'No quantity specials are configured.', { required: tiers.length > 0, href: `/admin/product-editor/?product_id=${product.product_id}&tab=basics` }),
    check('bundle_components', 'Set component definition', !bundleConfigured || components.length > 0, !bundleConfigured ? 'This is not configured as a product set.' : (components.length ? `${components.length} finished-product component${components.length===1?'':'s'} are assigned.` : 'The set has no components.'), { required: bundleConfigured, href: `/admin/product-editor/?product_id=${product.product_id}&tab=basics` }),
    check('bundle_reservation', 'Complete-set availability', !bundleConfigured || requested === 0 || reserved > 0, !bundleConfigured ? 'No set reservation applies.' : (reserved > 0 ? `${reserved} of ${requested} requested complete set${requested===1?'':'s'} are reserved.` : 'No complete set can currently be formed; storefront availability will be zero.'), { required: false, href: `/admin/product-editor/?product_id=${product.product_id}&tab=basics` })
...
function buildCatalogStage(product) {
  return stage('catalog', 'Catalog facts', [
    ...
    check('catalog_review', 'Catalog approval', ['approved', 'published'].includes(product.review_status), ['approved', 'published'].includes(product.review_status) ? 'The catalog review state is approved or published.' : 'Approve the product review before passing it into release work.', { href: `/admin/product-editor/?product_id=${product.product_id}&tab=basics` }),
    check('catalog_status', 'Store status', product.status === 'active', product.status === 'active' ? 'The product is active.' : 'Set an approved product to Active before release review.', { href: `/admin/product-editor/?product_id=${product.product_id}&tab=basics` }),
    check('name', 'Buyer-facing product name', text(product.name).length >= 3, text(product.name).length >= 3 ? 'A buyer-facing name is present.' : 'Add a clear product name.', { href: `/admin/product-editor/?product_id=${product.product_id}&tab=basics` }),
    check('slug', 'Stable product URL', /^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(product.slug), /^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(product.slug) ? 'A clean product slug is present.' : 'Add a lowercase slug using letters, numbers, and hyphens.', { href: `/admin/product-editor/?product_id=${product.product_id}&tab=basics` }),
    check('price', 'Price', Number(product.price_cents || 0) > 0, Number(product.price_cents || 0) > 0 ? 'A price is set.' : 'Set a price greater than $0.', { href: `/admin/product-editor/?product_id=${product.product_id}&tab=basics` }),
    check('category', 'Specific category', text(product.product_category).length > 0, text(product.product_category).length > 0 ? 'A product category is selected.' : 'Choose the most specific category available.', { href: `/admin/product-editor/?product_id=${product.product_id}&tab=basics` }),
    check('description', 'Short description', text(product.short_description).length >= 40, text(product.short_description).length >= 40 ? 'The short description has enough buyer-facing detail.' : 'Write at least one factual, buyer-facing short description.', { href: `/admin/product-editor/?product_id=${product.product_id}&tab=basics` }),
