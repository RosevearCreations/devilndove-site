// Release 467 Build 30 — administrator password reset authority.
...
async function getAdminUserFromRequest(request, env) {
  ...
  const session = await env.DB.prepare(`
    SELECT s.session_id,s.user_id,s.session_token,s.token,s.expires_at,u.user_id AS resolved_user_id,u.email,u.display_name,u.role,u.is_active
  ...
  return { session_id:Number(session.session_id||0), user_id:Number(session.resolved_user_id||session.user_id||0), email:session.email||'', display_name:session.display_name||'', role:session.role||'admin' };
}
...
export async function onRequestPost(context) {
  ...
  if (!Number.isInteger(user_id) || user_id <= 0) return json({ ok:false,error:'A valid user_id is required.' },400);
  ...
  if (new_password !== confirm_password) return json({ ok:false,error:'Passwords do not match.' },400);

  const targetUser = await env.DB.prepare(`SELECT user_id,email,display_name,role,is_active,created_at,updated_at FROM users WHERE user_id=? LIMIT 1`).bind(user_id).first();
  if (!targetUser) return json({ ok:false,error:'User not found.' },404);
  ...
  if (clear_sessions) {
    ...
    if (user_id === adminUser.user_id) sessionIds = sessionIds.filter((id)=>id !== adminUser.session_id);
  ...
  await auditAdminAction(env, request, adminUser, {
    ...
    target_type: 'user',
    target_id: user_id,
    target_key: updatedUser?.email || targetUser.email || String(user_id),
    details: { clear_other_sessions: clear_sessions, deleted_sessions, password_value_emitted: false, password_hash_emitted: false, existing_password_required: false }
  });

  return json({
    ok:true,
    message:'Password reset successfully.',
    reset_by:{ user_id:adminUser.user_id,email:adminUser.email,display_name:adminUser.display_name },
    user:{ user_id:Number(updatedUser?.user_id||user_id||0),email:updatedUser?.email||'',display_name:updatedUser?.display_name||'',role:updatedUser?.role||'member',is_active:Number(updatedUser?.is_active||0),created_at:updatedUser?.created_at||null,updated_at:updatedUser?.updated_at||null },
    sessions:{ cleared_other_sessions:!!clear_sessions,deleted_sessions },
    security:{ existing_password_required:false,password_value_emitted:false,password_hash_emitted:false }
  });
}
