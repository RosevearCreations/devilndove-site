// Release 467 Build 58 — administrator password reset authority with structured runtime failures.
...
export async function onRequestPost(context){
 ...
 try{
  const db=env.DB||env.DD_DB;if(!db)return json({ok:false,error:'Password reset is temporarily unavailable.',code:'ADMIN_RESET_PASSWORD_DB_UNAVAILABLE',hint:'Check the current D1 binding in I.T. readiness.'},503);
  ...
  const targetUser=await db.prepare(`SELECT user_id,email,display_name,role,is_active,created_at,updated_at FROM users WHERE user_id=? LIMIT 1`).bind(user_id).first();if(!targetUser)return json({ok:false,error:'User not found.'},404);
  const password_hash=await formatStoredPasswordHashFromPlaintext(new_password);
  await db.prepare(`UPDATE users SET password_hash=?,updated_at=CURRENT_TIMESTAMP WHERE user_id=?`).bind(password_hash,user_id).run();
  let deleted_sessions=0;
  if(clear_sessions){const result=await db.prepare(`SELECT session_id FROM sessions WHERE user_id=?`).bind(user_id).all();let ids=(Array.isArray(result?.results)?result.results:[]).map(r=>Number(r.session_id)).filter(id=>Number.isInteger(id)&&id>0);if(user_id===adminUser.user_id)ids=ids.filter(id=>id!==adminUser.session_id);if(ids.length){const placeholders=ids.map(()=>'?').join(', ');await db.prepare(`DELETE FROM sessions WHERE session_id IN (${placeholders})`).bind(...ids).run();deleted_sessions=ids.length;}}
  const updatedUser=await db.prepare(`SELECT user_id,email,display_name,role,is_active,created_at,updated_at FROM users WHERE user_id=? LIMIT 1`).bind(user_id).first();
  ...
  return json({ok:false,error:'Password reset is temporarily unavailable.',code:'ADMIN_RESET_PASSWORD_FAILED',hint:'Check the current users/session D1 schema and I.T. runtime diagnostics.',detail},503);
 }
}
