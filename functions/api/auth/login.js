...
export async function onRequestPost(context) {
  ...
  try {
    user = await env.DB.prepare(
      `SELECT user_id, email, password_hash, display_name, role, is_active, created_at, updated_at
       FROM users
    ...
    return json(
      {
        ...
        detail: String(error?.message || error || "Unknown database error."),
      },
      500
    );
  }
  ...
  if (Number(user.is_active || 0) !== 1) {
    return json({ ok: false, error: "This account is inactive." }, 403);
  }
  ...
  try {
    await env.DB.prepare(
      `INSERT INTO sessions (user_id, session_token, token, expires_at, created_at)
  ...
  try {
    session = await env.DB.prepare(
      `SELECT session_id, user_id, session_token, token, expires_at, created_at
       FROM sessions
       WHERE user_id = ?
    ...
    return json(
      {
        ok: false,
        error: "Failed loading created session.",
        detail: String(error?.message || error || "Unknown database error."),
      },
      500
    );
