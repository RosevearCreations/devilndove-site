...
// Release 467 Build 31: all newly registered passwords use the current salted hash authority.
...
function clearSessionCookie(request) { return buildSessionCookie(request, '', 0); }
...
export async function onRequestPost(context) {
  ...
  return json({ ok:true,message:"Account created successfully.",credential_hash_scheme:PASSWORD_HASH_SCHEME,session_token:session?.session_token || sessionToken,token:session?.token || sessionToken,session:{ session_id:Number(session?.session_id || 0),session_token:session?.session_token || sessionToken,token:session?.token || sessionToken,expires_at:session?.expires_at || null,created_at:session?.created_at || null },user:{ user_id:Number(user?.user_id || user_id || 0),email:user?.email || email,display_name:user?.display_name || "",role:user?.role || "member",is_active:Number(user?.is_active || 1),created_at:user?.created_at || null,updated_at:user?.updated_at || null } },201,{ "Set-Cookie":buildSessionCookie(request,session?.session_token || sessionToken) });
}
