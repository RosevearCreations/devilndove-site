...
export async function onRequestPost(context) {
  ...
  if (env.DD_BOOTSTRAP_TOKEN) {
    if (!bootstrap_token) {
      return json({ ok: false, error: "Bootstrap token is required." }, 403);
    }

    if (bootstrap_token !== env.DD_BOOTSTRAP_TOKEN) {
  ...
  if (active_admin_users > 0) {
    return json({
      ok: false,
      error: "Bootstrap is no longer available because an active admin already exists."
    }, 409);
  }
  ...
  if (!user_id) {
    return json({ ok: false, error: "Admin account could not be created." }, 500);
  }
