...
// Release 467 Build 31: verifies either current or legacy hashes, but writes only the current salted format.
import {
...
export async function onRequestPost(context) {
  ...
  if (!(await verifyStoredPasswordHash(current_password,sessionUser.password_hash))) return json({ ok:false,error:"Current password is incorrect." },401);
  ...
  await env.DB.prepare(`UPDATE users SET password_hash=?,updated_at=CURRENT_TIMESTAMP WHERE user_id=?`).bind(new_password_hash,Number(sessionUser.resolved_user_id || sessionUser.user_id || 0)).run();
  const updatedUser = await env.DB.prepare(`SELECT user_id,email,display_name,role,is_active,created_at,updated_at FROM users WHERE user_id=? LIMIT 1`).bind(Number(sessionUser.resolved_user_id || sessionUser.user_id || 0)).first();
  return json({ ok:true,message:"Password changed successfully.",credential_hash_scheme:PASSWORD_HASH_SCHEME,user:{ user_id:Number(updatedUser?.user_id || sessionUser.resolved_user_id || sessionUser.user_id || 0),email:updatedUser?.email || sessionUser.email || "",display_name:updatedUser?.display_name || sessionUser.display_name || "",role:updatedUser?.role || sessionUser.role || "member",is_active:Number(updatedUser?.is_active || sessionUser.is_active || 0),created_at:updatedUser?.created_at || null,updated_at:updatedUser?.updated_at || null } });
}
