import { hasProductReviewSchema } from '../_lib/memberRuntimeSchemaReadiness.js';
...
async function getMemberUserFromRequest(request, env) {
  ...
  const role = String(session.role || '').toLowerCase(); if (!['member','admin'].includes(role)) return null;
...
async function requireReviewSchema(db) {
  if (await hasProductReviewSchema(db)) return null;
  return json({ ok: false, error: 'product_review_schema_unavailable', message: 'Member reviews are temporarily unavailable.' }, 503);
}
...
export async function onRequestGet(context) {
  ...
  const schemaError = await requireReviewSchema(db); if (schemaError) return schemaError;
  const purchasedProducts = normalizeResults(await db.prepare(`
    ...
    FROM orders o INNER JOIN order_items oi ON oi.order_id = o.order_id LEFT JOIN products p ON p.product_id = oi.product_id
    ...
    GROUP BY oi.product_id ORDER BY last_order_at DESC LIMIT 50
  ...
  const reviews = normalizeResults(await db.prepare(`
    ...
    FROM product_reviews pr LEFT JOIN products p ON p.product_id = pr.product_id
...
export async function onRequestPost(context) {
  ...
  const productId = Number(body.product_id || 0) || null; const orderId = Number(body.order_id || 0) || null;
  const rating = Math.min(5, Math.max(1, Number(body.rating || 5) || 5)); const reviewText = normalizeText(body.review_text);
  ...
  await db.prepare(`
    INSERT INTO product_reviews (product_id, order_id, user_id, reviewer_name, reviewer_email, rating, review_text, review_kind, status, is_featured, created_at, updated_at)
