...
export async function onRequestGet(context) {
  ...
  const photos = rows(await db.prepare(`SELECT custom_order_stage_photo_id, custom_request_id, order_id, stage_key, image_url, image_caption, public_use_status, created_at FROM custom_order_stage_photos WHERE (custom_request_id=? OR order_id=?) AND COALESCE(public_use_status,'internal_review') IN ('customer_private','product_page_ok','social_ok','all_public_ok') ORDER BY datetime(created_at) DESC LIMIT 40`).bind(Number(link.custom_request_id || 0), Number(order.order_id || 0)).all().catch(() => ({ results: [] })));
  return json({ ok: true, order, items, stages, photos, link: { link_status: link.link_status || 'active', custom_request_id: Number(link.custom_request_id || 0) || null, order_stage: link.order_stage || 'planning', stage_notes: link.stage_notes || '', stage_updated_at: link.stage_updated_at || '' } });
}
