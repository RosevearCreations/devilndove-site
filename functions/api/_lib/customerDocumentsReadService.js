...
// Performs schema/readiness checks and bounded SELECTs only. No DDL or mutation.
...
const REQUIRED = Object.freeze({
  customer_document_sequences: Object.freeze(['document_type', 'sequence_year', 'next_number']),
  customer_documents: Object.freeze([
    ...
    'source_snapshot_json', 'issued_at', 'voided_at', 'void_reason',
  ]),
...
async function loadOrderDetail(db, orderId, ready) {
  ...
  const items = ready.order_items ? rows(await db.prepare(`
    SELECT order_item_id,product_id,sku,product_name,product_type,unit_price_cents,
           quantity,line_subtotal_cents,taxable,tax_class_code,requires_shipping
    FROM order_items WHERE order_id=? ORDER BY order_item_id
  `).bind(id).all().catch(() => ({ results: [] }))) : [];
  const payments = ready.payments ? rows(await db.prepare(`
    SELECT payment_id,provider,provider_payment_id,payment_status,amount_cents,currency,
           payment_method_label,transaction_reference,paid_at,created_at
    FROM payments WHERE order_id=? ORDER BY created_at,payment_id
  `).bind(id).all().catch(() => ({ results: [] }))) : [];
  const refunds = ready.payment_refunds ? rows(await db.prepare(`
    SELECT refund_id,payment_id,provider,provider_refund_id,amount_cents,currency,
           refund_status,reason,note,created_at,updated_at
    FROM payment_refunds WHERE order_id=? ORDER BY created_at,refund_id
  `).bind(id).all().catch(() => ({ results: [] }))) : [];
...
export async function readCustomerDocuments(db, { orderId = 0, documentId = 0 } = {}) {
  ...
  const check = await readiness(db);
  ...
  const orders = check.ready.orders ? rows(await db.prepare(`
    ...
    FROM orders o
    ORDER BY o.created_at DESC,o.order_id DESC LIMIT 300
  `).all().catch(() => ({ results: [] }))) : [];

  const documents = check.ready.customer_documents ? rows(await db.prepare(`
    SELECT customer_document_id,document_number,document_type,order_id,refund_id,document_status,
           currency,document_amount_cents,tax_adjustment_cents,issue_reason,customer_email,
           business_name,business_registration_number,issued_at,voided_at,void_reason
    FROM customer_documents
    ORDER BY issued_at DESC,customer_document_id DESC LIMIT 300
  `).all().catch(() => ({ results: [] }))) : [];

  let documentDetail = null;
  const docId = positiveId(documentId);
  if (docId && check.ready.customer_documents) {
    const row = await db.prepare('SELECT * FROM customer_documents WHERE customer_document_id=? LIMIT 1')
