...
export function convergeProductMedia({ product = {}, seo = {}, galleryRows = [], mediaRows = [], roleRows = [], annotationRows = [], qualityRows = [], assessmentRows = [] } = {}) {
  ...
  for (const candidate of candidates.filter(Boolean)) {
    ...
    if (!existing || candidate.source_rank < existing.source_rank) byKey.set(candidate.canonical_key, candidate);
    else {
      // Preserve richer metadata discovered on lower-priority evidence without changing canonical ownership.
      for (const field of ['role_key','role_assignment_id','public_use_status','consent_record_id','focal_x','focal_y','width_px','height_px','image_orientation','merchandising_score','quality_score','quality_source','quality_status','quality_reviewed_at','storage_provider','bucket_name','object_key']) {
        if ((existing[field] == null || existing[field] === '' || existing[field] === 'internal_review' || existing[field] === 'none') && candidate[field] != null && candidate[field] !== '') existing[field] = candidate[field];
      }
      if (!existing.alt_text && candidate.alt_text) existing.alt_text = candidate.alt_text;
    }
  }
  ...
  images.forEach((image, index) => {
    ...
    image.gallery_position = image.source === 'product_images' ? index + 1 : null;
  ...
  const featured = images.find((image) => image.is_featured) || images.find((image) => image.source === 'product_images') || images[0] || null;
  ...
  const sourceCounts = {};
  for (const image of images) sourceCounts[image.source] = Number(sourceCounts[image.source] || 0) + 1;
  ...
  return {
    ...
    summary: {
      ...
      gallery_image_count: images.filter((image) => image.source === 'product_images').length,
      support_recovery_count: images.filter((image) => image.source !== 'product_images').length,
