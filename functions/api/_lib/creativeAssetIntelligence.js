...
export async function ensureCreativeAssetIntelligenceSchema(db) {
  // Build 241: schema is migration-owned. Runtime requests verify; they never create tables or indexes.
  try {
    await db.prepare(`SELECT creative_project_id FROM creative_projects LIMIT 1`).all();
    await db.prepare(`SELECT creative_asset_id FROM creative_assets LIMIT 1`).all();
    await db.prepare(`SELECT creative_story_evidence_id FROM creative_story_evidence LIMIT 1`).all();
    await db.prepare(`SELECT creative_project_event_id FROM creative_project_events LIMIT 1`).all();
  } catch {
    throw new Error('CAIP schema is not installed. Back up D1 and apply the current migration chain through Build 241 before using Creative Asset Intelligence.');
  }
}
