...
export async function openInventoryKit(db,adminUser,{inventory_kit_template_id,kit_quantity_opened=1,source_lot_code='',note=''}={}){
  ...
  try{
    ...
    for(let index=0;index<resolved.length;index+=1){
      ...
      const newUnit=newQty>EPSILON?Math.round(((oldQty*oldCost)+allocated)/newQty):oldCost; const componentUnit=addQty>EPSILON?Math.round(allocated/addQty):0;
      ...
      state.children.push({id:positiveId(item.site_item_inventory_id),oldQty,oldCost,newQty});
      ...
      if(clean(item.source_type,20).toLowerCase()==='supply' && addQty>EPSILON){
...
async function compensateUsage(db,state){
  ...
  for(const lot of [...state.lots].reverse()){
    const r=await db.prepare(`UPDATE inventory_purchase_lots SET quantity_remaining=?,updated_at=CURRENT_TIMESTAMP WHERE inventory_purchase_lot_id=? AND ABS(quantity_remaining-?)<?`).bind(lot.before,lot.id,lot.after,EPSILON).run().catch(()=>null);
    if(Number(r?.meta?.changes||0)!==1) failures.push(`lot:${lot.id}`);
  }
...
export async function consumeKitComponent(db,adminUser,{inventory_kit_template_component_id,usage_quantity,note=''}={}){
  ...
  try{
    ...
    state.item={id:positiveId(row.site_item_inventory_id),before:plan.previous_on_hand_quantity,after:plan.new_on_hand_quantity};
    for(const allocation of lotPlan.allocations||[]){
      const before=round6(allocation.quantity_remaining); const after=round6(allocation.quantity_remaining_after);
      ...
      state.lots.push({id:positiveId(allocation.inventory_purchase_lot_id),before,after});
    }
    ...
    if(plan.source_type==='supply'){
      await db.prepare(`UPDATE inventory_lot_policies SET reconcile_status=CASE WHEN ABS(?-COALESCE((SELECT SUM(COALESCE(quantity_remaining,0)) FROM inventory_purchase_lots WHERE site_item_inventory_id=? AND lot_status<>'returned'),0))<? THEN 'reconciled' ELSE 'needs_review' END,last_reconciled_quantity=?,last_reconciled_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE site_item_inventory_id=?`).bind(plan.new_on_hand_quantity,row.site_item_inventory_id,EPSILON,plan.new_on_hand_quantity,row.site_item_inventory_id).run();
    }
