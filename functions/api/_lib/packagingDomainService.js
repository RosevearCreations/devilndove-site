...
export async function onRequestPost(context){
  ...
  try{
    ...
    if(action==='record_translation_draft'){
      ...
      const theme=safeJson(template.theme_json,{});const layout=safeJson(template.layout_json,{});const profile=String(layout.design_profile||'');const rosePreset=productRosePreset(body.collection_name||productName);if(template.package_type==='soap_ribbon'){theme.rose_colour=rosePreset.colour;theme.secondary_colour=rosePreset.colour;}const artwork={rose_asset_id:template.package_type==='soap_ribbon'?rosePreset.asset:'rose-purple-v1',badge_shape:layout.shape==='oval'?'oval':'oval',rose_style:'full_rose',top_arc_text:layout.default_top_arc_text||'',bottom_arc_text:layout.default_bottom_arc_text||'',candle_primary_text:layout.default_primary_text||'',candle_date_line_1:layout.default_date_line_1||'',candle_event_line:layout.default_event_line||'',candle_date_line_2:layout.default_date_line_2||'',artwork_asset:template.package_type==='soap_ribbon'?'':(layout.artwork_asset||''),cupcake_preset_key:profile==='cupcake_soap_square_v1'?text(layout.cupcake_preset_key,80):'',cupcake_purpose_text:profile==='cupcake_soap_square_v1'?text(layout.default_cupcake_purpose_text,300):''};
