...
const VALID_PACKAGE_TYPES = new Set(['soap_ribbon','soap_cupcake_label','product_label','candle_label','candle_top','engraved_round','jewelry_card','package_insert','shipping_label','gift_set']);
const SOAP_PACKAGE_TYPES = new Set(['soap_ribbon','soap_cupcake_label']);
...
async function syncSoapRecords(db,projectId,data){
  ...
  if(!SOAP_PACKAGE_TYPES.has(String(data.package_type||'')))return null;
...
export async function onRequestPost(context){
  ...
  try{
    ...
    if(action==='record_translation_draft'){
      ...
      const allowedShapes=new Set(['rectangle','round','oval','square','soap_wrap']);const shape=allowedShapes.has(text(body.shape,30))?text(body.shape,30):(packageType==='candle_top'||packageType==='engraved_round'?'round':packageType==='soap_cupcake_label'?'square':'rectangle');
      const allowedProfiles=new Set(['soap_reference_v2','soap_reference_v3','cupcake_soap_square','candle_top_wedding','candle_top_centered','round_maker_mark','general_centered','general_rectangle']);
      const requestedDesignProfile=allowedProfiles.has(text(body.design_profile,60))?text(body.design_profile,60):(shape==='round'?'candle_top_centered':packageType==='soap_cupcake_label'?'cupcake_soap_square':'general_centered');const designProfile=packageType==='soap_ribbon'?'soap_reference_v3':packageType==='soap_cupcake_label'?'cupcake_soap_square':requestedDesignProfile;
      ...
      const theme=safeJson(template.theme_json,{});const layout=safeJson(template.layout_json,{});const rosePreset=productRosePreset(body.collection_name||productName);const isRibbon=template.package_type==='soap_ribbon';const isCupcake=template.package_type==='soap_cupcake_label';if(isRibbon){theme.rose_colour=rosePreset.colour;theme.secondary_colour=rosePreset.colour;}const artwork={rose_asset_id:isRibbon?rosePreset.asset:'rose-purple-v1',badge_shape:layout.shape==='oval'?'oval':isCupcake?'square':'oval',rose_style:'full_rose',top_arc_text:layout.default_top_arc_text||'',bottom_arc_text:layout.default_bottom_arc_text||'',candle_primary_text:layout.default_primary_text||'',candle_date_line_1:layout.default_date_line_1||'',candle_event_line:layout.default_event_line||'',candle_date_line_2:layout.default_date_line_2||'',artwork_asset:isRibbon?'':(layout.artwork_asset||''),cupcake_reference_id:isCupcake?(layout.cupcake_reference_id||'sweet-orange'):'',cupcake_label_title:isCupcake?(layout.default_label_title||'Cupcake Soap'):'',cupcake_purpose_text:isCupcake?(layout.default_purpose||'hand & body soap'):''};
      ...
      if(SOAP_PACKAGE_TYPES.has(String(template.package_type||''))&&productId){
      ...
      message=template.package_type==='soap_ribbon'?'Soap-label project created from the approved ribbon structure; marked product ingredients were carried into the review draft.':template.package_type==='soap_cupcake_label'?'Editable 2 × 2 inch Soap Cupcake label project created; marked product ingredients were carried into the review draft.':template.package_type==='candle_top'?'Editable candle-top project created from the selected round template.':'Labeling and packaging project created from the selected business-wide template.';
