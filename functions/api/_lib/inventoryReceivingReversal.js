...
export async function reverseReceivingClaim(db, { inventory_receiving_claim_id, reversal_key, reversal_reason }, actorUserId) {
  ...
  if (preview.po_line) {
    ...
    statements.push(db.prepare(`UPDATE supplier_purchase_order_items SET quantity_received=?,received_at=CASE WHEN ?>0 THEN received_at ELSE NULL END WHERE supplier_purchase_order_item_id=? AND ABS(COALESCE(quantity_received,0)-?)<? AND COALESCE(quantity_received,0)>=? AND EXISTS(SELECT 1 FROM inventory_receiving_reversals WHERE reversal_key=? AND inventory_receiving_claim_id=?)`).bind(preview.next_po_received_quantity, preview.next_po_received_quantity, id(preview.po_line.supplier_purchase_order_item_id), num(preview.po_line.quantity_received,0), EPSILON, preview.quantity_reversed, key, claimId));
    statements.push(db.prepare(`UPDATE supplier_purchase_orders SET status='ordered',received_completed_at=NULL,updated_at=CURRENT_TIMESTAMP WHERE supplier_purchase_order_id=? AND status='received' AND EXISTS(SELECT 1 FROM inventory_receiving_reversals WHERE reversal_key=? AND inventory_receiving_claim_id=?)`).bind(id(preview.po_line.supplier_purchase_order_id), key, claimId));
  }
