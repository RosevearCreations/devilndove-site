import { hasProductInterestSchema } from "./_lib/publicRuntimeSchemaReadiness.js";
...
export async function onRequestPost(context) {
  ...
  if (!(await hasProductInterestSchema(db))) return json({ ok: false, error: 'product_interest_schema_unavailable', message: 'Product interest requests are temporarily unavailable.' }, 503);
  ...
  const existing = await db.prepare(`
    SELECT product_interest_request_id FROM product_interest_requests
